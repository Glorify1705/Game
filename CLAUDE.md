...
games/               Example game projects (space-garbage, platformer, flappybird)
assets/              Test programs (Lua/Fennel), sprites, shaders, audio
tests/               GoogleTest test suite (272 tests, 14 files)
...
- **Lua bindings**: `lua_*.cc` files (23 files) expose the `G.*` API to
  ...
  (regenerated by `game stubs`); the README has a curated summary.
- **CLI commands**: `cmd_*.cc` (`cmd_run.cc`, `cmd_package.cc`, `cmd_init.cc`,
  `cmd_stubs.cc`, `cmd_completions.cc`, etc.)
