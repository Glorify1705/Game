 ...
 */
static double bench_hash_internal(BMK_benchFn_t hashfn, void* payload,
                                  size_t nbBlocks, sizeFunction_f selectSize,
                                  size_t size, unsigned total_time_ms,
                                  unsigned iter_time_ms) {
  BMK_timedFnState_shell shell;
  BMK_timedFnState_t* const txf = BMK_initStatic_timedFnState(
      &shell, sizeof(shell), total_time_ms, iter_time_ms);
  assert(txf != NULL);
...
static size_t benchLatency(const void* src, size_t srcSize, void* dst,
...
double bench_hash(BMK_benchFn_t hashfn, BMK_benchMode benchMode, size_t size,
                  BMK_sizeMode sizeMode, unsigned total_time_ms,
                  unsigned iter_time_ms) {
  sizeFunction_f const sizef =
      (sizeMode == BMK_fixedSize) ? identity : rand_1_N;
  BMK_benchFn_t const benchfn =
      (benchMode == BMK_throughput) ? hashfn : benchLatency;
  ...
  return bench_hash_internal(benchfn, payload, nbBlocks, sizef, size,
