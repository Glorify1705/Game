 ...
 * directly. Instead, use its assigned methods : BMK_isSuccessful_runOutcome,
 * BMK_extract_runTime, BMK_extract_errorResult. The structure is only described
 * here to allow its allocation on stack. */
 ...
 * "default" : all parameters must be specified by the caller. optional
 * parameters are labelled explicitly, and accept value NULL when not used */
typedef struct {
  ...
  BMK_errorFn_t
      errorFn; /* errorFn will check each return value of benchFn over each
                * block, to determine if it failed or not. errorFn can be NULL,
                * in which case no check is performed. errorFn must return 0
                * when benchFn was successful, and >= 1 if it detects an error.
  ...
  size_t blockCount; /* number of blocks to operate benchFn on.
                      ...
                      * blockResults */
  const void* const*
      srcBuffers; /* read-only array of buffers to be operated on by benchFn */
  ...
  void* const* dstBuffers; /* array of buffers to be written into by benchFn.
                              This array is not optional, it must be provided
                              even if unused by benchfn. */
  const size_t* dstCapacities; /* read-only array containing capacities of
                                  dstBuffers. This array must be present. */
  size_t* blockResults; /* Optional: store the return value of benchFn for each
                           block. Use NULL if this result is not requested. */
} BMK_benchParams_t;
 ...
 * successful. If yes, extract the result with BMK_extract_runTime(), it will
 * contain : .sumOfReturn : the sum of all return values of benchFn through all
 * of blocks .nanoSecPerRun : time per run of benchFn + (time for initFn /
 * nbLoops) .sumOfReturn is generally intended for functions which return a # of
 * bytes written into dstBuffer, in which case, this value will be the total
 * amount of bytes written into dstBuffer.
 *
 ...
 * over all blocks. when provided (!= NULL), and when benchmark failed,
 ...
 * `benchFn`. Returns the faulty return value. note : this function will abort()
 * program execution if benchmark did not failed. always check if benchmark
 * failed first !
 */
 ...
 * at interval of about run_ms. Note : minimum `nbLoops` is 1, therefore a run
 * may last more than run_ms, and possibly even more than total_ms. Usage -
 * initialize timedFnState, select benchmark duration (total_ms) and each
 * measurement duration (run_ms) call BMK_benchTimedFn() repetitively, each
 * measurement is supposed to last about run_ms Check if total time budget is
 * spent or exceeded, using BMK_isCompleted_TimedFn()
 */
