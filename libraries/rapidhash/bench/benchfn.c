        ...
        if (loopNb == 0) {
          ...
          if ((p.errorFn != NULL) && (p.errorFn(res))) {
            RETURN_QUIET_ERROR(BMK_runOutcome_error(res),
                               ...
                               "%u) with error %i",
                               blockNb, (unsigned)p.srcSizes[blockNb],
                               (int)res);
          }
          dstSize += res;
        }
...
BMK_timedFnState_t* BMK_initStatic_timedFnState(void* buffer, size_t size,
                                                ...
                                                unsigned run_ms) {
  typedef char
      check_size[2 * (sizeof(BMK_timedFnState_shell) >=
  ...
  size_t const tfs_alignment =
      offsetof(tfs_align, tfs); /* provides the minimal alignment restriction
                                   for BMK_timedFnState_t */
...
void BMK_resetTimedFnState(BMK_timedFnState_t* timedFnState, unsigned total_ms,
  ...
  timedFnState->fastestRun.nanoSecPerRun =
      ...
      2000000000; /* hopefully large enough : must be larger than any potential
                     measurement */
