 ...
 * This algorithm generates unique inputs flipping on average 16 bits per
 * candidate. It is generally much more friendly for most hash algorithms,
 * especially weaker ones, as it shuffles more the input. The algorithm also
 * avoids overfitting the per4 or per8 ingestion patterns.
 */
...
static void init_sampleFactory(sampleFactory* sf, uint64_t htotal) {
  ...
  for (size_t n = 0; n < sf->size; n++) p[n] = randomByte(n);
  sf->hnb = 0;
}

static sampleFactory* create_sampleFactory(size_t size, uint64_t htotal,
                                           uint64_t seed) {
...
static void init_sampleFactory(sampleFactory* sf, uint64_t htotal) {
  ...
  unsigned char* const p = (unsigned char*)sf->buffer;
  for (size_t n = 0; n < sf->size; n++) p[n] = randomByte(sf->prngSeed + n);
}
...
void free_Filter(Filter* bf) { free(bf); }
...
inline int Filter_insert(Filter* bf, int bflog, uint64_t hash) {
  ...
  ((unsigned char*)bf)[pos] |=
      (unsigned char)(nextValue[existingCandidates] << shift);
...
#define NB_BITS_MAX \
  64 /* can't store nor analyze hash wider than 64-bits for the time being */
...
#define DISPLAY(...)                  \
  {                                   \
    if (display) printf(__VA_ARGS__); \
  }

static int isEqual(void* hTablePtr, size_t index1, size_t index2,
                   Htype_e htype) {
  if ((htype == ht64) || (htype == ht32)) {
...
static int isHighEqual(void* hTablePtr, size_t index1, size_t index2,
                       Htype_e htype, int rShift) {
  uint64_t h1, h2;
  if ((htype == ht64) || (htype == ht32)) {
...
static void addHashCandidate(void* hTablePtr, UniHash h, Htype_e htype,
                             size_t index) {
  if ((htype == ht64) || (htype == ht32)) {
...
static size_t search_collisions(searchCollisions_parameters param) {
  ...
  /* init */
  sampleFactory* const sf =
      create_sampleFactory(sampleSize, totalH, param.prngSeed);
  if (!sf) EXIT("not enough memory");
  ...
  if (filter) {
...

    {
      ...
      DISPLAY(
          " Generation and filter completed in %s, detected up to %llu "
          "candidates \n",
  ...
  if (nbCandidates < maxNbH) {
    ...
    assert(checkPtr ==
           hashCandidates); /* simplification: since we are reducing the size,
  ...
  if (!filter /* all candidates */ && display /*single thead*/) {
    ...
    for (int nbHBits = 1; nbHBits < hashBits; nbHBits++) {
      ...
      double const expectedCollisions =
          estimateNbCollisions(nbCandidates, nbHBits);
      if ((nbSlots >
           nbCandidates *
               100) /* within range for meaningful collision analysis results */
        ...
        double const collisionRatio =
            (double)HBits_collisions / expectedCollisions;
...
void time_collisions(searchCollisions_parameters param) {
  ...
  DISPLAY("===>   Found %llu collisions (x%.2f, %.1f expected) in %s\n",
          (unsigned long long)collisions, (double)collisions / targetColls,
          targetColls, displayDelay(programTDelay));
  if (programBytesSelf)
    DISPLAY("===>   MaxRSS(self) %zuMB, MaxRSS(children) %zuMB\n",
            programBytesSelf >> 20, programBytesChildren >> 20);
  DISPLAY("------------------------------------------ \n");
}
 ...
 * Will also modify `*stringPtr`, advancing it to the position where it stopped
 * reading.
 */
...
void help(const char* exeName) {
  ...
  printf(
      "  --nbh=NB       Select nb of hashes to generate (%llu by default) \n",
      (unsigned long long)select_nbh(64));
  printf(
      "  --filter       Activates the filter. Slower, but reduces memory usage "
      "for the same nb of hashes.\n");
...
int main(int argc, const char** argv) {
  ...
  for (arg_nb = 1; arg_nb < argc; arg_nb++) {
    ...
    if (longCommandWArg(arg, "-T")) {
      threadlog = (int)readU64FromChar(arg);
      continue;
    }

    if (!strcmp(*arg, "--filter")) {
      filter = 1;
      continue;
    }
    ...
    if (longCommandWArg(arg, "--seed")) {
      prngSeed = readU64FromChar(arg);
      continue;
    }
    if (longCommandWArg(arg, "--nbh=")) {
      totalH = readU64FromChar(arg);
      continue;
    }
    if (longCommandWArg(arg, "--filter=")) {
      filter = 1;
      bflog = (int)readU64FromChar(arg);
      assert(bflog < 64);
      continue;
    }
    if (longCommandWArg(arg, "--filterlog=")) {
      filter = 1;
      bflog = (int)readU64FromChar(arg);
      assert(bflog < 64);
      continue;
    }
    if (longCommandWArg(arg, "--size=")) {
      sampleSize = (size_t)readU64FromChar(arg);
      continue;
    }
    if (longCommandWArg(arg, "--len=")) {
      sampleSize = (size_t)readU64FromChar(arg);
      continue;
    }
    if (longCommandWArg(arg, "--threadlog=")) {
      threadlog = (int)readU64FromChar(arg);
      continue;
    }
    ...
    for (hnb = 0; hnb < HASH_FN_TOTAL; hnb++) {
      if (!strcmp(*arg, hashfnTable[hnb].name)) {
        hashID = hnb;
        break;
      }
    }
  ...
  if (sizeof(size_t) < 8) {
    printf(
        "warning: in 32-bit mode, the program is more likely going to lack "
        "address space \n");
  }
  ...
  if (nbThreads == 1) {
    ...
    searchCollisions_results* const MTresults =
        calloc(sizeof(searchCollisions_results), (size_t)nbThreads);
    if (!MTresults) EXIT("not enough memory");
    searchCollisions_parameters* const MTparams =
        calloc(sizeof(searchCollisions_parameters), (size_t)nbThreads);
    ...
    printf("===>   Found %llu collisions (x%.2f, %.1f expected) in %s\n",
           (unsigned long long)nbCollisions, (double)nbCollisions / targetColls,
           targetColls, displayDelay(programTDelay));
    if (programBytesSelf)
