...
// Guard against -Wunused-function on platforms without file tests.
        ...
        void GenerateTableTextTest(const std::string& tests_data_path)
        {
            ...
            std::string jsonfile;
            bool ok = flatbuffers::LoadFile((tests_data_path + "monster_test.fbs").c_str(),
                          false, &schemafile)
                && flatbuffers::LoadFile((tests_data_path + "monsterdata_test.json").c_str(),
                    false, &jsonfile);
            TEST_EQ(ok, true);
            auto include_test_path = flatbuffers::ConCatPathFileName(tests_data_path, "include_test");
        ...
        {
            const auto name_clash_path = flatbuffers::ConCatPathFileName(tests_data_path, "name_clash_test");
            const char* include_directories[] = { name_clash_path.c_str() };

            // Load valid 2 file Flatbuffer schema
            const auto valid_path = flatbuffers::ConCatPathFileName(name_clash_path, "valid_test1.fbs");
            ...
            const auto invalid_path = flatbuffers::ConCatPathFileName(name_clash_path, "invalid_test1.fbs");
        ...
        void UnionVectorTest(const std::string& tests_data_path)
        {
            ...
            // create Movie.
            const auto movie_offset = CreateMovie(fbb, Character_Rapunzel,
            ...
            auto s = flatbuffers::FlatBufferToString(fbb.GetBufferPointer(), MovieTypeTable());
            TEST_EQ_STR(
                s.c_str(),
        ...
        void CreateSharedStringTest()
        {
            ...
            const auto vector_offset = builder.CreateVector<flatbuffers::Offset<flatbuffers::String>>(array);
        ...
        void FlatbuffersSpanTest()
        {
            ...
            enum class Dummy : uint16_t { Zero = 0,
                One,
                Two };
...
// VS10 does not support typed enums, exclude from tests
        ...
        void FixedLengthArraySpanTest(const std::string& tests_data_path)
        {
            ...
            {
                flatbuffers::span<const MyGame::Example::TestEnum, 2> const_d_c = flatbuffers::make_span(*const_nested.c());
                ...
                TEST_ASSERT(mutable_d_c.end() == std::copy(const_d_c.begin(), const_d_c.end(), mutable_d_c.begin()));
                TEST_ASSERT(
            ...
            {
                flatbuffers::span<const int32_t, 2> const_d_a = flatbuffers::make_span(*const_nested.a());
                ...
                TEST_ASSERT(mutable_d_a.end() == std::copy(const_d_a.begin(), const_d_a.end(), mutable_d_a.begin()));
                TEST_ASSERT(
        ...
        {
            {
                ...
                const std::vector<unsigned char> inv_data = { 1, 2, 3 };
                {
                    ...
                    auto empty_i64_vec = fbb.CreateVector(static_cast<const int64_t*>(nullptr), 0);
...

            {
                ...
                const auto& array_table = *flatbuffers::GetRoot<ArrayTable>(fbb.GetBufferPointer());
        ...
        void VectorSpanTest()
        {
            ...
            { // using references
                ...
                flatbuffers::span<const uint8_t> const_inventory = flatbuffers::make_span(*monster->inventory());
                TEST_EQ(const_inventory.size(), 10);
                TEST_EQ(const_inventory[0], 0);
                TEST_EQ(const_inventory[9], 9);

                flatbuffers::span<uint8_t> mutable_inventory = flatbuffers::make_span(*mutable_monster->mutable_inventory());
...

            {
                ...
                flatbuffers::span<const uint8_t> const_nested = flatbuffers::make_span(monster->testnestedflatbuffer());
                TEST_ASSERT(const_nested.empty());

                flatbuffers::span<uint8_t> mutable_nested = flatbuffers::make_span(mutable_monster->mutable_testnestedflatbuffer());
        ...
        void NativeInlineTableVectorTest()
        {
            ...
            auto* root = flatbuffers::GetRoot<TestNativeInlineTable>(fbb.GetBufferPointer());
...
int main(int argc, const char* argv[])
{
    ...
    tests_data_path = FLATBUFFERS_STRING(FLATBUFFERS_TEST_PATH_PREFIX) + tests_data_path;
#endif
