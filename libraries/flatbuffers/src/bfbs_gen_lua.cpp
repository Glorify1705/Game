...
namespace {
    ...
    public:
        ...
        {
            if (!GenerateLua(parser, path, filename)) {
                return ERROR;
            }
            return OK;
        }
            ...
            const flatbuffers::Vector<flatbuffers::Offset<r::Enum>>* enums)
        {
            ForAllEnums(enums, [&](const r::Enum* enum_def) {
                ...
                ForAllEnumValues(enum_def, [&](const reflection::EnumVal* enum_val) {
                    ...
                    code += "  " + namer_.Variant(enum_val->name()->str()) + " = " + NumToString(enum_val->value()) + ",\n";
                });
                code += "}\n";
                code += "\n";
            ...
            const r::Object* root_object)
        {
            ForAllObjects(objects, [&](const r::Object* object) {
                ...
                ForAllFields(object, /*reverse=*/false, [&](const r::Field* field) {
                    ...
                    const std::string getter_signature = "function mt:" + field_name + "()\n";
                    ...
                    if (IsScalar(base_type)) {
                        ...
                        if (object->is_struct()) {
                            ...
                            // self.view.pos.
                            code += "  return " + GenerateGetter(field->type()) + "self.view.pos + " + NumToString(field->offset()) + ")\n";
                        } else {
                            // Table accessors
                            code += "  " + offset_prefix;
                            code += "  " + offset_prefix_2;

                            std::string getter = GenerateGetter(field->type()) + "self.view.pos + o)";
                            if (IsBool(base_type)) {
                                getter = "(" + getter + " ~=0)";
                            }
                            code += "    return " + getter + "\n";
                        ...
                        case r::String: {
                            ...
                            code += "  " + offset_prefix;
                            code += "  " + offset_prefix_2;
                            code += "    return " + GenerateGetter(field->type()) + "self.view.pos + o)\n";
                            code += "  end\n";
                            code += "end\n";
                            code += "\n";
                            break;
                        ...
                        case r::Obj: {
                            if (object->is_struct()) {
                                ...
                                code += "    local x = " + std::string(field_object->is_struct() ? "self.view.pos + o\n" : "self.view:Indirect(self.view.pos + o)\n");
                        ...
                        case r::Vector: {
                            ...
                            if (IsStructOrTable(vector_base_type)) {
                                ...
                                if (IsTable(field->type(), /*use_element=*/true)) {
                                    ...
                                    // size of the struct.
                                    const reflection::Object* obj = GetObjectByIndex(field->type()->index());
                                    element_size = obj->bytesize();
                                }

                                // Include the referenced type, thus we need to make sure
                                // we set `use_element` to true.
                                const std::string require_name = RegisterRequires(field, /*use_element=*/true);
                                code += "    local obj = " + require_name + ".New()\n";
                                code += "    obj:Init(self.view.bytes, x)\n";
                                code += "    return obj\n";
                            } else {
                            ...
                            // supported.
                            if (!IsStructOrTable(vector_base_type)) {
                                code += "  return " + std::string(vector_base_type == r::String ? "''\n" : "0\n");
                            }
                            code += "end\n";
                            code += "\n";
                ...
                if (object->is_struct()) {
                    code += "function " + object_name + ".Create" + object_name + "(builder" + GenerateStructBuilderArgs(object) + ")\n";
                    ...
                    ForAllFields(object, /*reverse=*/false, [&](const r::Field* field) {
                        ...
                        if (IsVector(field->type()->base_type())) {
                            code += "function " + object_name + ".Start" + field_name + "Vector(builder, numElems)\n";
                            ...
                            code += "  return builder:StartVector(" + NumToString(element_size) + ", numElems, " + NumToString(alignment) + ")\n";
                            code += "end\n";
                            code += "\n";
                        }
                    });
    ...
    private:
        ...
        {
            std::string code;
            code += "  builder:Prep(" + NumToString(object->minalign()) + ", " + NumToString(object->bytesize()) + ")\n";
            ...
            ForAllFields(object, /*reverse=*/true, [&](const r::Field* field) {
                ...
                if (IsStructOrTable(field->type()->base_type())) {
                    ...
                    code += "  builder:Prepend" + GenerateMethod(field) + "(" + prefix + namer_.Variable(*field) + ")\n";
                }
            });
            ...
            bool element_type = false) const
        {
            ...
            default:
                return "self.view:Get(flatbuffers.N." + namer_.Type(GenerateType(type, element_type)) + ", ";
            }
        }
