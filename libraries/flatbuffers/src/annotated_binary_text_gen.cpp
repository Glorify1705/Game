...
namespace {
        ...
        const OutputConfig& output_config)
    {
        ...
        if (region.array_length) {
            if (region.type == BinaryRegionType::Uint8 || region.type == BinaryRegionType::Unknown) {
    ...
    {
        return ToString(region.type) + ((region.array_length) ? "[" + std::to_string(region.array_length) + "]" : "");
    }
        ...
        const BinarySection&)
    {
        ...
        case BinaryRegionCommentType::ArrayField:
            s = "array field `" + comment.name + "`[" + std::to_string(comment.index) + "]";
            break;
        ...
        case BinaryRegionStatus::ERROR_INCOMPLETE_BINARY:
            s = "ERROR: " + s + ". Incomplete binary, expected to read " + comment.status_message + " bytes.";
            break;
        ...
        const OutputConfig& output_config)
    {
        ...
        if (region.array_length) {
            ...
            continuation.value = value.substr(std::min(output_config.max_bytes_per_line, value.size()));
        } else {
            std::stringstream ss;
            ss << std::setw(static_cast<int>(output_config.largest_value_string))
               << std::left;
        ...
        const OutputConfig& output_config)
    {
        ...
        for (uint64_t i = 0; i < region.length; ++i) {
            ...
            if (((i + 1) % output_config.max_bytes_per_line == 0) || i + 1 == region.length) {
        ...
        const OutputConfig& output_config)
    {
        ...
        if (section.type == BinarySectionType::Vector && !output_config.include_vector_contents && section.regions.size() > 4) {
    ...
    const std::string& filename, const std::string& schema_filename)
{
    ...
    // the max output).
    output_config.offset_max_char = binary_length_ > 0xFFFFFF
        ? 8
    ...
    output_filename += "." + (options_.output_extension.empty() ? GetExtension(filename) : options_.output_extension);
