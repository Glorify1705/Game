...
struct JsonPrinter {
        ...
        const Type& type, int indent, const uint8_t*)
    {
        ...
        for (uoffset_t i = 0; i < size; i++) {
            ...
            if (!PrintScalar(c[i], type, elem_indent)) {
                return false;
            }
        }
        AddNewLine();
    ...
    bool GenStruct(const StructDef& struct_def, const Table* table, int indent)
    {
        ...
        for (auto it = struct_def.fields.vec.begin();
            ...
            auto output_anyway = (opts.output_default_scalars_in_json || fd.key) && IsScalar(fd.value.type.base_type) && !fd.deprecated;
            if (is_present || output_anyway) {
                ...
                if (!opts.protobuf_ascii_alike || (fd.value.type.base_type != BASE_TYPE_STRUCT && fd.value.type.base_type != BASE_TYPE_VECTOR))
                    text += ':';
                text += ' ';
                // clang-format off
    ...
    const StructDef& struct_def, std::string* _text)
{
    ...
    if (!printer.GenStruct(struct_def, table, 0)) {
        return false;
    }
    printer.AddNewLine();
    return true;
    ...
    const std::string& file_name)
{
    ...
    if (!parser.builder_.GetSize() || !parser.root_struct_def_)
        return true;
    std::string text;
    ...
    const std::string& file_name)
{
    ...
    auto included_files = parser.GetIncludedFilesRecursive(parser.root_struct_def_->file);
...
namespace {
    ...
    public:
        ...
        {
            if (!GenerateTextFile(parser, path, filename)) {
                return Status::ERROR;
            }
            return Status::OK;
        }
