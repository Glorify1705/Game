...
namespace {
    ...
    public:
        ...
        {
            return r::AdvancedArrayFeatures | r::AdvancedUnionFeatures | r::OptionalScalars | r::DefaultVectorsAndStrings;
        }
    ...
    protected:
        void GenerateEnum(const r::Enum* enum_def)
        {
            ...
            const std::string enum_type = GenerateTypeBasic(enum_def->underlying_type());
            ...
            ForAllEnumValues(enum_def, [&](const reflection::EnumVal* enum_val) {
                ...
                code += "  " + namer_.Variant(enum_val->name()->str()) + " = " + NumToString(enum_val->value()) + "." + enum_type + ",\n";
            });
        ...
        void GenerateObject(const r::Object* object)
        {
            ...
            ForAllFields(object, /*reverse=*/false, [&](const r::Field* field) {
                ...
                const std::string offset_prefix = "let o = self.tab.Offset(" + NumToString(field->offset()) + ")\n";
                const std::string offset_prefix_2 = "if o != 0:\n";

                if (IsScalar(base_type) || base_type == r::String || base_type == r::Obj || base_type == r::Union) {
                    GenerateDocumentation(field->documentation(), "", code);

                    std::string getter_signature = "func " + namer_.Method(field_name) + "*(self: " + object_name + "): " + field_type + " =\n";
                    ...
                    if (base_type == r::Obj || base_type == r::Union || field->type()->index() >= 0) {
                        RegisterImports(object, field);
                    }

                    if (object->is_struct()) {
                        std::string field_getter = GenerateGetter(field->type(), NumToString(field->offset()));
                        getter_code += "  return " + field_getter + "\n";

                        if (IsScalar(base_type)) {
                            setter_code += "  return self.tab.Mutate(self.tab.Pos + " + NumToString(field->offset()) + ", n)\n";
                        }
                    } else {
                        // Table accessors
                        ...
                        if (IsScalar(base_type)) {
                            setter_code += "  return self.tab.MutateSlot(" + NumToString(field->offset()) + ", n)\n";
                        }
                    }
                    ...
                    if (vector_base_type == r::Obj || vector_base_type == r::Union || field->type()->index() >= 0) {
                    ...
                    code += "func " + namer_.Method(field_name) + "*(self: " + object_name + ", j: int): " + GenerateType(field->type(), true) + " = \n";
                    code += "  " + offset_prefix;
                    code += "  " + offset_prefix_2;
                    ...
                    // Get entire vector:
                    code += "func " + namer_.Method(field_name) + "*(self: " + object_name + "): " + GenerateType(field->type()) + " = \n";
            ...
            if (object->is_struct()) {
                code += "proc " + namer_.Function(object_name + "Create") + "*(self: var Builder";
                ...
                ForAllFields(object, /*reverse=*/false, [&](const r::Field* field) {
                    ...
                    code += "proc " + namer_.Function(object_name + "Add" + field_name) + "*(builder: var Builder, " + variable_name + ": " + variable_type + ") =\n";
                    ...
                    if (IsVector(field->type()->base_type())) {
                        code += "proc " + namer_.Function(object_name + "Start" + field_name) + "Vector*(builder: var Builder, numElems: uoffset) =\n";
                        ...
                        code += "  builder.StartVector(" + NumToString(element_size) + ", numElems, " + NumToString(alignment) + ")\n";
                    }
                });
    ...
    private:
        void GenerateDocumentation(
            const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::String>>* documentation,
            ...
            std::string prefix = "") const
        {
            ...
            ForAllFields(object, /*reverse=*/false, [&](const r::Field* field) {
                if (IsStructOrTable(field->type()->base_type())) {
                    ...
                    signature += ", " + prefix + namer_.Variable(*field) + ": " + GenerateType(field->type());
                }
            });
            return signature;
        }
        ...
        {
            std::string code;
            code += "  self.Prep(" + NumToString(object->minalign()) + ", " + NumToString(object->bytesize()) + ")\n";
        ...
        {
            const r::BaseType base_type = element_type ? type->element() : type->base_type();
            ...
            case r::Obj: {
                return GenerateType(type, element_type) + "(tab: Vtable(Bytes: self.tab.Bytes, Pos: " + offset + "))";
            }
            case r::Vector:
            ...
            default:
                ...
                if (type_enum != nullptr) {
                    return GenerateType(type, element_type) + "(" + "Get[" + GenerateType(base_type) + "](self.tab, " + offset + ")" + ")";
                } else {
            ...
            bool use_element = false)
        {
            ...
            const r::BaseType type = use_element ? field->type()->element() : field->type()->base_type();
            ...
            std::string import_path = GetRelativePathFromNamespace(object->name()->str(), type_name);
            ...
            const std::string& ns, const std::string& declaring_file)
        {
            ...
            code += "  Automatically generated by the FlatBuffers compiler, do not "
                    "modify.\n";
            ...
            if (schema_->root_table() != nullptr) {
                ...
                const std::string root_file = schema_->root_table()->declaration_file()->str();
