    ...
    private:
        ...
        void generateEntry()
        {
            ...
            for (const auto& it : ns_defs_) {
                ...
                for (const auto& def : it.second.definitions) {
                    ...
                    if (it.second.ns->components.size() > 1)
                        ...
                        std::copy(it.second.ns->components.begin(),
                            it.second.ns->components.end(),
                            std::back_inserter(rel_components));
                    auto base_file_name = namer_.File(*(def.second), SkipFile::SuffixAndExtension);
                    auto base_name = namer_.Directories(it.second.ns->components, SkipDir::OutputPath) + base_file_name;
                    auto ts_file_path = base_name + ".ts";
                    auto base_name_rel = std::string("./");
                    base_name_rel += namer_.Directories(rel_components, SkipDir::OutputPath);
                    base_name_rel += base_file_name;
                    auto ts_file_path_rel = base_name_rel + ".ts";
                    auto type_name = def.first;
                    auto fully_qualified_type_name = it.second.ns->GetFullyQualifiedName(type_name);
                ...
                for (const auto& it2 : ns_defs_) {
                    if (it2.second.ns->components.size() != child_ns_level)
                        continue;
        ...
        bool generateBundle()
        {
            if (parser_.opts.ts_flat_files) {
                ...
                std::string bundlepath = GeneratedFileName(path_, file_name_, parser_.opts);
        ...
        std::string GenDefaultValue(const FieldDef& field, import_set& imports)
        {
            ...
            if (value.type.enum_def && value.type.base_type != BASE_TYPE_UNION && value.type.base_type != BASE_TYPE_VECTOR) {
                switch (value.type.base_type) {
                case BASE_TYPE_ARRAY: {
                    std::string ret = "[";
                    for (auto i = 0; i < value.type.fixed_length; ++i) {
                        std::string enum_name = AddImport(imports, *value.type.enum_def, *value.type.enum_def)
                                                    .name;
                        std::string enum_value = namer_.Variant(
                            *value.type.enum_def->FindByValue(value.constant));
                        ret += enum_name + "." + enum_value + (i < value.type.fixed_length - 1 ? ", " : "");
                    }
                    ret += "]";
                    return ret;
                }
            ...
            bool allowNull = false)
        {
            ...
            case BASE_TYPE_ARRAY: {
                ...
                if (type.element == BASE_TYPE_LONG || type.element == BASE_TYPE_ULONG) {
                    ...
                    if (parser_.opts.generate_object_based_api) {
                        name = "(any|" + GetTypeName(*type.struct_def, /*object_api =*/true) + ")[]";
                    }
                }
            ...
            default:
                if (IsScalar(type.base_type)) {
                    if (type.enum_def) {
                        const auto enum_name = AddImport(imports, owner, *type.enum_def).name;
            ...
            std::string* arguments, const std::string& nameprefix)
        {
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (IsStruct(field.value.type)) {
                    ...
                    *arguments += ", " + nameprefix + field.name + ": " + GenTypeName(imports, field, field.value.type, true, field.IsOptional());
                }
            }
        }
            ...
            for (auto it = struct_def.fields.vec.rbegin();
                ...
                if (IsStruct(field.value.type)) {
                    ...
                    if (field.value.type.base_type == BASE_TYPE_ARRAY) {
                        ...
                        case BASE_TYPE_STRUCT: {
                            std::string str_last_item_idx = NumToString(field.value.type.fixed_length - 1);
                            ...
                            *body += "    const item = " + fname + "?.[i];\n\n";
                            ...
                            if (parser_.opts.generate_object_based_api) {
                                *body += "    if (item instanceof " + GetTypeName(*field.value.type.struct_def,
                                             /*object_api =*/true)
                                    + ") {\n";
                            ...
                            std::string pack_func_create_call = class_name + ".create" + class_name + "(builder,\n";
                        ...
                        default: {
                            ...
                            *body += element_type == BASE_TYPE_BOOL ? "+" : "";
                            ...
                            if (element_type == BASE_TYPE_LONG || element_type == BASE_TYPE_ULONG) {
                        ...
                        *body += "  builder.write" + GenWriteMethod(field.value.type) + "(";
                        if (field.value.type.base_type == BASE_TYPE_BOOL) {
                            *body += "Number(Boolean(" + fname + ")));\n";
                            continue;
                        } else if (field.value.type.base_type == BASE_TYPE_LONG || field.value.type.base_type == BASE_TYPE_ULONG) {
                            *body += "BigInt(" + fname + " ?? 0));\n";
                            continue;
                        }
            ...
            bool size_prefixed)
        {
            if (!struct_def.fixed) {
                ...
                code += "static get" + (size_prefixed ? sizePrefixed : "") + "Root" + GetPrefixedName(struct_def, "As");
            ...
            std::string& code, bool size_prefixed)
        {
            if (parser_.root_struct_def_ == &struct_def) {
                ...
                code += "static finish" + (size_prefixed ? sizePrefixed : "") + GetPrefixedName(struct_def) + "Buffer";
            ...
            const std::string& object_name)
        {
            ...
            if (has_name_clash) {
                ...
                if (parser_.opts.generate_object_based_api) {
                    symbols_expression += ", " + GetTypeName(struct_def, /*object_api =*/true) + " as " + object_name;
                }
            } else {
            ...
            const DefinitionT& dependency)
        {
            ...
            const std::string object_name = GetTypeName(dependency, /*object_api=*/true, has_name_clash);
            ...
            bare_file_path += kPathSeparator + namer_.Directories(dependency.defined_namespace->components, SkipDir::OutputPath) + namer_.File(dependency, SkipFile::SuffixAndExtension);
            ...
            std::string import_extension = parser_.opts.ts_no_import_ext ? "" : ".js";
            import.import_statement = "import { " + symbols_expression + " } from '" + rel_file_path + import_extension + "';";
            import.export_statement = "export { " + symbols_expression + " } from '." + bare_file_path + import_extension + "';";
                ...
                const auto union_enum_loop = [&](const std::string& accessor_str) {
                    ...
                    for (auto it = enum_def.Vals().begin(); it != enum_def.Vals().end();
                        ...
                        if (IsString(ev.union_type)) {
                            ...
                            ret += "return " + accessor_str + "new " + type + "())! as " + type + ";";
                        } else {
                            FLATBUFFERS_ASSERT(false);
                        }
                        ret += "\n";
            ...
            const bool is_array = false)
        {
            if (union_type.enum_def) {
                ...
                if (!is_array) {
                    ...
                    ret += "    for(let targetEnumIndex = 0; targetEnumIndex < this." + namer_.Method(field_name, "TypeLength") + "()" + "; "
                                                                                                                                         "++targetEnumIndex) {\n";
                    ret += "      const targetEnum = this." + namer_.Method(field_name, "Type") + "(targetEnumIndex);\n";
                    ret += "      if(targetEnum === null || " + enum_type + "[targetEnum!] === 'NONE') { "
                                                                            "continue; }\n\n";
            ...
            import_set& imports)
        {
            ...
            if (has_create) {
                pack_func_create_call = "  return " + struct_name + ".create" + GetPrefixedName(struct_def) + "(builder" + (struct_def.fields.vec.empty() ? "" : ",\n    ");
            } else {
            ...
            if (struct_def.fixed) {
                ...
                // offset are used
                pack_func_create_call += GenStructMemberValueTS(struct_def, "this", ",\n    ", false) + "\n  ";
            }
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                const std::string field_binded_method = "this." + field_method + ".bind(this)";
                ...
                // Emit an object field
                    ...
                    case BASE_TYPE_STRUCT: {
                        ...
                        const std::string field_accessor = "this." + namer_.Method(field) + "()";
                    ...
                    case BASE_TYPE_ARRAY: {
                        ...
                        auto vectortypename = GenTypeName(imports, struct_def, vectortype, false);
                        is_vector = true;
                        ...
                        case BASE_TYPE_STRUCT: {
                            ...
                            field_val = GenBBAccess() + ".createObjList<" + vectortypename + ", " + field_type_name + ">(" + field_binded_method + ", " + NumToString(field.value.type.fixed_length) + ")";

                            if (sd.fixed) {
                                field_offset_decl = "builder.createStructOffsetList(this." + field_field + ", " + AddImport(imports, struct_def, struct_def).name + "." + namer_.Method("start", field, "Vector") + ")";
                            } else {
                                field_offset_decl = AddImport(imports, struct_def, struct_def).name + "." + namer_.Method("create", field, "Vector") + "(builder, builder.createObjectOffsetList(" + "this." + field_field + "))";
                            }
                        ...
                        case BASE_TYPE_STRING: {
                            field_type += "string)[]";
                            field_val = GenBBAccess() + ".createScalarList<string>(" + field_binded_method + ", this." + namer_.Field(field, "Length") + "())";
                            field_offset_decl = AddImport(imports, struct_def, struct_def).name + "." + namer_.Method("create", field, "Vector") + "(builder, builder.createObjectOffsetList(" + "this." + namer_.Field(field) + "))";
                            break;
                        }
                        ...
                        default: {
                            ...
                            field_type += ")[]";
                            field_val = GenBBAccess() + ".createScalarList<" + vectortypename + ">(" + field_binded_method + ", " + NumToString(field.value.type.fixed_length) + ")";

                            field_offset_decl = AddImport(imports, struct_def, struct_def).name + "." + namer_.Method("create", field, "Vector") + "(builder, this." + field_field + ")";
                    ...
                    case BASE_TYPE_VECTOR: {
                        ...
                        case BASE_TYPE_STRUCT: {
                            ...
                            field_val = GenBBAccess() + ".createObjList<" + vectortypename + ", " + field_type_name + ">(" + field_binded_method + ", this." + namer_.Method(field, "Length") + "())";
                        ...
                        default: {
                            ...
                            field_type += ")[]";
                            field_val = GenBBAccess() + ".createScalarList<" + vectortypename + ">(" + field_binded_method + ", this." + namer_.Method(field, "Length") + "())";
                    ...
                    case BASE_TYPE_UNION: {
                        ...
                        field_val = GenUnionValTS(imports, struct_def, field_method,
                            field.value.type);
                        field_offset_decl = "builder.createObjectOffset(this." + field_field + ")";
                        break;
                    }
                ...
                if (!field_offset_decl.empty()) {
                    field_offset_decl = "  const " + field_field + " = " + field_offset_decl + ";";
                }
                ...
                constructor_func += "  public " + field_field + ": " + field_type + " = " + field_default_val;
            ...
            obj_api_class += pack_func_prototype + pack_func_offset_decl + pack_func_create_call + "\n}";
        ...
        static bool CanCreateFactoryMethod(const StructDef& struct_def)
        {
            ...
            // struct
            return struct_def.fields.vec.size() < 2 || std::all_of(std::begin(struct_def.fields.vec) + 1, std::end(struct_def.fields.vec), [](const FieldDef* f) -> bool {
            ...
            std::string* code_ptr, import_set& imports)
        {
            ...
            if (!struct_def.fixed && parser_.root_struct_def_ == &struct_def && !parser_.file_identifier_.empty()) {
                GenDocComment(code_ptr);
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (field.value.type.base_type == BASE_TYPE_ARRAY) {
                    ...
                    offset_prefix = "  const offset = " + GenBBAccess() + ".__offset(this.bb_pos, " + NumToString(field.value.offset) + ");\n";
                ...
                if (IsScalar(field.value.type.base_type) || is_string) {
                    ...
                    if (field.value.type.enum_def) {
                        code += "):" + GenTypeName(imports, struct_def, field.value.type, false, field.IsOptional()) + " {\n";
                    } else {
                        code += "):" + GenTypeName(imports, struct_def, field.value.type, false, has_null_default) + " {\n";
                    }

                    if (struct_def.fixed) {
                        code += "  return " + GenGetter(field.value.type, "(this.bb_pos" + MaybeAdd(field.value.offset) + ")") + ";\n";
                    } else {
                ...
                // Emit an object field
                    ...
                    switch (field.value.type.base_type) {
                    case BASE_TYPE_STRUCT: {
                        const auto type = AddImport(imports, struct_def, *field.value.type.struct_def)
                                              .name;
                        GenDocComment(field.doc_comment, code_ptr);
                    ...
                    case BASE_TYPE_ARRAY: {
                        ...
                        auto index = "this.bb_pos + " + NumToString(field.value.offset) + " + index" + MaybeScale(inline_size);
                        std::string ret_type;
                        bool is_union = false;
                        switch (vectortype.base_type) {
                        case BASE_TYPE_STRUCT:
                        ...
                        prefix += "(index: number";
                        if (is_union) {
                            const auto union_type = GenUnionGenericTypeTS(*(field.value.type.enum_def));
                    ...
                    case BASE_TYPE_VECTOR: {
                        ...
                        auto vectortypename = GenTypeName(imports, struct_def, vectortype, false);
                        auto inline_size = InlineSize(vectortype);
                        auto index = GenBBAccess() + ".__vector(this.bb_pos + offset) + index" + MaybeScale(inline_size);
                        std::string ret_type;
                        bool is_union = false;
                        switch (vectortype.base_type) {
                        case BASE_TYPE_STRUCT:
                        ...
                        if (field.value.type.element == BASE_TYPE_BOOL) {
                            code += "false";
                        } else if (field.value.type.element == BASE_TYPE_LONG || field.value.type.element == BASE_TYPE_ULONG) {
                            code += "BigInt(0)";
                    ...
                    case BASE_TYPE_UNION: {
                        ...
                        code += "<T extends flatbuffers.Table>(obj:" + union_type + "):" + union_type + "|null "
                                                                                                        "{\n";
                ...
                if (IsScalar(field.value.type.base_type) && parser.opts.mutable_buffer && !IsUnion(field.value.type)) {
                    ...
                    const std::string write_method = "." + namer_.Method("write", GenType(field.value.type));

                    if (struct_def.fixed) {
                        code += "  " + GenBBAccess() + write_method + "(this.bb_pos + " + NumToString(field.value.offset) + ", ";
                    } else {
                        code += "  const offset = " + GenBBAccess() + ".__offset(this.bb_pos, " + NumToString(field.value.offset) + ");\n\n";
                        code += "  if (offset === 0) {\n";
                        code += "    return false;\n";
                        code += "  }\n\n";

                        // special case for bools, which are treated as uint8
                        code += "  " + GenBBAccess() + write_method + "(this.bb_pos + offset, ";
                        if (field.value.type.base_type == BASE_TYPE_BOOL) {
                            code += "+";
                        }
                    }
                ...
                if (IsVector(field.value.type)) {
                    ...
                    if (IsScalar(vectorType.base_type) && !IsLong(vectorType.base_type)) {
                        ...
                        code += "new " + GenType(vectorType) + "Array(" + GenBBAccess() + ".bytes().buffer, " + GenBBAccess() + ".bytes().byteOffset + " + GenBBAccess() + ".__vector(this.bb_pos + offset), " + GenBBAccess() + ".__vector_len(this.bb_pos + offset)) : null;\n}\n\n";
                    }
                }
            }
            ...
            if (struct_def.fixed) {
                ...
                code += "static create" + GetPrefixedName(struct_def) + "(builder:flatbuffers.Builder";
                ...
                for (auto it = struct_def.fields.vec.begin();
                    ...
                    code += "(builder:flatbuffers.Builder, " + argname + ":" + GetArgType(imports, struct_def, field, false) + ") {\n";
                    ...
                    if (IsVector(field.value.type)) {
                        ...
                        if (!IsStruct(vector_type)) {
                            ...
                            const std::string sig_begin = "static " + namer_.Method("create", field, "Vector") + "(builder:flatbuffers.Builder, data:";
                            const std::string sig_end = "):flatbuffers.Offset";
                            std::string type = GenTypeName(imports, struct_def, vector_type, true) + "[]";
                            if (type == "number[]") {
                ...
                if (CanCreateFactoryMethod(struct_def)) {
                    ...
                    for (auto it = struct_def.fields.vec.begin();
                        ...
                        if (field.deprecated)
                            continue;
                        code += ", " + GetArgName(field) + ":" + GetArgType(imports, struct_def, field, true);
                    }
            ...
            if (!struct_def.fixed && parser_.services_.vec.size() != 0) {
                ...
                code += "\n";
                code += "static deserialize(buffer: Uint8Array):" + namer_.EscapeKeyword(name) + " {\n";
...
std::string TSMakeRule(const Parser& parser, const std::string& path,
    const std::string& file_name)
{
    std::string filebase = flatbuffers::StripPath(flatbuffers::StripExtension(file_name));
    ts::TsGenerator generator(parser, path, file_name);
    std::string make_rule = generator.GeneratedFileName(path, filebase, parser.opts) + ": ";
