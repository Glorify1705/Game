...
uint64_t BinaryAnnotator::BuildHeader(const uint64_t header_offset)
{
    ...
    if (IsValidRead(offset, flatbuffers::kFileIdentifierLength) && IsPrintableRegion(offset, flatbuffers::kFileIdentifierLength, binary_)) {
        BinaryRegionComment comment;
    ...
    AddSection(header_offset, MakeBinarySection("", BinarySectionType::Header, std::move(regions)));
    ...
    const uint64_t offset_of_referring_table)
{
    ...
    ref_table_len_comment.type = BinaryRegionCommentType::VTableRefferingTableLength;
    ...
    if (!table_length.has_value()) {
        ...
        AddSection(offset, MakeSingleRegionBinarySection(referring_table_name, BinarySectionType::VTable, MakeBinaryRegion(offset, remaining, BinaryRegionType::Unknown, remaining, 0, ref_table_len_comment)));
        return nullptr;
    }
    ...
    ForAllFields(table, /*reverse=*/false, [&](const reflection::Field* field) {
        ...
        field_comment.name = std::string(field->name()->c_str()) + "` (id: " + std::to_string(field->id()) + ")";
        ...
        if (!IsValidOffset(offset_of_referring_table + offset_from_table.value() - 1)) {
        ...
        if (offset_from_table.value() == 0) {
            ...
            if (field->required()) {
                ...
                default_label += reflection::EnumNameBaseType(field->type()->base_type());
                default_label += ")";
            }
        }
    ...
    // as unknown fields.
    uint16_t expectant_vtable_fields = (vtable_size - sizeof(uint16_t) - sizeof(uint16_t)) / sizeof(uint16_t);
    ...
    const reflection::Object* const table)
{
    ...
    // are.
    const VTable* const vtable = GetOrBuildVTable(vtable_offset, table, table_offset);
    ...
    for (size_t i = 0; i < fields.size(); ++i) {
        ...
        if (IsScalar(field->type()->base_type())) {
            ...
            const BinaryRegionType region_type = GetRegionType(field->type()->base_type());

            BinaryRegionComment scalar_field_comment;
            scalar_field_comment.type = BinaryRegionCommentType::TableField;
            scalar_field_comment.name = std::string(field->name()->c_str()) + "` (" + reflection::EnumNameBaseType(field->type()->base_type()) + ")";
        ...
        const std::string offset_prefix = "offset to field `" + std::string(field->name()->c_str()) + "`";
        ...
        case reflection::BaseType::Obj: {
            const reflection::Object* next_object = schema_->objects()->Get(field->type()->index());
        ...
        case reflection::BaseType::Union: {
            ...
            const uint64_t type_offset = table_offset + vtable_field->second.offset_from_table;
            ...
            const std::string enum_type = BuildUnion(union_offset, realized_type.value(), field);

            offset_field_comment.default_value = "(union of type `" + enum_type + "`)";
    ...
    const reflection::Object* const object)
{
    ...
    ForAllFields(object, /*reverse=*/false, [&](const reflection::Field* field) {
        if (IsScalar(field->type()->base_type())) {
            ...
            comment.default_value = "of '" + object->name()->str() + "' (" + std::string(reflection::EnumNameBaseType(field->type()->base_type())) + ")";
            ...
            const BinaryRegionType region_type = GetRegionType(field->type()->element());
            ...
            for (uint16_t i = 0; i < field->type()->fixed_length(); ++i) {
                if (is_scalar) {
                    ...
                    array_comment.name = referring_field_name + "." + field->name()->str();
                    array_comment.index = i;
                    array_comment.default_value = "of '" + object->name()->str() + "' (" + std::string(reflection::EnumNameBaseType(field->type()->element())) + ")";
    ...
    const reflection::Field* const field)
{
    ...
    if (!string_length.has_value()) {
        ...
        const uint64_t string_end = string_offset + sizeof(uint32_t) + string_size + sizeof(char);
        ...
        if (!IsValidOffset(string_end - 1)) {
            ...
            string_terminator_comment.type = BinaryRegionCommentType::StringTerminator;
    ...
    AddSection(string_offset,
        MakeBinarySection(std::string(table->name()->c_str()) + "." + field->name()->c_str(),
    ...
    const std::map<uint16_t, VTable::Entry> vtable_fields)
{
    ...
    // items.
    const uint64_t last_item_offset = vector_offset + sizeof(uint32_t) + vector_length.value() * GetElementSize(field);
    ...
    case reflection::BaseType::Obj: {
        const reflection::Object* object = schema_->objects()->Get(field->type()->index());

        if (object->is_struct()) {
            // Vector of structs
            for (size_t i = 0; i < vector_length.value(); ++i) {
                // Structs are inline to the vector.
                const uint64_t next_offset = BuildStruct(offset, regions, "[" + NumToString(i) + "]", object);
            ...
            for (size_t i = 0; i < vector_length.value(); ++i) {
                ...
                vector_object_comment.type = BinaryRegionCommentType::VectorTableValue;
    ...
    case reflection::BaseType::Union: {
        ...
        const uint64_t union_type_vector_field_offset = parent_table_offset + vtable_entry->second.offset_from_table;

        const auto union_type_vector_field_relative_offset = ReadScalar<uint16_t>(union_type_vector_field_offset);
        ...
        const uint64_t union_type_vector_data_offset = union_type_vector_field_offset + union_type_vector_field_relative_offset.value() + sizeof(uint32_t);
        ...
        for (size_t i = 0; i < vector_length.value(); ++i) {
            ...
            const auto realized_type = ReadScalar<uint8_t>(union_type_vector_data_offset + i);
    ...
    default: {
        if (IsScalar(field->type()->element())) {
            const BinaryRegionType binary_region_type = GetRegionType(field->type()->element());
...
std::string BinaryAnnotator::BuildUnion(const uint64_t union_offset,
    ...
    const reflection::Field* const field)
{
    const reflection::Enum* next_enum = schema_->enums()->Get(field->type()->index());
    ...
    if (union_type->base_type() == reflection::BaseType::Obj) {
        const reflection::Object* object = schema_->objects()->Get(union_type->index());
        ...
        if (object->is_struct()) {
            ...
            AddSection(
                union_offset,
                MakeBinarySection(std::string(object->name()->c_str()) + "." + field->name()->c_str(),
...
void BinaryAnnotator::FixMissingSections()
{
    ...
    for (auto& current_section : sections_) {
        ...
        const uint64_t section_end_offset = section.regions.back().offset + section.regions.back().length;
...
bool BinaryAnnotator::ContainsSection(const uint64_t offset)
{
    ...
    return offset >= it->first && offset < it->second.regions.back().offset + it->second.regions.back().length;
}
