    ...
    const std::string& program_name) const
{
    ...
    std::string files_description = "FILEs may be schemas (must end in .fbs), binary schemas (must end in "
    ...
    const FlatCOptions& options)
{
    ...
    for (const std::string& filename : options.filenames) {
        ...
        const uint8_t* binary = reinterpret_cast<const uint8_t*>(binary_contents.c_str());
        ...
        text_gen_opts.include_vector_contents = options.annotate_include_vector_contents;
    ...
    for (int argi = 1; argi < argc; argi++) {
        ...
        if (arg[0] == '-') {
            ...
            if (arg == "-o") {
                ...
                if (++argi >= argc)
                    Error("missing golang namespace" + arg, true);
                opts.go_namespace = argv[argi];
            } else if (arg == "--go-import") {
                if (++argi >= argc)
                    Error("missing golang import" + arg, true);
                opts.go_import = argv[argi];
            } else if (arg == "--go-module-name") {
                if (++argi >= argc)
                    Error("missing golang module name" + arg, true);
                ...
                if (++argi >= argc)
                    Error("missing include following: " + arg, true);
                opts.cpp_includes.push_back(argv[argi]);
            } else if (arg == "--cpp-ptr-type") {
                if (++argi >= argc)
                    Error("missing type following: " + arg, true);
                ...
                if (++argi >= argc)
                    Error("missing case style following: " + arg, true);
                if (!strcmp(argv[argi], "unchanged"))
                    opts.cpp_object_api_field_case_style = IDLOptions::CaseStyle_Unchanged;
                ...
                if (++argi >= argc)
                    Error("missing prefix following: " + arg, true);
                ...
                if (++argi >= argc)
                    Error("missing suffix following: " + arg, true);
                ...
                if (++argi >= argc)
                    Error("missing namespace suffix" + arg, true);
                ...
                if (++argi >= argc)
                    Error("missing filename suffix: " + arg, true);
                opts.filename_suffix = argv[argi];
            } else if (arg == "--filename-ext") {
                if (++argi >= argc)
                    Error("missing filename extension: " + arg, true);
                ...
                std::shared_ptr<CodeGenerator> code_generator = code_generator_it->second;
...
void FlatCompiler::ValidateOptions(const FlatCOptions& options)
{
    ...
    if (!options.filenames.size())
        Error("missing input files", false, true);

    if (opts.proto_mode) {
        if (options.any_generator)
            Error("cannot generate code directly from .proto files", true);
    } else if (!options.any_generator && options.conform_to_schema.empty() && options.annotate_schema.empty()) {
    ...
    const FlatCOptions& options)
{
    ...
    if (!options.conform_to_schema.empty()) {
        ...
        if (flatbuffers::GetExtension(options.conform_to_schema) == reflection::SchemaExtension()) {
...
{
    std::unique_ptr<Parser> parser = std::unique_ptr<Parser>(new Parser(options.opts));
    ...
    for (auto file_it = options.filenames.begin();
        ...
        bool is_binary = static_cast<size_t>(file_it - options.filenames.begin()) >= options.binary_files_from;
        ...
        if (is_binary) {
            ...
            if (!options.raw_binary) {
                ...
                if (!parser->file_identifier_.length()) {
                    Error("current schema has no file_identifier: cannot test if \"" + filename + "\" matches the schema, use --raw-binary to read this file"
                                                                                                  " anyway.");
                } else if (!flatbuffers::BufferHasIdentifier(
                               contents.c_str(), parser->file_identifier_.c_str(),
                               opts.size_prefixed)) {
                    Error("binary \"" + filename + "\" does not have expected file_identifier \"" + parser->file_identifier_ + "\", use --raw-binary to read this file anyway.");
                }
            }
        } else {
            // Check if file contains 0 bytes.
            if (!opts.use_flexbuffers && !is_binary_schema && contents.length() != strlen(contents.c_str())) {
            ...
            if (is_binary_schema) {
                ...
                if (!is_schema && !parser->builder_.GetSize()) {
                    ...
                    Error("input file is neither json nor a .fbs (schema) file: " + filename,
                        true);
                }
            }
            if ((is_schema || is_binary_schema) && !options.conform_to_schema.empty()) {
                auto err = parser->ConformTo(conform_parser);
                if (!err.empty())
                    Error("schemas don\'t conform: " + err, false);
            }
        ...
        std::string filebase = flatbuffers::StripPath(flatbuffers::StripExtension(filename));
        ...
        for (const std::shared_ptr<CodeGenerator>& code_generator :
            ...
            if (options.print_make_rules) {
                ...
                if (status == CodeGenerator::Status::OK && !make_rule.empty()) {
                    ...
                    Error("Cannot generate make rule for " + code_generator->LanguageName());
                }
            } else {
                ...
                if (code_generator->SupportsBfbsGeneration()) {
                    const CodeGenerator::Status status = code_generator->GenerateCode(bfbs_buffer, bfbs_length);
                    if (status != CodeGenerator::Status::OK) {
                        Error("Unable to generate " + code_generator->LanguageName() + " for " + filebase + " using bfbs generator.");
                    }
                } else {
                    if ((!code_generator->IsSchemaOnly() || (is_schema || is_binary_schema)) && code_generator->GenerateCode(*parser, options.output_path, filebase) != CodeGenerator::Status::OK) {
                        Error("Unable to generate " + code_generator->LanguageName() + " for " + filebase);
                    }
                }
            }
            ...
            if (options.grpc_enabled) {
                ...
                if (status == CodeGenerator::Status::NOT_IMPLEMENTED) {
                    Warn("GRPC interface generator not implemented for " + code_generator->LanguageName());
                } else if (status == CodeGenerator::Status::ERROR) {
                    Error("Unable to generate GRPC interface for " + code_generator->LanguageName());
                }
            }
        }
...
int FlatCompiler::Compile(const FlatCOptions& options)
{
    ...
    if (!options.annotate_schema.empty()) {
        ...
        if (!(ext == reflection::SchemaExtension() || ext == "fbs")) {
            Error("Expected a `.bfbs` or `.fbs` schema, got: " + options.annotate_schema);
        }
        ...
        if (binary_schema == nullptr || !binary_schema_size) {
            Error("could not parse a value binary schema from: " + options.annotate_schema);
        }
...
{
    if (!option.short_opt.empty() && code_generators_.find("-" + option.short_opt) != code_generators_.end()) {
    ...
    if (!option.long_opt.empty() && code_generators_.find("--" + option.long_opt) != code_generators_.end()) {
