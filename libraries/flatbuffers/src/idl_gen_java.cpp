...
namespace java {
    ...
    public:
            ...
            const std::string& classcode, bool needs_includes) const
        {
            ...
            const std::string filename = dirs + namer_.File(defname, /*skips=*/SkipFile::Suffix);
        ...
        {
            return parser_.opts.gen_nullable && !IsScalar(DestinationType(t, true).base_type) && t.base_type != BASE_TYPE_VECTOR
                ? " @Nullable "
                : "";
        }
        ...
        {
            return parser_.opts.java_checkerframework && !IsScalar(DestinationType(t, true).base_type)
                ? " @Pure "
                : "";
        }
    ...
    static const char * const java_typename[] = {
      ...
      #undef FLATBUFFERS_TD
        ...
        std::string GenEnumDefaultValue(const FieldDef& field) const
        {
            ...
            return enum_val ? Prefixed(namer_.NamespacedEnumVariant(enum_def, *enum_val))
                            : value.constant;
        }
        ...
        void GenEnum(EnumDef& enum_def, std::string& code) const
        {
            ...
            for (auto it = enum_def.Vals().begin(); it != enum_def.Vals().end(); ++it) {
                ...
                if (enum_def.underlying_type.base_type == BASE_TYPE_LONG || enum_def.underlying_type.base_type == BASE_TYPE_ULONG) {
                    code += "L";
                }
                code += ";\n";
            }
            ...
            if (range / static_cast<uint64_t>(enum_def.size()) < kMaxSparseness && GenTypeBasic(DestinationType(enum_def.underlying_type, false)) != "long") {
            ...
            const char* num = nullptr) const
        {
            ...
            getter = dest_cast + getter + "(" + GenOffsetGetter(key_field, num) + ")" + dest_mask;
            return getter;
        }
            ...
            const IDLOptions& opts) const
        {
            ...
            if (!struct_def.fixed) {
                ...
                // of a FlatBuffer
                const std::string method_name = namer_.LegacyJavaMethod2("getRootAs", struct_def, "");
                const std::string method_signature = "  public static " + struct_class + " " + method_name;
                ...
                code += method_signature + "(ByteBuffer _bb, " + struct_class + " obj) { ";
                ...
                if (parser_.root_struct_def_ == &struct_def) {
                    if (parser_.file_identifier_.length()) {
                        ...
                        code += "  public static ";
                        code += "boolean " + namer_.LegacyJavaMethod2("", struct_def, "BufferHasIdentifier(ByteBuffer _bb)") + " { return ";
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                const std::string method_start = "  public " + (field.IsRequired() ? "" : GenNullableAnnotation(field.value.type)) + GenPureAnnotation(field.value.type) + type_name_dest + " " + namer_.Field(field);
                ...
                auto offset_prefix = IsArray(field.value.type)
                    ? " { return "
                    : (" { int o = __offset(" + NumToString(field.value.offset) + "); return o != 0 ? ");
                ...
                if (field.value.type.base_type == BASE_TYPE_STRUCT) {
                    ...
                    code += "(new ";
                    code += type_name + "()); }\n";
                } else if (IsSeries(field.value.type) && field.value.type.element == BASE_TYPE_STRUCT) {
                ...
                if (IsVector(field.value.type)) {
                    ...
                    if (IsScalar(element_base_type)) {
                        vector_type_name = ConvertCase(type_name, Case::kUpperCamel) + "Vector";
                    ...
                    auto vector_method_start = GenNullableAnnotation(field.value.type) + "  public " + vector_type_name + " " + namer_.Field(field, "vector");
                ...
                if (field.nested_flatbuffer) {
                    auto nested_type_name = Prefixed(namer_.NamespacedType(*field.nested_flatbuffer));
                    auto nested_method_name = namer_.Field(field) + "As" + field.nested_flatbuffer->name;
                ...
                if (parser_.opts.mutable_buffer) {
                    ...
                    // mutate.
                    auto mutator_params = (is_series ? "(int j, " : "(") + GenTypeNameDest(underlying_type) + " " + field.name + ") { ";
                ...
                if (parser_.opts.java_primitive_has_method && IsScalar(field.value.type.base_type) && !struct_def.fixed) {
                    auto vt_offset_constant = "  public static final int VT_" + namer_.Constant(field) + " = " + NumToString(field.value.offset) + ";";
                ...
                for (auto it = struct_def.fields.vec.begin();
                    ...
                    if (IsVector(field.value.type)) {
                        ...
                        if (!IsStruct(vector_type)) {
                            ...
                            if ((vector_type.base_type == BASE_TYPE_CHAR || vector_type.base_type == BASE_TYPE_UCHAR)) {
        ...
        std::string GenOptionalScalarCheck(FieldDef& field) const
        {
            if (!field.IsScalarOptional())
                return "";
            return "  public boolean " + namer_.Method("has", field) + "() { return 0 != __offset(" + NumToString(field.value.offset) + "); }\n";
        }
        ...
        void GenEnum_ObjectAPI(EnumDef& enum_def, std::string& code) const
        {
            ...
            for (auto it = enum_def.Vals().begin(); it != enum_def.Vals().end(); ++it) {
                ...
                if (ev.union_type.base_type == BASE_TYPE_STRUCT && ev.union_type.struct_def->attributes.Lookup("private")) {
                    code += "  ";
                } else {
                    code += "  public ";
                }
            ...
            code += "\n";
            // pack()
            code += "  public static int pack(FlatBufferBuilder builder, " + union_name + " _o) {\n";
            code += "    switch (_o.type) {\n";
            for (auto it = enum_def.Vals().begin(); it != enum_def.Vals().end(); ++it) {
                auto& ev = **it;
                if (ev.union_type.base_type == BASE_TYPE_NONE) {
        ...
        {
            const std::string variable_type = is_vector ? type_name.substr(0, type_name.length() - 2) : type_name;
            const std::string variable_name = "_" + namer_.Variable("o", field_name) + (is_vector ? "Element" : "");
            ...
            for (auto eit = enum_def.Vals().begin(); eit != enum_def.Vals().end();
                ...
                if (ev.union_type.base_type == BASE_TYPE_NONE) {
                    ...
                    code += indent + "  case " + Prefixed(namer_.NamespacedEnumVariant(enum_def, ev)) + ":\n";
                    auto actual_type = GenTypeGet(ev.union_type);
                    code += indent + "    " + variable_name + "Value = " + field_name + "(new " + actual_type + "()" + value_params + ");\n";
            ...
            code += indent + "}\n";
            if (is_vector) {
                code += indent + "_" + namer_.Variable("o", field_name) + "[_j] = " + variable_name + ";\n";
            }
        }
            ...
            const std::set<FieldDef*>& field_has_create) const
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                case BASE_TYPE_ARRAY: {
                    ...
                    auto unpack_method = field.value.type.struct_def == nullptr ? "" : ".unpack()";
                ...
                case BASE_TYPE_VECTOR:
                    if (field.value.type.element == BASE_TYPE_UNION) {
                        code += start + "new " + GenConcreteTypeGet_ObjectAPI(field.value.type).substr(0, type_name.length() - 1) + accessor + "Length()];\n";
                        ...
                        code += start + "new " + GenConcreteTypeGet_ObjectAPI(field.value.type).substr(0, type_name.length() - 1) + length_accessor + "()];\n";
                        code += "    for (int _j = 0; _j < " + length_accessor + "(); ++_j) {";
                        code += variable + "[_j] = ";
                        if (fixed) {
                            code += accessor + "(_j)";
                        } else {
                            code += "(" + accessor + "(_j) != null ? " + accessor + "(_j).unpack() : null)";
                        }
                        code += ";}\n";
                    }
                    break;
                ...
                default: {
                    if (field.IsScalarOptional()) {
                        code += start + namer_.Method("has", field) + "() ? " + accessor + "() : null;\n";
                    } else {
                        code += start + accessor + "();\n";
                    }
                    break;
            ...
            code += "  }\n";
            // pack()
            code += "  public static " + GenOffsetType() + " pack(FlatBufferBuilder builder, " + struct_name + " _o) {\n";
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                case BASE_TYPE_STRUCT: {
                    if (!field.value.type.struct_def->fixed) {
                        code += "    " + GenOffsetType() + " _" + namer_.Variable(field) + " = _o." + get_field + "() == null ? 0 : " + GenTypeGet(field.value.type) + ".pack(builder, _o." + get_field + "());\n";
                ...
                case BASE_TYPE_STRING: {
                    code += "    int _" + field_name + " = _o." + get_field + "() == null ? 0 : "
                ...
                case BASE_TYPE_VECTOR: {
                    if (field_has_create.find(&field) != field_has_create.end()) {
                        ...
                        case BASE_TYPE_STRUCT:
                            array_type = "int";
                            element_type = GenTypeGet_ObjectAPI(field.value.type, true, true);
                            ;
                        ...
                        case BASE_TYPE_UTYPE:
                            ...
                            to_array = "_o." + namer_.Method("get", property_name) + "()[_j].getType()";
                            break;
                        case BASE_TYPE_UNION:
                            array_type = "int";
                            element_type = Prefixed(namer_.NamespacedType(*field.value.type.enum_def))
                                + "Union";
                            to_array = element_type + ".pack(builder,  _o." + namer_.Method("get", property_name) + "()[_j])";
                            break;
                        ...
                        if (gen_for_loop) {
                            code += "      " + array_type + "[] " + array_name + " = new " + array_type + "[_o." + namer_.Method("get", property_name) + "().length];\n";
                            code += "      int _j = 0;\n";
                            code += "      for (" + element_type + " _e : _o." + namer_.Method("get", property_name) + "()) { ";
                        ...
                        auto element_type_name = GenTypeGet_ObjectAPI(field.value.type, true, true);
                        auto pack_method = field.value.type.struct_def == nullptr
                            ? "builder.add" + GenMethod(field.value.type.VectorType()) + "(" + variable + "[_j]);"
                            : "_unused_offset = " + type_name + ".pack(builder, " + variable + "[_j]);";
                        code += "    int _" + field_name + " = 0;\n";
                        ...
                        code += "      for (int _j = " + variable + ".length - 1; _j >=0; _j--) { ";
                        code += pack_method + "}\n";
                ...
                case BASE_TYPE_ARRAY: {
                    if (field.value.type.struct_def != nullptr) {
                        ...
                        code += "    " + GenTypeGet_ObjectAPI(field.value.type, false, true) + " _" + field_name + " = _o." + get_field + "();\n";
                    }
                    break;
                }
                case BASE_TYPE_UNION: {
                    code += "    " + GenTypeBasic(DestinationType(field.value.type.enum_def->underlying_type, false)) + " _" + field_name + "Type = _o." + get_field + "() == null ? " + Prefixed(namer_.NamespacedType(*field.value.type.enum_def)) + ".NONE : " + "_o." + get_field + "().getType();\n";
            ...
            if (struct_has_create) {
                // Create
                code += "    return " + namer_.LegacyJavaMethod2("create", struct_def, "") + "(\n";
                code += "      builder";
                for (auto it = struct_def.fields.vec.begin();
                     it != struct_def.fields.vec.end(); ++it) {
                    auto& field = **it;
                        ...
                        if (struct_def.fixed) {
                            ...
                            if (field.value.type.struct_def->fixed) {
                                if (opts.generate_object_based_api)
                                    ...
                                    code += "      " + GenTypeGet(field.value.type) + ".Pack(builder, _o." + field_name + ")";
                            } else {
                                code += "      _" + field_name;
                            }
                        }
                ...
                // Start, End
                code += "    " + namer_.LegacyJavaMethod2("start", struct_def, "") + "(builder);\n";
                for (auto it = struct_def.fields.vec.begin();
                     it != struct_def.fields.vec.end(); ++it) {
                    auto& field = **it;
                    if (field.deprecated)
                    ...
                    case BASE_TYPE_STRUCT: {
                        if (field.value.type.struct_def->fixed) {
                            code += "    " + add_field + "(builder, " + GenTypeGet(field.value.type) + ".pack(builder, _o." + get_field + "()));\n";
                        } else {
                    ...
                    default: {
                        if (field.IsScalarOptional()) {
                            code += "    if (_o." + get_field + "() != null) { " + add_field + "(builder, _o." + get_field + "()); }\n";
                        } else {
                            code += "    " + add_field + "(builder, _o." + get_field + "());\n";
                        }
                        break;
                    }
                    }
                }
                code += "    return " + namer_.LegacyJavaMethod2("end", struct_def, "") + "(builder);\n";
            }
            code += "  }\n";
        }
            ...
            std::string& code) const
        {
            for (auto it = struct_def.fields.vec.begin();
                ...
                const Type& field_type = is_array ? field.value.type.VectorType() : field.value.type;
                FieldArrayLength tmp_array_length = {
                    field.name,
                ...
                if (field_type.struct_def != nullptr) {
                    ...
                    if (array_only_lengths.size() > 0) {
                        ...
                        for (size_t i = 0; i < array_only_lengths.size(); ++i) {
                            ...
                            code += "for (int " + idx + " = 0; " + idx + " < " + NumToString(array_only_lengths[i].length) + "; ++" + idx + ") {";
                        }
                        for (size_t i = 0; i < array_only_lengths.size(); ++i) {
                            auto idx = "idx" + NumToString(i);
                            if (i == 0) {
            ...
            std::string& code) const
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                code += "  public " + type_name + " " + get_field + "() { return " + field_name + "; }\n\n";
                std::string array_validation = "";
                if (field.value.type.base_type == BASE_TYPE_ARRAY) {
                    array_validation = "if (" + field_name + " != null && " + field_name + ".length == " + NumToString(field.value.type.fixed_length) + ") ";
                }
