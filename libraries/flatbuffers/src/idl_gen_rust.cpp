    ...
    const std::string& output_dir)
{
    ...
    const bool success = SaveFile((output_dir + "mod.rs").c_str(), code.ToString(), false);
    code.Clear();
    return success;
}
    ...
    public:
            ...
            std::function<void(const T&)> gen_symbol)
        {
            for (auto it = symbols.vec.begin(); it != symbols.vec.end(); it++) {
                ...
                const std::string directories = namer_.Directories(*symbol.defined_namespace);
                EnsureDirExists(directories);
                const std::string file_path = directories + namer_.File(symbol);
                const bool save_success = SaveFile(file_path.c_str(), code_.ToString(), /*binary=*/false);
                if (!save_success)
                    return false;
            }
            return true;
        ...
        bool GenerateOneFile()
        {
            ...
            for (auto ns_it = parser_.namespaces_.begin();
                ...
                for (auto it = parser_.structs_.vec.begin();
                    ...
                    if (struct_def.defined_namespace == ns && struct_def.fixed && !struct_def.generated) {
    ...
    #undef FLATBUFFERS_TD
        ...
        std::string GetTypeGet(const Type& type) const
        {
            ...
            case ftArrayOfStruct: {
                return "[" + GetTypeGet(type.VectorType()) + "; " + NumToString(type.fixed_length) + "]";
            }
            case ftTable: {
        ...
        {
            const bool is_private = parser_.opts.no_leak_private_annotations && (enum_def.attributes.Lookup("private") != nullptr);
            ...
            if (IsBitFlagsEnum(enum_def)) {
                ...
                const std::string deprecation_warning = "#[deprecated(since = \"2.0.0\", note = \"Use associated constants"
                ...
                code_ += "pub const ENUM_VALUES_{{ENUM_CONSTANT}}: [{{ENUM_TY}}; " + num_fields + "] = [";
                ForAllEnumValues1(enum_def, [&](const EnumVal& ev) {
                ...
                code_ += "  fn fmt(&self, f: &mut core::fmt::Formatter) ->"
                         " core::fmt::Result {";
            ...
            if (parser_.opts.rust_serialize) {
                ...
                if (IsBitFlagsEnum(enum_def)) {
                    ...
                    code_ += "    serializer.serialize_unit_variant(\"{{ENUM_TY}}\", self.0 "
                             "as "
            ...
            const DefaultContext context)
        {
            ...
            case ftFloat: {
                const std::string float_prefix = (field.value.type.base_type == BASE_TYPE_FLOAT) ? "f32::" : "f64::";
            ...
            case ftString: {
                ...
                // TODO: Escape strings?
                const std::string defval = field.IsRequired() ? "\"\"" : "\"" + field.value.constant + "\"";
                if (context == kObject)
            ...
            const std::string& lifetime)
        {
            ...
            auto WrapVector = [&](std::string ty) {
                return WrapOption("flatbuffers::WIPOffset<flatbuffers::Vector<" + lifetime + ", " + ty + ">>");
            };
        ...
        std::string ObjectFieldType(const FieldDef& field, bool in_a_table)
        {
            ...
            case ftArrayOfEnum: {
                ty = "[" + WrapInNameSpace(*type.VectorType().enum_def) + "; " + NumToString(type.fixed_length) + "]";
                break;
            }
            case ftArrayOfStruct: {
                ty = "[" + NamespacedNativeName(*type.VectorType().struct_def) + "; " + NumToString(type.fixed_length) + "]";
                break;
            }
            case ftArrayOfBuiltin: {
                ty = "[" + GetTypeBasic(type.VectorType()) + "; " + NumToString(type.fixed_length) + "]";
                break;
            }
            }
            ...
            const std::string& lifetime)
        {
            ...
            case ftVectorOfStruct: {
                const auto typname = WrapInNameSpace(*type.struct_def);
                return "flatbuffers::WIPOffset<flatbuffers::Vector<" + lifetime + ", " + typname + ">>";
            }
            case ftVectorOfTable: {
                const auto typname = WrapInNameSpace(*type.struct_def);
                return "flatbuffers::WIPOffset<flatbuffers::Vector<" + lifetime + ", flatbuffers::ForwardsUOffset<" + typname + "<" + lifetime + ">>>>";
            }
            case ftVectorOfInteger:
            case ftVectorOfBool:
            case ftVectorOfFloat: {
            ...
            case ftVectorOfString: {
                return "flatbuffers::WIPOffset<flatbuffers::Vector<" + lifetime + ", flatbuffers::ForwardsUOffset<&" + lifetime + " str>>>";
            }
            case ftVectorOfEnumKey: {
                const auto typname = WrapInNameSpace(*type.enum_def);
                return "flatbuffers::WIPOffset<flatbuffers::Vector<" + lifetime + ", " + typname + ">>";
            ...
            case ftVectorOfUnionValue: {
                return "flatbuffers::WIPOffset<flatbuffers::Vector<" + lifetime + ", flatbuffers::ForwardsUOffset<flatbuffers::Table<" + lifetime + ">>>";
            }
            case ftEnumKey:
            case ftUnionKey: {
                const auto typname = WrapInNameSpace(*type.enum_def);
            ...
            case ftArrayOfBuiltin: {
                const auto typname = GetTypeBasic(type.VectorType());
                return "flatbuffers::Array<" + lifetime + ", " + typname + ", " + NumToString(type.fixed_length) + ">";
            }
            case ftArrayOfEnum: {
                const auto typname = WrapInNameSpace(*type.enum_def);
                return "flatbuffers::Array<" + lifetime + ", " + typname + ", " + NumToString(type.fixed_length) + ">";
            ...
            const std::string& lifetime)
        {
            ...
            case ftVectorOfTable: {
                const auto typname = WrapInNameSpace(*type.struct_def);
                return WrapOption("flatbuffers::Vector<" + lifetime + ", flatbuffers::ForwardsUOffset<" + typname + "<" + lifetime + ">>>");
            }
            case ftVectorOfString: {
                return WrapOption("flatbuffers::Vector<" + lifetime + ", flatbuffers::ForwardsUOffset<&" + lifetime + " str>>");
            }
            case ftVectorOfUnionValue: {
                FLATBUFFERS_ASSERT(false && "vectors of unions are not yet supported");
            ...
            const std::string& lifetime)
        {
            ...
            return "unsafe { self._tab.get::<" + typname + ">({{STRUCT_TY}}::" + vt_offset + ", " + default_value + ")" + unwrap + "}";
        }
        ...
        {
            const std::string fully_qualified_name = struct_def.defined_namespace->GetFullyQualifiedName(name);
        ...
        void GenTable(const StructDef& struct_def)
        {
            ...
            const bool is_private = parser_.opts.no_leak_private_annotations && (struct_def.attributes.Lookup("private") != nullptr);
            code_.SetValue("ACCESS_TYPE", is_private ? "pub(crate)" : "pub");
            ...
            code_ += "  #[inline]";
            code_ += "  pub unsafe fn init_from_table(table: flatbuffers::Table<'a>) -> "
                     "Self {";
            ...
            ForAllTableFields(struct_def, [&](const FieldDef& field) {
                ...
                if (nested) {
                    ...
                    if (field.IsRequired()) {
                        ...
                        code_ += "  unsafe { <flatbuffers::ForwardsUOffset<{{NESTED}}<'a>>>"
                                 "::follow(data.bytes(), 0) }";
                    } else {
            ...
            ForAllTableFields(struct_def, [&](const FieldDef& field) {
                ...
                ForAllUnionVariantsBesidesNone(
                    *field.value.type.enum_def, [&](const EnumVal& unused) {
                        ...
                        code_ += "  if self.{{DISCRIMINANT}}() == {{U_ELEMENT_ENUM_TYPE}} {";
            ...
            if (parser_.opts.rust_serialize) {
                ...
                code_ += "  {";
                if (numFields == 0) {
                    code_ += "    let s = serializer.serialize_struct(\"{{STRUCT_TY}}\", 0)?;";
                } else {
                    code_ += "    let mut s = serializer.serialize_struct(\"{{STRUCT_TY}}\", "
                             "{{NUM_FIELDS}})?;";
                }
                ForAllTableFields(struct_def, [&](const FieldDef& field) {
                    const Type& type = field.value.type;
            ...
            code_ += "  #[inline]";
            code_ += "  pub fn new(_fbb: &'b mut flatbuffers::FlatBufferBuilder<'a>) -> "
            ...
            code_ += "  fn fmt(&self, f: &mut core::fmt::Formatter<'_>"
                     ") -> core::fmt::Result {";
        ...
        {
            *code_ptr += "  padding" + NumToString((*id)++) + "__: u" + NumToString(bits) + ",";
        }
            ...
            ForAllStructFields(struct_def, [&](const FieldDef& field) {
                ...
                if (IsStruct(field.value.type)) {
                    ...
                    if (GetFullType(field.value.type) == ftArrayOfBuiltin) {
                        ...
                        code_ += "  // Created from a valid Table for this object";
                        code_ += "  // Which contains a valid array in this slot";
                        code_ += "  unsafe { flatbuffers::emplace_scalar_array(&mut self.0, "
                                 "{{FIELD_OFFSET}}, items) };";
                    } else {
            ...
            if (!parser_.opts.generate_all) {
                for (auto it = parser_.included_files_.begin();
                    ...
                    if (parser_.opts.include_prefix.empty()) {
                        ...
                        code_ += indent + "use crate::" + prefix + "::" + basename + parser_.opts.filename_suffix + "::*;";
                    }
                }
            }
