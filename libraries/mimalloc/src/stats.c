...
static void mi_stat_count_add_mt(mi_stat_count_t* stat,
                                 const mi_stat_count_t* src) {
  if (stat == src) return;
  mi_atomic_void_addi64_relaxed(&stat->total, &src->total);
  const int64_t prev_current =
...
static void mi_stat_counter_add_mt(mi_stat_counter_t* stat,
...
static void mi_printf_amount(int64_t n, int64_t unit, mi_output_fun* out,
  ...
  if (pos < base) {
    ...
    char unitdesc[8];
    _mi_snprintf(unitdesc, 8, "%s%s%s", magnitude, (base == 1024 ? "i" : ""),
                 suffix);
    _mi_snprintf(buf, len, "%ld.%ld %-3s", whole, (frac1 < 0 ? -frac1 : frac1),
                 unitdesc);
  }
...
static void mi_stat_print_ex(const mi_stat_count_t* stat, const char* msg,
...
#if MI_STAT > 1
static void mi_stat_total_print(const mi_stat_count_t* stat, const char* msg,
                                int64_t unit, mi_output_fun* out, void* arg) {
  _mi_fprintf(out, arg, "  %-10s:", msg);
...
static void mi_stat_average_print(size_t count, size_t total, const char* msg,
...
static void mi_print_header(const char* name, mi_output_fun* out, void* arg) {
  _mi_fprintf(out, arg, " %-11s %11s %11s %11s %11s %11s\n", name, "peak   ",
              "total   ", "current   ", "block   ", "total#   ");
}

#if MI_STAT > 1
static bool mi_stats_print_bins(const mi_stat_count_t* bins, size_t max,
                                mi_output_fun* out, void* arg) {
  bool found = false;
  char buf[64];
  for (size_t i = 0; i <= max; i++) {
    if (bins[i].total > 0) {
      ...
      const char* pagekind =
          (unit <= MI_SMALL_MAX_OBJ_SIZE
               ? "S"
               : (unit <= MI_MEDIUM_MAX_OBJ_SIZE
                      ? "M"
...
mi_decl_export void mi_process_info_print_out(mi_output_fun* out,
  ...
  size_t peak_commit;
  size_t page_faults;
  mi_process_info(&elapsed, &user_time, &sys_time, &current_rss, &peak_rss,
                  &current_commit, &peak_commit, &page_faults);
  _mi_fprintf(out, arg, "  %-10s: %5zu.%03zu s\n", "elapsed", elapsed / 1000,
              elapsed % 1000);
  _mi_fprintf(out, arg,
              "  %-10s: user: %zu.%03zu s, system: %zu.%03zu s, faults: %zu, "
              "peak rss: ",
              "process", user_time / 1000, user_time % 1000, sys_time / 1000,
              sys_time % 1000, page_faults);
...
void _mi_stats_print(const char* name, size_t id, mi_stats_t* stats,
...
#endif
#if MI_STAT
    mi_stat_print(&stats->malloc_normal, "binned",
                  (stats->malloc_normal_count.total == 0 ? 1 : -1), out, arg);
    mi_stat_print(&stats->malloc_huge, "huge",
                  (stats->malloc_huge_count.total == 0 ? 1 : -1), out, arg);
  ...
  if (stats->pages.total != 0) {
    ...
    mi_stat_average_print(stats->page_searches_count.total,
                          stats->page_searches.total, "searches", out, arg);
    _mi_fprintf(out, arg, "\n");
  }
...
static mi_stats_t* mi_stats_merge_theap_to_heap(mi_theap_t* theap)
    mi_attr_noexcept {
...
static mi_stats_t* mi_heap_get_stats(mi_heap_t* heap) {
  ...
  if (theap == NULL)
    ...
    return mi_stats_merge_theap_to_heap(theap);
}
...
void mi_heap_stats_print_out(mi_heap_t* heap, mi_output_fun* out,
                             void* arg) mi_attr_noexcept {
  if (heap == NULL) {
    heap = mi_heap_main();
  }
...
mi_msecs_t _mi_clock_now(void) { return _mi_prim_clock_now(); }
...
mi_decl_export void mi_process_info(size_t* elapsed_msecs, size_t* user_msecs,
  ...
  pinfo.current_commit = (size_t)(mi_atomic_loadi64_relaxed(
      (_Atomic(int64_t)*)(&subproc->stats.committed.current)));
  pinfo.peak_commit = (size_t)(mi_atomic_loadi64_relaxed(
      (_Atomic(int64_t)*)(&subproc->stats.committed.peak)));
    ...
    *elapsed_msecs =
        (pinfo.elapsed < 0
             ...
             : (pinfo.elapsed < (mi_msecs_t)PTRDIFF_MAX ? (size_t)pinfo.elapsed
                                                        : PTRDIFF_MAX));
  if (user_msecs != NULL)
    *user_msecs = (pinfo.utime < 0 ? 0
                                   : (pinfo.utime < (mi_msecs_t)PTRDIFF_MAX
                                          ? (size_t)pinfo.utime
                                          : PTRDIFF_MAX));
  if (system_msecs != NULL)
    *system_msecs = (pinfo.stime < 0 ? 0
                                     : (pinfo.stime < (mi_msecs_t)PTRDIFF_MAX
                                            ? (size_t)pinfo.stime
                                            : PTRDIFF_MAX));
...
static bool _mi_stats_get(mi_stats_t* stats_in,
                          mi_stats_t* stats_out) mi_attr_noexcept {
  if (stats_out == NULL || stats_out->size != sizeof(mi_stats_t) ||
...
bool mi_subproc_stats_get_exclusive(mi_subproc_id_t subproc_id,
                                    mi_stats_t* stats) mi_attr_noexcept {
...
bool mi_subproc_stats_get(mi_subproc_id_t subproc_id,
                          mi_stats_t* stats) mi_attr_noexcept {
  mi_subproc_t* subproc = _mi_subproc_from_id(subproc_id);
  if (stats == NULL || stats->size != sizeof(mi_stats_t) ||
...
static bool mi_json_buf_expand(mi_json_buf_t* hbuf) {
  ...
  const size_t newsize =
      (hbuf->size == 0 ? mi_good_size(12 * MI_KiB) : 2 * hbuf->size);
...
static void mi_json_buf_print_count_bin(mi_json_buf_t* hbuf, const char* prefix,
                                        mi_stat_count_t* stat, size_t bin,
                                        bool add_comma) {
  const size_t binsize = mi_stats_get_bin_size(bin);
  const size_t pagesize =
      (binsize <= MI_SMALL_MAX_OBJ_SIZE
           ? MI_SMALL_PAGE_SIZE
           : (binsize <= MI_MEDIUM_MAX_OBJ_SIZE
                  ? MI_MEDIUM_PAGE_SIZE
                  : (binsize <= MI_LARGE_MAX_OBJ_SIZE ? MI_LARGE_PAGE_SIZE
                                                      : 0)));
  char buf[128];
  _mi_snprintf(buf, 128,
               "%s{ \"total\": %lld, \"peak\": %lld, \"current\": %lld, "
               "\"block_size\": %zu, \"page_size\": %zu }%s\n",
               prefix, stat->total, stat->peak, stat->current, binsize,
               pagesize, (add_comma ? "," : ""));
  buf[127] = 0;
  mi_json_buf_print(hbuf, buf);
}

static void mi_json_buf_print_count_cbin(mi_json_buf_t* hbuf,
                                         const char* prefix,
                                         mi_stat_count_t* stat,
                                         mi_chunkbin_t bin, bool add_comma) {
  const char* cbin = " ";
  switch (bin) {
    case MI_CBIN_SMALL:
      cbin = "S";
  ...
  _mi_snprintf(buf, 128,
               "%s{ \"total\": %lld, \"peak\": %lld, \"current\": %lld, "
               "\"bin\": \"%s\" }%s\n",
               prefix, stat->total, stat->peak, stat->current, cbin,
               (add_comma ? "," : ""));
  buf[127] = 0;
  mi_json_buf_print(hbuf, buf);
}

static void mi_json_buf_print_count(mi_json_buf_t* hbuf, const char* prefix,
                                    mi_stat_count_t* stat, bool add_comma) {
  char buf[128];
  _mi_snprintf(
      buf, 128, "%s{ \"total\": %lld, \"peak\": %lld, \"current\": %lld }%s\n",
      prefix, stat->total, stat->peak, stat->current, (add_comma ? "," : ""));
  buf[127] = 0;
  mi_json_buf_print(hbuf, buf);
}

static void mi_json_buf_print_count_value(mi_json_buf_t* hbuf, const char* name,
                                          mi_stat_count_t* stat) {
  char buf[128];
...
static void mi_json_buf_print_value(mi_json_buf_t* hbuf, const char* name,
                                    int64_t val) {
  char buf[128];
...
static void mi_json_buf_print_size(mi_json_buf_t* hbuf, const char* name,
                                   size_t val, bool add_comma) {
  char buf[128];
...
static void mi_json_buf_print_counter_value(mi_json_buf_t* hbuf,
                                            const char* name,
                                            mi_stat_counter_t* stat) {
  mi_json_buf_print_value(hbuf, name, stat->total);
}

#define MI_STAT_COUNT(stat) \
  mi_json_buf_print_count_value(&hbuf, #stat, &stats->stat);
#define MI_STAT_COUNTER(stat) \
  mi_json_buf_print_counter_value(&hbuf, #stat, &stats->stat);

static char* mi_stats_get_json_from(mi_stats_t* stats, size_t output_size,
                                    char* output_buf) mi_attr_noexcept {
  if (stats == NULL || stats->size != sizeof(mi_stats_t) ||
      stats->version != MI_STAT_VERSION)
    return NULL;
  ...
  for (size_t i = 0; i <= MI_BIN_HUGE; i++) {
    mi_json_buf_print_count_bin(&hbuf, "    ", &stats->malloc_bins[i], i,
                                i != MI_BIN_HUGE);
  }
  mi_json_buf_print(&hbuf, "  ],\n");
  mi_json_buf_print(&hbuf, "  \"page_bins\": [\n");
  for (size_t i = 0; i <= MI_BIN_HUGE; i++) {
    mi_json_buf_print_count_bin(&hbuf, "    ", &stats->page_bins[i], i,
                                i != MI_BIN_HUGE);
  }
  mi_json_buf_print(&hbuf, "  ],\n");
  mi_json_buf_print(&hbuf, "  \"chunk_bins\": [\n");
  for (size_t i = 0; i < MI_CBIN_COUNT; i++) {
    mi_json_buf_print_count_cbin(&hbuf, "    ", &stats->chunk_bins[i],
                                 (mi_chunkbin_t)i, i != MI_CBIN_COUNT - 1);
  }
  mi_json_buf_print(&hbuf, "  ]\n");
  mi_json_buf_print(&hbuf, "}\n");
...
char* mi_subproc_stats_get_json(mi_subproc_id_t subproc_id, size_t buf_size,
                                char* buf) mi_attr_noexcept {
  mi_subproc_t* subproc = _mi_subproc_from_id(subproc_id);
...
char* mi_heap_stats_get_json(mi_heap_t* heap, size_t buf_size,
                             char* buf) mi_attr_noexcept {
...
char* mi_stats_as_json(mi_stats_t* stats, size_t buf_size,
                       char* buf) mi_attr_noexcept {
