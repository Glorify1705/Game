...
static size_t mi_page_list_count(mi_page_t* page, mi_block_t* head) {
  ...
  size_t count = 0;
  while (head != NULL) {
    mi_assert_internal((uint8_t*)head - (uint8_t*)page >
                           (ptrdiff_t)MI_LARGE_PAGE_SIZE ||
                       page == _mi_ptr_page(head));
    count++;
...
static bool mi_page_list_is_valid(mi_page_t* page, mi_block_t* p) {
  ...
  if (page->free_is_zero) {
    ...
    for (mi_block_t* block = page->free; block != NULL;
      ...
      mi_assert_expensive(
          mi_mem_is_zero(block + 1, ubsize - sizeof(mi_block_t)));
    }
  }
#endif
  return true;
...
static void mi_page_thread_collect_to_local(mi_page_t* page, mi_block_t* head) {
  ...
  // find the last block in the list -- also to get a proper use count (without
  // data races)
...
static void mi_page_thread_free_collect(mi_page_t* page) {
  ...
  do {
    ...
    tfreex = mi_tf_create(
        NULL, mi_tf_is_owned(tfree));  // set the thread free list to NULL
  } while (!mi_atomic_cas_weak_acq_rel(&page->xthread_free, &tfree,
                                       tfreex));  // release is enough?
...
// Collect elements in the thread-free list starting at `head`. This is an
// optimized version of `_mi_page_free_collect` to be used from
// `free.c:_mi_free_collect_mt` that avoids atomic access to `xthread_free`.
//
// `head` must be in the `xthread_free` list. It will not collect `head` itself
// so the `used` count is not fully updated in general. However, if the `head`
// is the last remaining element, it will be collected and the used count will
// become `0` (so `mi_page_all_free` becomes true).
void _mi_page_free_collect_partly(mi_page_t* page, mi_block_t* head) {
  if (head == NULL) return;
  mi_block_t* next =
      mi_block_next(page, head);  // we cannot collect the head element itself
                                  // as `page->thread_free` may point to it (and
                                  // we want to avoid atomic ops)
  if (next != NULL) {
  ...
  if (page->used == 1) {
    ...
    mi_assert_internal(
        mi_tf_block(mi_atomic_load_relaxed(&page->xthread_free)) == head);
...
void _mi_page_reclaim(mi_theap_t* theap, mi_page_t* page) {
  ...
  // _mi_page_use_delayed_free(page, MI_USE_DELAYED_FREE, true); // override
never (after theap is set) _mi_page_free_collect(page, false); // ensure used
count is up to date
...
// called from `mi_free` on a reclaim, and fresh_alloc if we get an abandoned
// page
...
void _mi_page_abandon(mi_page_t* page, mi_page_queue_t* pq) {
  ...
  if (mi_page_all_free(page)) {
    ...
    page->theap = theap;  // don't actually set theap to NULL so we can
                          // reclaim_on_free within the same theap
...
static mi_page_t* mi_page_fresh_alloc(mi_theap_t* theap, mi_page_queue_t* pq,
                                      size_t block_size,
                                      size_t page_alignment) {
#if !MI_HUGE_PAGE_ABANDON
  mi_assert_internal(pq != NULL);
  mi_assert_internal(mi_theap_contains_queue(theap, pq));
  mi_assert_internal(page_alignment > 0 || block_size > MI_LARGE_MAX_OBJ_SIZE ||
...
// Move a page from the full list back to a regular list (called from
// thread-local mi_free)
...
static void mi_page_to_full(mi_page_t* page, mi_page_queue_t* pq) {
  ...
  if (theap->allow_page_abandon) {
    // abandon full pages (this is the usual case in order to allow for sharing
    // of memory between theaps)
    _mi_page_abandon(page, pq);
  } else if (!mi_page_is_in_full(page)) {
    // put full pages in a theap local queue (this is for theaps that cannot
    // abandon, for example, if the theap can be destroyed)
    mi_page_queue_enqueue_from(&mi_page_theap(page)->pages[MI_BIN_FULL], pq,
                               page);
...
#define MI_MAX_RETIRE_SIZE \
  MI_LARGE_OBJ_SIZE_MAX  // should be less than size for MI_BIN_HUGE
...
void _mi_page_retire(mi_page_t* page) mi_attr_noexcept {
  ...
  if (page->retire_expire != 0)
    return;  // already retired, just keep it retired
  mi_page_set_has_interior_pointers(page, false);
  ...
  const size_t bsize = mi_page_block_size(page);
  if mi_likely (/* bsize < MI_MAX_RETIRE_SIZE && */ !mi_page_queue_is_special(
      pq)) {                                      // not full or huge queue?
...
#endif
      page->retire_expire =
          (bsize <= MI_SMALL_MAX_OBJ_SIZE ? MI_RETIRE_CYCLES
                                          : MI_RETIRE_CYCLES / 4);
...
void _mi_theap_collect_retired(mi_theap_t* theap, bool force) {
  ...
  size_t max = 0;
  for (size_t bin = theap->page_retired_min; bin <= theap->page_retired_max;
       bin++) {
...
static void mi_theap_collect_full_pages(mi_theap_t* theap) {
  // note: normally full pages get immediately abandoned and the full queue is
always empty
  // this path is only used if abandoning is disabled due to a destroy-able
theap or options
  // set by the user.
...
static void mi_page_free_list_extend_secure(mi_theap_t* const theap,
...
static bool mi_page_extend_free(mi_theap_t* theap, mi_page_t* page) {
  ...
  if (page->slice_committed > 0) {
    ...
    const size_t needed_commit = _mi_align_up(
        mi_page_slice_offset_of(page, needed_size), MI_PAGE_MIN_COMMIT_SIZE);
    if (needed_commit > page->slice_committed) {
      ...
      if (!_mi_os_commit(mi_page_slice_start(page) + page->slice_committed,
                         needed_commit - page->slice_committed, NULL)) {
        return false;
      }
  ...
  if (extend < MI_MIN_SLICES ||
      MI_SECURE < 3) {  //! mi_option_is_enabled(mi_option_secure)) {
...
mi_decl_nodiscard bool _mi_page_init(mi_theap_t* theap, mi_page_t* page) {
  ...
  page->heap = (_mi_is_heap_main(theap->heap)
                    ? NULL
                    : theap->heap);  // faster for `mi_page_associated_theap`
  mi_page_set_theap(page, theap);
...
static mi_decl_noinline mi_page_t* mi_page_queue_find_free_ex(
    mi_theap_t* theap, mi_page_queue_t* pq, bool first_try) {
  // search through the pages in "next fit" order
  size_t count = 0;
  long candidate_limit =
      0;  // we reset this on the first candidate to limit the search
  long page_full_retain =
      (pq->block_size > MI_SMALL_MAX_OBJ_SIZE
           ? 0
           : theap->page_full_retain);  // only retain small pages
  mi_page_t* page_candidate = NULL;     // a page with free space
  mi_page_t* page = pq->first;

  while (page != NULL) {
    mi_page_t* next =
        page->next;  // remember next (as this page can move to another queue)
    count++;
    candidate_limit--;
    ...
    if (!immediate_available) {
      // collect freed blocks by us and other threads to we get a proper use
      // count
    ...
    if (!immediate_available && !mi_page_is_expandable(page)) {
      ...
      // we prefer non-expandable pages with high usage as candidates (to reduce
      // commit, and increase chances of free-ing up pages)
      ...
      // prefer to reuse fuller pages (in the hope the less used page gets
      // freed)
      else if (page->used >= page_candidate->used &&
               !mi_page_is_mostly_used(
                   page)) {  // && !mi_page_is_expandable(page)) {
        page_candidate = page;
      }
  ...
  if (page == NULL) {
    ...
    page->retire_expire = 0;
    // _mi_theap_collect_retired(theap, false); // update retire counts; note:
    // increases rss on MemoryLoad bench so don't do this
  }
  mi_assert_internal(page == NULL || mi_page_immediate_available(page));
...
static mi_page_t* mi_find_free_page(mi_theap_t* theap, mi_page_queue_t* pq) {
  ...
  // check the first page: we even do this with candidate search or otherwise we
  // re-search every time
  mi_page_t* page = pq->first;
  if mi_likely (page != NULL && mi_page_free_quick_collect(page)) {
#if (MI_SECURE >= \
     3)  // in secure mode, we extend half the time to increase randomness
    if (page->capacity < page->reserved &&
        ((_mi_theap_random_next(theap) & 1) == 1)) {
...
void _mi_deferred_free(mi_theap_t* theap, bool force) {
  ...
  if (deferred_free != NULL && !theap->tld->recurse) {
    ...
    deferred_free(force, theap->heartbeat,
                  mi_atomic_load_ptr_relaxed(void, &deferred_arg));
...
// MI_BLOCK_ALIGNMENT_MAX) so their size is not always `>
// MI_LARGE_OBJ_SIZE_MAX`.
static mi_page_t* mi_huge_page_alloc(mi_theap_t* theap, size_t size,
                                     size_t page_alignment,
                                     mi_page_queue_t* pq) {
...
#error todo.
#else
  // mi_page_queue_t* pq = mi_page_queue(theap, MI_LARGE_MAX_OBJ_SIZE+1);  //
  // always in the huge queue regardless of the block size
  mi_assert_internal(mi_page_queue_is_huge(pq));
#endif
...
// overflowed.
static mi_page_t* mi_find_page(mi_theap_t* theap, size_t size,
  ...
  mi_page_queue_t* pq = mi_page_queue(
      theap, (huge_alignment > 0 ? MI_LARGE_MAX_OBJ_SIZE + 1 : size));
  // huge allocation?
...
// have overflowed. The `huge_alignment` is normally 0 but is set to a multiple
// of MI_SLICE_SIZE for very large requested alignments in which case we use a
// huge singleton page. Note: we put `bool zero, size_t huge_alignment` into one
// parameter (with zero in the low bit) to use 4 parameters which compiles
// better on msvc for the malloc fast path.
void* _mi_malloc_generic(mi_theap_t* theap, size_t size,
  ...
  if mi_unlikely (++theap->generic_count >= 1000) {
    ...
    const long generic_collect =
        mi_option_get_clamp(mi_option_generic_collect, 1, 1000000L);
