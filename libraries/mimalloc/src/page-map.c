...
// 1 < ofs <= 127 = the slice is part of a page, starting at `(((a>>16) - ofs -
// 1) << 16)`.
//
// 1 byte per slice => 1 TiB address space needs a 2^14 * 2^16 = 16 MiB page
// map. A full 256 TiB address space (48 bit) needs a 4 GiB page map. A full 4
// GiB address space (32 bit) needs only a 64 KiB page map.
...
mi_decl_nodiscard static bool mi_page_map_ensure_committed(size_t idx,
                                                           size_t slice_count);

bool _mi_page_map_init(void) {
  size_t vbits =
      (size_t)mi_option_get_clamp(mi_option_max_vabits, 0, MI_SIZE_BITS);
  if (vbits == 0) {
  ...
  mi_page_map_max_address =
      (void*)(vbits >= MI_SIZE_BITS ? (SIZE_MAX - MI_ARENA_SLICE_SIZE + 1)
                                    : (MI_PU(1) << vbits));
  const size_t page_map_size = (MI_ZU(1) << (vbits - MI_ARENA_SLICE_SHIFT));
  const bool commit =
      (page_map_size <= 1 * MI_MiB ||
       mi_option_is_enabled(
           mi_option_pagemap_commit));  // _mi_os_has_overcommit(); // commit
                                        // on-access on Linux systems?
  const size_t commit_bits =
      _mi_divide_up(page_map_size, MI_PAGE_MAP_ENTRIES_PER_COMMIT_BIT);
  const size_t bitmap_size = (commit ? 0 : mi_bitmap_size(commit_bits, NULL));
  const size_t reserve_size = bitmap_size + page_map_size;
  uint8_t* const base = (uint8_t*)_mi_os_alloc_aligned(
      reserve_size, 1, commit, true /* allow large */, &mi_page_map_memid);
  if (base == NULL) {
    _mi_error_message(
        ENOMEM, "unable to reserve virtual memory for the page map (%zu KiB)\n",
        page_map_size / MI_KiB);
    return false;
  }
  if (mi_page_map_memid.initially_committed &&
      !mi_page_map_memid.initially_zero) {
    _mi_warning_message(
        "internal: the page map was committed but not zero initialized!\n");
  ...
  // commit the first part so NULL pointers get resolved without an access
  // violation
  if (!commit) {
...
void _mi_page_map_unsafe_destroy(mi_subproc_t* subproc) {
  ...
  _mi_os_free_ex(mi_page_map_memid.mem.os.base, mi_page_map_memid.mem.os.size,
                 true, mi_page_map_memid, subproc);
  _mi_page_map = NULL;
...
static bool mi_page_map_ensure_committed(size_t idx, size_t slice_count) {
  ...
  if (mi_page_map_commit != NULL) {
    ...
    const size_t commit_idx_hi =
        (idx + slice_count - 1) / MI_PAGE_MAP_ENTRIES_PER_COMMIT_BIT;
    for (size_t i = commit_idx; i <= commit_idx_hi;
         i++) {  // per bit to avoid crossing over bitmap chunks
      if (mi_bitmap_is_clear(mi_page_map_commit, i)) {
        // this may race, in which case we do multiple commits (which is ok)
        bool is_zero;
        uint8_t* const start =
            _mi_page_map + (i * MI_PAGE_MAP_ENTRIES_PER_COMMIT_BIT);
        const size_t size = MI_PAGE_MAP_ENTRIES_PER_COMMIT_BIT;
        if (!_mi_os_commit(start, size, &is_zero)) {
          mi_page_map_cannot_commit();
          return false;
        }
        if (!is_zero && !mi_page_map_memid.initially_zero) {
          _mi_memzero(start, size);
        }
...
static size_t mi_page_map_get_idx(mi_page_t* page, uint8_t** page_start,
                                  size_t* slice_count) {
  size_t page_size;
  *page_start = mi_page_area(page, &page_size);
  if (page_size > MI_LARGE_PAGE_SIZE) {
    page_size = MI_LARGE_PAGE_SIZE - MI_ARENA_SLICE_SIZE;
  }  // furthest interior pointer
  *slice_count = mi_slice_count_of_size(page_size) +
                 (((uint8_t*)*page_start - (uint8_t*)page) /
                  MI_ARENA_SLICE_SIZE);  // add for large aligned blocks
...
bool _mi_page_map_register(mi_page_t* page) {
  ...
  mi_assert_internal(
      _mi_page_map !=
      NULL);  // should be initialized before multi-thread access!
...
void _mi_page_map_unregister_range(void* start, size_t size) {
  ...
  if (!mi_page_map_ensure_committed(
          index, slice_count)) {  // we commit the range in total;
    return;
  }
...
mi_page_t* _mi_safe_ptr_page(const void* p) {
  ...
  if mi_unlikely (mi_page_map_commit != NULL &&
                  !mi_bitmap_is_set(mi_page_map_commit,
                                    idx / MI_PAGE_MAP_ENTRIES_PER_COMMIT_BIT))
    return NULL;
  const uintptr_t ofs = _mi_page_map[idx];
  if mi_unlikely (ofs == 0) return NULL;
  return (mi_page_t*)((((uintptr_t)p >> MI_ARENA_SLICE_SHIFT) - ofs + 1)
...
// divide the main map in 64 (`MI_BFIELD_BITS`) parts commit those parts on
// demand
static _Atomic(mi_bfield_t) mi_page_map_commit;

mi_decl_nodiscard static inline bool mi_page_map_is_committed(
    size_t idx, size_t* pbit_idx) {
...
mi_decl_nodiscard static bool mi_page_map_ensure_committed(
    size_t idx, mi_submap_t* submap) {
  mi_assert_internal(submap != NULL && *submap == NULL);
  size_t bit_idx;
  if mi_unlikely (!mi_page_map_is_committed(idx, &bit_idx)) {
    uint8_t* start =
        (uint8_t*)&_mi_page_map[bit_idx * MI_PAGE_MAP_ENTRIES_PER_CBIT];
    if (!_mi_os_commit(
            start, MI_PAGE_MAP_ENTRIES_PER_CBIT * sizeof(mi_submap_t), NULL)) {
      mi_page_map_cannot_commit();
      return false;
    }
    mi_atomic_or_acq_rel(&mi_page_map_commit, MI_ZU(1) << bit_idx);
  }
  *submap = mi_atomic_load_ptr_acquire(
      mi_page_t*, &_mi_page_map[idx]);  // acquire _mi_page_map_at(idx);
  return true;
}
...
bool _mi_page_map_init(void) {
  ...
  const size_t page_map_size =
      _mi_align_up(mi_page_map_count * sizeof(mi_page_t**), os_page_size);
  const size_t submap_size = MI_PAGE_MAP_SUB_SIZE;
  const size_t reserve_size = page_map_size + submap_size;
#if MI_SECURE
  const bool commit = true;  // the whole page map is valid and we can reliably
                             // check any pointer
#else
  const bool commit = page_map_size <= 64 * MI_KiB ||
                      mi_option_is_enabled(mi_option_pagemap_commit) ||
                      _mi_os_has_overcommit();
#endif
  _mi_page_map = (_Atomic(mi_page_t**)*)_mi_os_alloc_aligned(
      reserve_size, 1, commit, true /* allow large */, &mi_page_map_memid);
  ...
  mi_atomic_store_release(
      &mi_page_map_commit,
      (mi_page_map_memid.initially_committed ? ~MI_ZU(0) : MI_ZU(0)));

  // ensure there is a submap for the NULL address
  mi_page_t** const sub0 =
      (mi_page_t**)((uint8_t*)_mi_page_map +
                    page_map_size);  // we reserved a submap part at the end
                                     // already
  if (!mi_page_map_memid.initially_committed) {
    if (!_mi_os_commit(sub0, submap_size,
                       NULL)) {  // commit full submap (issue #1087)
      mi_page_map_cannot_commit();
      return false;
    }
  }
  if (!mi_page_map_memid
           .initially_zero) {  // initialize low addresses with NULL
  ...
  mi_lock_init(&mi_page_map_lock);  // initialize late in case the lock init
                                    // causes allocation
...
void _mi_page_map_unsafe_destroy(mi_subproc_t* subproc) {
  ...
  for (size_t idx = 1; idx < mi_page_map_count;
       idx++) {  // skip entry 0 (as we allocate that submap at the end of the
                 // page_map)
    // free all sub-maps
    if (mi_page_map_is_committed(idx, NULL)) {
      ...
      if (sub != NULL) {
        mi_memid_t memid =
            _mi_memid_create_os(sub, MI_PAGE_MAP_SUB_SIZE, true, false, false);
        _mi_os_free_ex(memid.mem.os.base, memid.mem.os.size, true, memid,
                       subproc);
        mi_atomic_store_ptr_release(mi_page_t*, &_mi_page_map[idx], NULL);
      }
    }
  }
  _mi_os_free_ex(_mi_page_map, mi_page_map_memid.mem.os.size, true,
                 mi_page_map_memid, subproc);
  _mi_page_map = NULL;
  mi_page_map_count = 0;
  mi_page_map_memid = _mi_memid_none();
  mi_page_map_max_address = NULL;
...
mi_decl_nodiscard static bool mi_page_map_ensure_submap_at(
    size_t idx, mi_submap_t* submap) {
  mi_assert_internal(submap != NULL && *submap == NULL);
  ...
  if mi_unlikely (sub == NULL) {
    ...
    mi_lock(&mi_page_map_lock) {
      sub =
          mi_atomic_load_ptr_acquire(mi_page_t*, &_mi_page_map[idx]);  // reload
      if (sub == NULL)  // not yet allocated by another thread?
      {
        ...
        if (sub == NULL) {
          ...
          if (!mi_atomic_cas_ptr_strong_acq_rel(mi_page_t*, &_mi_page_map[idx],
                                                &expect, sub)) {
...
static bool mi_page_map_set_range_prim(mi_page_t* page, size_t idx,
                                       size_t sub_idx, size_t slice_count) {
  // is the page map area that contains the page address committed?
  while (slice_count > 0) {
    mi_submap_t sub = NULL;
...
static bool mi_page_map_set_range(mi_page_t* page, size_t idx, size_t sub_idx,
                                  size_t slice_count) {
  if mi_unlikely (!mi_page_map_set_range_prim(page, idx, sub_idx,
                                              slice_count)) {
...
static size_t mi_page_map_get_idx(mi_page_t* page, size_t* sub_idx,
                                  size_t* slice_count) {
  size_t page_size;
...
void _mi_page_map_unregister_range(void* start, size_t size) {
  ...
  mi_page_map_set_range(
      NULL, idx, sub_idx,
      slice_count);  // todo: avoid committing if not already committed?
}
