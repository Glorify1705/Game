...
mi_decl_thread mi_thread_locals_t* mi_thread_locals =
    &mi_thread_locals_empty;  // always point to a valid `mi_thread_locals_t`
...
static mi_thread_locals_t* mi_thread_locals_expand(mi_thread_local_t atleast) {
  ...
  mi_thread_locals_t* tls = (mi_thread_locals_t*)mi_rezalloc(
      tls_old, sizeof(mi_thread_locals_t) + count * sizeof(void*));
...
static mi_decl_noinline bool mi_thread_local_set_expand(mi_thread_local_t key,
                                                        void* val) {
...
static mi_lock_t mi_thread_locals_lock;  // we need a lock in order to
                                         // re-allocate the slot bits
static mi_bitmap_t*
    mi_thread_locals_free;  // reuse an arena bitmap to track which slots were
                            // assigned (1=free, 0=in-use)

void _mi_thread_locals_init(void) { mi_lock_init(&mi_thread_locals_lock); }
...
// strange signature but allows us to reuse the arena code for claiming free
// pages
static bool mi_thread_local_claim(size_t _slice_index, mi_arena_t* _arena,
                                  bool* keep_set) {
...
static mi_thread_local_t mi_thread_local_create_expand(void) {
  ...
  if (slots != NULL) {
    mi_bitmap_init(slots, newcount,
                   true /* or otherwise we would zero all old entries */);
    mi_bitmap_unsafe_setN(slots, oldcount, newcount - oldcount);
    mi_thread_locals_free = slots;
    size_t idx = 0;
    if mi_likely (slots != NULL &&
                  mi_bitmap_try_find_and_claim(slots, 0, &idx,
                                               &mi_thread_local_claim, NULL)) {
      key = idx + 1;
    }
  }
  return key;
