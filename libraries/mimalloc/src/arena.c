...
mi_arena_id_t _mi_arena_id_none(void) { return NULL; }

mi_arena_t* _mi_arena_from_id(mi_arena_id_t id) { return (mi_arena_t*)id; }

static bool mi_arena_id_is_suitable(mi_arena_t* arena, mi_arena_t* req_arena) {
  return ((arena == req_arena) ||  // they match,
          (req_arena == NULL &&
           !arena->is_exclusive));  // or the arena is not exclusive, and we
...
#if MI_DEBUG > 1
static bool mi_heap_has_page(mi_heap_t* heap, mi_arena_t* arena,
                             mi_page_t* page) {
  ...
  return (page->memid.memkind == MI_MEM_ARENA &&
          page->memid.mem.arena.arena == arena && arena_pages != NULL &&
          mi_bitmap_is_setN(arena_pages->pages,
                            page->memid.mem.arena.slice_index, 1));
}
#endif

size_t mi_arena_min_alignment(void) { return MI_ARENA_SLICE_ALIGN; }
...
static size_t mi_arena_max_object_size(void) {
  ...
  if (max_size <= MI_ARENA_MIN_OBJ_SIZE) {
    return MI_ARENA_MIN_OBJ_SIZE;
             ...
             MI_ARENA_MAX_SIZE -
                 MI_BCHUNK_SIZE) {  // minus a bchunk to accommodate meta info
...
mi_decl_nodiscard static bool mi_arena_commit(mi_arena_t* arena, void* start,
                                              size_t size, bool* is_zero,
                                              size_t already_committed) {
  if (arena != NULL && arena->commit_fun != NULL) {
    return (*arena->commit_fun)(true, start, size, is_zero,
                                arena->commit_fun_arg);
...
static uint8_t* mi_arena_start(mi_arena_t* arena) { return ((uint8_t*)arena); }
...
static mi_memid_t mi_memid_create_arena(mi_arena_t* arena, size_t slice_index,
                                        size_t slice_count) {
...
static mi_arena_t* mi_arena_from_memid(mi_memid_t memid, size_t* slice_index,
                                       size_t* slice_count) {
...
static size_t mi_page_full_size(mi_page_t* page) {
  ...
  } else if (mi_memid_is_os(page->memid) ||
             page->memid.memkind == MI_MEM_EXTERNAL) {
    mi_assert_internal((uint8_t*)page->memid.mem.os.base <= (uint8_t*)page);
    const ptrdiff_t presize =
        (uint8_t*)page - (uint8_t*)page->memid.mem.os.base;
    mi_assert_internal((ptrdiff_t)page->memid.mem.os.size >= presize);
    return (presize > (ptrdiff_t)page->memid.mem.os.size
                ? 0
                : page->memid.mem.os.size - presize);
  } else {
    return 0;
  }
}
...
static mi_decl_noinline void* mi_arena_try_alloc_at(mi_arena_t* arena,
                                                    size_t slice_count,
                                                    bool commit, size_t tseq,
                                                    mi_memid_t* memid) {
  size_t slice_index;
  if (!mi_bbitmap_try_find_and_clearN(arena->slices_free, slice_count, tseq,
                                      &slice_index))
    return NULL;
  ...
  *memid = mi_memid_create_arena(arena, slice_index, slice_count);
  ...
  if (arena->memid.initially_zero) {
    ...
    memid->initially_zero = mi_bitmap_setN(arena->slices_dirty, slice_index,
                                           slice_count, &already_dirty);
  ...
  if (commit) {
    ...
    // it is committed now
    const size_t already_committed =
        mi_bitmap_popcountN(arena->slices_committed, slice_index, slice_count);
    if (already_committed < slice_count) {
      // not all committed, try to commit now
      bool commit_zero = false;
      if (!_mi_os_commit_ex(
              p, mi_size_of_slices(slice_count), &commit_zero,
              mi_size_of_slices(slice_count - already_committed))) {
...
// committed
      ...
      if (memid->initially_zero) {
        if (!mi_mem_is_zero(p, mi_size_of_slices(slice_count))) {
          _mi_error_message(
              EFAULT,
              "internal error: arena allocation was not zero-initialized!\n");
      ...
      // if the OS has overcommit, and this is the first time we access these
      // pages, then count the commit now (as at arena reserve we didn't count
      // those commits as these are on-demand)
      if (_mi_os_has_overcommit() && touched_slices > 0) {
        mi_subproc_stat_increase(arena->subproc, committed,
                                 mi_size_of_slices(touched_slices));
      }
    }

    mi_assert_internal(
        mi_bitmap_is_setN(arena->slices_committed, slice_index, slice_count));
    ...
    if (!memid->initially_committed) {
      ...
      mi_bitmap_setN(arena->slices_committed, slice_index, slice_count,
                     &already_committed_count);
      mi_bitmap_clearN(arena->slices_committed, slice_index, slice_count);
      mi_subproc_stat_decrease(arena->subproc, committed,
...
static int mi_reserve_os_memory_ex2(mi_subproc_t* subproc, size_t size,
                                    bool commit, bool allow_large,
                                    bool exclusive, mi_arena_id_t* arena_id);

// try to reserve a fresh arena space
static bool mi_arena_reserve(mi_subproc_t* subproc, size_t req_size,
                             bool allow_large, mi_arena_id_t* arena_id) {
  ...
  req_size =
      _mi_align_up(req_size + MI_ARENA_MAX_CHUNK_OBJ_SIZE,
                   MI_ARENA_MAX_CHUNK_OBJ_SIZE);  // over-reserve for meta-info
  ...
  // on an OS with overcommit (Linux) we don't count the commit yet as it is
  // on-demand. Once a slice is actually allocated for the first time it will be
  // counted.
  ...
  int err =
      mi_reserve_os_memory_ex2(subproc, arena_reserve, arena_commit,
                               allow_large, false /* exclusive? */, arena_id);
  if (err != 0) {
    if (adjust) {
      mi_subproc_stat_adjust_increase(subproc, committed, arena_reserve);
    }  // roll back
    // failed to allocate: try a smaller size arena as fallback?
    const size_t small_arena_reserve =
        4 * MI_ARENA_MIN_SIZE;  // 128 MiB (or 32 MiB on 32-bit)
    if (arena_reserve > small_arena_reserve && small_arena_reserve > req_size) {
      // try again
      if (adjust) {
        mi_subproc_stat_adjust_decrease(subproc, committed,
                                        small_arena_reserve);
      }
      err = mi_reserve_os_memory_ex2(subproc, small_arena_reserve, arena_commit,
                                     allow_large, false /* exclusive? */,
                                     arena_id);
      if (err != 0 && adjust) {
        mi_subproc_stat_adjust_increase(subproc, committed,
                                        small_arena_reserve);
      }  // roll back
    }
  }
...
static inline bool mi_arena_is_suitable(mi_arena_t* arena,
                                        mi_arena_t* req_arena, bool match_numa,
                                        int numa_node, bool allow_pinned) {
...
static size_t mi_arena_start_idx(mi_heap_t* heap, size_t tseq,
                                 size_t arena_cycle) {
  ...
  if (hseq == 0 || hcount <= 1)
    return (tseq % arena_cycle);  // common for single heap programs

  // spread heaps evenly among arena's, and then evenly for threads in their
  // fraction
  size_t start;
  mi_assert_internal(arena_cycle <= 0x8FF);  // prevent overflow on 32-bit
  const size_t frac =
      (arena_cycle * 256) /
      hcount;  // fraction in the arena_cycle; at most: arena_cycle * 0x100
  if (frac == 0) {
...
#define mi_forall_arenas(heap, req_arena, tseq, name_arena)               \
  {                                                                       \
    ...
    const size_t _arena_cycle =                                           \
        (_arena_count == 0                                                \
             ? 0                                                          \
             : _arena_count -                                             \
                   1); /* first search the arenas below the last one */   \
    ...
    for (size_t _i = 0; _i < _arena_count; _i++) {                        \
      ...
      if (req_arena != NULL) {                                            \
        name_arena = req_arena; /* if there is a specific req_arena, only \
                                   search that one */                     \
        ...
        if (_i < _arena_cycle) {                                          \
          ...
          if (_idx >= _arena_cycle) {                                     \
            _idx -= _arena_cycle;                                         \
          } /* adjust so we rotate through the cycle */                   \
        } else {                                                          \
...
#define mi_forall_suitable_arenas(heap, req_arena, tseq, match_numa,       \
                                  numa_node, allow_large, name_arena)      \
  mi_forall_arenas(heap, req_arena, tseq, name_arena) {                    \
    if (mi_arena_is_suitable(name_arena, req_arena, match_numa, numa_node, \
                             allow_large)) {
...
static mi_decl_noinline void* mi_arenas_try_find_free(
    mi_heap_t* heap, size_t slice_count, size_t alignment, bool commit,
    bool allow_large, mi_arena_t* req_arena, size_t tseq, int numa_node,
    mi_memid_t* memid) {
  // mi_assert_internal(slice_count <=
  // mi_slice_count_of_size(MI_ARENA_MAX_CHUNK_OBJ_SIZE));
  mi_assert(alignment <= MI_ARENA_SLICE_ALIGN);
  if (alignment > MI_ARENA_SLICE_ALIGN) return NULL;

  // search arena's
  mi_forall_suitable_arenas(heap, req_arena, tseq,
                            true /* only numa matching */, numa_node,
                            allow_large, arena) {
  ...
  mi_forall_suitable_arenas(heap, req_arena, tseq,
                            false /* numa non-matching now */, numa_node,
                            allow_large, arena) {
    void* p = mi_arena_try_alloc_at(arena, slice_count, commit, tseq, memid);
    if (p != NULL) return p;
  }
...
static mi_decl_noinline void* mi_arenas_try_alloc(
  ...
  p = mi_arenas_try_find_free(heap, slice_count, alignment, commit, allow_large,
                              req_arena, tseq, numa_node, memid);
  if (p != NULL) return p;
  ...
  // otherwise, try to reserve a new arena -- but one thread at a time.. (todo:
  // allow 2 or 4 to reduce contention?)
  ...
  mi_lock(&subproc->arena_reserve_lock) {
    if (arena_count == mi_arenas_get_count(subproc)) {
      ...
      mi_arena_reserve(subproc, mi_size_of_slices(slice_count), allow_large,
                       &arena_id);
    } else {
...
static void* mi_arena_os_alloc_aligned(size_t size, size_t alignment,
...
void* _mi_arenas_alloc_aligned(mi_heap_t* heap, size_t size, size_t alignment,
                               size_t align_offset, bool commit,
                               bool allow_large, mi_arena_t* req_arena,
                               size_t tseq, int numa_node, mi_memid_t* memid) {
  mi_assert_internal(memid != NULL);
  mi_assert_internal(size > 0);

  // try to allocate in an arena if the alignment is small enough and the object
  // is not too small (as for theap meta data)
  if (!mi_option_is_enabled(
          mi_option_disallow_arena_alloc) &&  // is arena allocation allowed?
      size >= MI_ARENA_MIN_OBJ_SIZE &&
      size <= mi_arena_max_object_size() &&  // and not too small or too large
      alignment <= MI_ARENA_SLICE_ALIGN &&
      align_offset == 0)  // and good alignment
  {
    const size_t slice_count = mi_slice_count_of_size(size);
    void* p =
        mi_arenas_try_alloc(heap, slice_count, alignment, commit, allow_large,
                            req_arena, tseq, numa_node, memid);
    if (p != NULL) return p;
  }

  // fall back to the OS
  void* p = mi_arena_os_alloc_aligned(size, alignment, align_offset, commit,
                                      allow_large, req_arena, memid);
  return p;
}

void* _mi_arenas_alloc(mi_heap_t* heap, size_t size, bool commit,
                       bool allow_large, mi_arena_t* req_arena, size_t tseq,
                       int numa_node, mi_memid_t* memid) {
  return _mi_arenas_alloc_aligned(heap, size, MI_ARENA_SLICE_SIZE, 0, commit,
                                  allow_large, req_arena, tseq, numa_node,
                                  memid);
}
...
// release ownership of a page. This may free the page if all blocks were
// concurrently freed in the meantime. Returns true if the page was freed.
static bool mi_abandoned_page_unown(mi_page_t* page,
                                    mi_theap_t* current_theap) {
  ...
  do {
    ...
    while
      mi_unlikely(mi_tf_block(tf_old) != NULL) {
        _mi_page_free_collect(page, false);  // update used
        if (mi_page_all_free(
                page)) {  // it may become free just before unowning it
...
static bool mi_arena_try_claim_abandoned(size_t slice_index, mi_arena_t* arena,
                                         bool* keep_abandoned) {
  ...
  if (!mi_page_claim_ownership(page)) {
    ...
    // we need to keep it in the abandoned map as the free will call
    // `mi_arena_page_unabandon`, and wait for readers (us!) to finish. This is
    // why it is very important to set the abandoned bit again (or otherwise the
    // unabandon will never stop waiting).
    *keep_abandoned = true;
    return false;
  } else {
...
static mi_arena_pages_t* mi_heap_arena_pages(mi_heap_t* heap,
                                             mi_arena_t* arena) {
  mi_assert_internal(arena != NULL);
  mi_assert_internal(heap != NULL);
  mi_assert(arena->arena_idx < MI_MAX_ARENAS);
  return mi_atomic_load_ptr_relaxed(mi_arena_pages_t,
                                    &heap->arena_pages[arena->arena_idx]);
}

static mi_arena_t* mi_page_arena_pages(mi_page_t* page, size_t* slice_index,
                                       size_t* slice_count,
                                       mi_arena_pages_t** parena_pages) {
  // todo: maybe store the arena* directly in the page?
  mi_assert_internal(mi_page_is_owned(page));
  mi_arena_t* const arena =
      mi_arena_from_memid(page->memid, slice_index, slice_count);
  mi_assert_internal(arena != NULL);
  if (parena_pages != NULL) {
    mi_arena_pages_t* const arena_pages =
        mi_heap_arena_pages(mi_page_heap(page), arena);
    mi_assert_internal(arena_pages != NULL);
    mi_assert_internal(slice_index == NULL ||
...
static mi_arena_pages_t* mi_heap_ensure_arena_pages(mi_heap_t* heap,
                                                    mi_arena_t* arena) {
  mi_assert_internal(arena != NULL);
  mi_assert_internal(heap != NULL);
  mi_assert(arena->arena_idx < MI_MAX_ARENAS);
    ...
    mi_lock(&heap->arena_pages_lock) {
      ...
      if (arena_pages == NULL) {  // still NULL?
        if (_mi_is_heap_main(heap)) {
          // the page info for the main heap is always allocated as part of an
          // arena
        ...
        mi_atomic_store_ptr_release(mi_arena_pages_t,
                                    &heap->arena_pages[arena->arena_idx],
                                    arena_pages);
      }
    }
  ...
  if (_mi_is_heap_main(heap)) {
    mi_assert(arena_pages != NULL);
  }  // can never fail
  return arena_pages;
}

static mi_page_t* mi_arenas_page_try_find_abandoned(mi_theap_t* theap,
                                                    size_t slice_count,
                                                    size_t block_size) {
  ...
  mi_forall_suitable_arenas(heap, req_arena, tseq, match_numa, any_numa,
                            allow_large, arena) {
    ...
    if (arena_pages != NULL) {
      ...
      if (mi_bitmap_try_find_and_claim(bitmap, tseq, &slice_index,
        ...
        mi_assert_internal(
            mi_bbitmap_is_clearN(arena->slices_free, slice_index, slice_count));
        mi_assert_internal(page->slice_committed > 0 ||
                           mi_bitmap_is_setN(arena->slices_committed,
...
static mi_page_t* mi_arenas_page_alloc_fresh(mi_theap_t* theap,
                                             size_t slice_count,
                                             size_t block_size,
                                             size_t block_alignment,
                                             bool commit) {
  const bool allow_large =
      (MI_SECURE < 2);  // 2 = guard page at end of each arena page
  ...
  mi_arena_t* const req_arena = heap->exclusive_arena;
  const int numa_node =
      (heap->numa_node >= 0 ? heap->numa_node : tld->numa_node);
  ...
  if (!mi_option_is_enabled(
          mi_option_disallow_arena_alloc) &&  // allowed to allocate from
                                              // arena's?
      !os_align &&                            // not large alignment
      slice_count <= mi_arena_max_object_size() /
                         MI_ARENA_SLICE_SIZE)  // and not too large
  {
    page = (mi_page_t*)mi_arenas_try_alloc(heap, slice_count, page_alignment,
                                           commit, allow_large, req_arena,
                                           tld->thread_seq, numa_node, &memid);
    if (page != NULL) {
      mi_arena_pages_t* const arena_pages =
          mi_heap_ensure_arena_pages(heap, memid.mem.arena.arena);
      if (arena_pages == NULL) {
        _mi_arenas_free(page, mi_size_of_slices(slice_count),
                        page->memid);  // roll back
        page = NULL;
      } else {
        mi_assert_internal(mi_bitmap_is_clearN(arena_pages->pages,
                                               memid.mem.arena.slice_index,
                                               memid.mem.arena.slice_count));
  ...
  if (page == NULL) {
    if (os_align) {
      ...
      mi_assert_internal(slice_count >=
                         mi_slice_count_of_size(block_size) +
                             mi_slice_count_of_size(page_alignment));
      page = (mi_page_t*)mi_arena_os_alloc_aligned(
          alloc_size, block_alignment, page_alignment /* align offset */,
          commit, allow_large, req_arena, &memid);
    } else {
      page = (mi_page_t*)mi_arena_os_alloc_aligned(
          alloc_size, page_alignment, 0 /* align offset */, commit, allow_large,
          req_arena, &memid);
    }
  }
  ...
  mi_assert_internal(
      !os_align ||
      _mi_is_aligned((uint8_t*)page + page_alignment, block_alignment));
...
// guard page at the end of mimalloc page?
  ...
  if (memid.initially_zero && memid.initially_committed) {
    if (!mi_mem_is_zero(page, page_noguard_size)) {
      _mi_error_message(
          EFAULT, "internal error: page memory was not zero initialized.\n");
      memid.initially_zero = false;
      _mi_memzero_aligned(page, sizeof(*page));
    }
  }
  ...
  size_t block_start;
#if MI_GUARDED
  // in a guarded build, we align pages with blocks a multiple of an OS page
  // size, to the OS page size this ensures that all blocks in such pages are OS
  // page size aligned (which is needed for the guard pages)
  const size_t os_page_size = _mi_os_page_size();
  mi_assert_internal(MI_PAGE_ALIGN >= os_page_size);
  if (!os_align && block_size % os_page_size == 0 &&
      block_size > os_page_size /* at least 2 or more */) {
    ...
    block_start = MI_PAGE_ALIGN;
             ...
             block_size <= MI_PAGE_MAX_START_BLOCK_ALIGN2) {
    // naturally align power-of-2 blocks up to MI_PAGE_MAX_START_BLOCK_ALIGN2
    // size (4KiB)
    block_start = _mi_align_up(mi_page_info_size(), block_size);
  } else if (block_size != 0 &&
             (block_size % MI_PAGE_OSPAGE_BLOCK_ALIGN2) == 0) {
    // also align large pages that are a multiple of MI_PAGE_OSPAGE_BLOCK_ALIGN2
    // (4KiB)
    block_start =
        _mi_align_up(mi_page_info_size(), MI_PAGE_OSPAGE_BLOCK_ALIGN2);
  } else {
    // otherwise start after the info
    block_start = mi_page_info_size();
  }
  const size_t reserved =
      (os_align ? 1 : (page_noguard_size - block_start) / block_size);
  ...
  if (!memid.initially_committed) {
    commit_size =
        _mi_align_up(block_start + block_size, MI_PAGE_MIN_COMMIT_SIZE);
    if (commit_size > page_noguard_size) {
      commit_size = page_noguard_size;
    }
    bool is_zero;
    if mi_unlikely (!mi_arena_commit(mi_memid_arena(memid), page, commit_size,
                                     &is_zero, 0)) {
...
static mi_page_t* mi_arenas_page_regular_alloc(mi_theap_t* theap,
                                               size_t slice_count,
                                               size_t block_size) {
  // 1. look for an abandoned page
  mi_page_t* page =
      mi_arenas_page_try_find_abandoned(theap, slice_count, block_size);
  if (page != NULL) {
  ...
  const bool commit =
      (slice_count <=
           mi_slice_count_of_size(
               MI_PAGE_MIN_COMMIT_SIZE) ||  // always commit small pages
  ...
  mi_assert_internal(page->memid.memkind != MI_MEM_ARENA ||
                     page->memid.mem.arena.slice_count == slice_count);
...
static mi_page_t* mi_arenas_page_singleton_alloc(mi_theap_t* theap,
                                                 size_t block_size,
                                                 size_t block_alignment) {
  const bool os_align = (block_alignment > MI_PAGE_MAX_OVERALLOC_ALIGN);
...
#else
  const size_t slice_count = mi_slice_count_of_size(
      _mi_align_up(info_size + block_size, _mi_os_secure_guard_page_size()) +
      _mi_os_secure_guard_page_size());
#endif

  mi_page_t* page = mi_arenas_page_alloc_fresh(
      theap, slice_count, block_size, block_alignment,
      true /* commit singletons always */);
  if (page == NULL) return NULL;
...
mi_page_t* _mi_arenas_page_alloc(mi_theap_t* theap, size_t block_size,
                                 size_t block_alignment) {
  mi_page_t* page;
  if mi_unlikely (block_alignment > MI_PAGE_MAX_OVERALLOC_ALIGN) {
    ...
    page = mi_arenas_page_regular_alloc(
        theap, mi_slice_count_of_size(MI_SMALL_PAGE_SIZE), block_size);
  } else if (block_size <= MI_MEDIUM_MAX_OBJ_SIZE) {
    page = mi_arenas_page_regular_alloc(
        theap, mi_slice_count_of_size(MI_MEDIUM_PAGE_SIZE), block_size);
  }
#if MI_ENABLE_LARGE_PAGES
  else if (block_size <= MI_LARGE_MAX_OBJ_SIZE) {
    page = mi_arenas_page_regular_alloc(
        theap, mi_slice_count_of_size(MI_LARGE_PAGE_SIZE), block_size);
  }
#endif
  else {
    page = mi_arenas_page_singleton_alloc(theap, block_size, block_alignment);
  ...
  mi_assert_internal(_mi_ptr_page(page) == page);
  mi_assert_internal(_mi_ptr_page(mi_page_start(page)) == page);
  mi_assert_internal(block_alignment <= MI_PAGE_MAX_OVERALLOC_ALIGN ||
                     _mi_is_aligned(mi_page_start(page), block_alignment));
...
void _mi_arenas_page_free(mi_page_t* page, mi_theap_t* current_theapx) {
  ...
  mi_assert_internal(current_theapx == NULL ||
                     _mi_thread_id() == current_theapx->tld->thread_id);

  if (current_theapx != NULL) {
    mi_theap_stat_decrease(current_theapx, page_bins[_mi_page_stats_bin(page)],
                           1);
  ...
  if (page->memid.memkind == MI_MEM_ARENA && !mi_page_is_full(page)) {
    ...
    mi_arena_t* const arena =
        mi_page_arena_pages(page, &slice_index, &slice_count, &arena_pages);
    mi_assert_internal(
        mi_bbitmap_is_clearN(arena->slices_free, slice_index, slice_count));
    mi_assert_internal(
        page->slice_committed > 0 ||
        mi_bitmap_is_setN(arena->slices_committed, slice_index, slice_count));
    mi_assert_internal(
    ...
    // note: we cannot check for `!mi_page_is_abandoned_and_mapped` since that
    // may be (temporarily) not true if the free happens while trying to reclaim
    // see `mi_arena_try_claim_abandoned`
  }
#endif

// recommit guard page at the end?
// we must do this since we may later allocate large spans over this page and
// cannot have a guard page in between
#if MI_SECURE >= 2
  if (!page->memid.is_pinned) {
    _mi_os_secure_guard_page_reset_before(
        (uint8_t*)page + mi_page_full_size(page), page->memid);
  }
#endif
  ...
  if (page->memid.memkind == MI_MEM_ARENA) {
    ...
    if (page->slice_committed > 0) {
      ...
      const size_t total_slices =
          page->slice_committed / MI_ARENA_SLICE_SIZE;  // conservative
      // mi_assert_internal(mi_bitmap_is_clearN(arena->slices_committed,
      // slice_index, total_slices));
      mi_assert_internal(slice_count >= total_slices);
      if (total_slices > 0) {
        mi_bitmap_setN(arena->slices_committed, slice_index, total_slices,
                       NULL);
      }
      // any left over?
...
void _mi_arenas_page_abandon(mi_page_t* page, mi_theap_t* current_theap) {
  ...
  mi_assert_internal(heap == current_theap->heap);
  if (page->memid.memkind == MI_MEM_ARENA && !mi_page_is_full(page)) {
...
bool _mi_arenas_page_try_reabandon_to_mapped(mi_page_t* page) {
  ...
  mi_assert_internal(!mi_page_all_free(page));
  mi_assert_internal(!mi_page_is_singleton(page));
  if (mi_page_is_full(page) || mi_page_is_abandoned_mapped(page) ||
      page->memid.memkind != MI_MEM_ARENA) {
    return false;
  } else {
    // do not use _mi_heap_theap as we may call this during shutdown of threads
    // and don't want to reinitialize the theap
    mi_theap_t* const theap = _mi_page_associated_theap_peek(page);
    if (theap == NULL) {
      return false;
    } else {
      mi_theap_stat_counter_increase(theap, pages_reabandon_full, 1);
      mi_theap_stat_adjust_decrease(
          theap, pages_abandoned, 1);  // adjust as we are not abandoning fresh
  ...
  if (mi_page_is_abandoned_mapped(page)) {
    ...
    if (page->memid.memkind != MI_MEM_ARENA &&
      ...
      mi_lock(&heap->os_abandoned_pages_lock) {
        ...
        if (heap->os_abandoned_pages == page) {
          heap->os_abandoned_pages = page->next;
        }
        page->next = NULL;
        page->prev = NULL;
      }
...
/* -----------------------------------------------------------
  Arena free
----------------------------------------------------------- */
static void mi_arena_schedule_purge(mi_arena_t* arena, size_t slice_index,
                                    size_t slices);
static void mi_arenas_try_purge(bool force, bool visit_all,
                                mi_subproc_t* subproc, size_t tseq);
...
void _mi_arenas_free(void* p, size_t size, mi_memid_t memid) {
  ...
  if (mi_memkind_is_os(memid.memkind)) {
    ...
    mi_assert_internal(mi_arena_slice_start(arena, slice_index) +
                           mi_size_of_slices(slice_count) >
                       (uint8_t*)p);
    // checks
    if (arena == NULL) {
      _mi_error_message(
    ...
    if (slice_index < mi_arena_info_slices(arena) ||
        slice_index > arena->slice_count) {
    ...
    if (!arena->memid
             .is_pinned /* && !arena->memid.initially_committed */) {  // todo:
                                                                       // allow
                                                                       // decommit
                                                                       // even
                                                                       // if
    ...
    if (!all_inuse) {
      _mi_error_message(
          ...
          mi_arena_slice_start(arena, slice_index),
          mi_size_of_slices(slice_count));
      return;
    };
...
bool mi_arena_contains(mi_arena_id_t arena_id, const void* p) {
  ...
  return (mi_arena_start(arena) <= (const uint8_t*)p &&
          mi_arena_start(arena) + mi_size_of_slices(arena->slice_count) >
              (const uint8_t*)p);
}
...
bool _mi_arenas_contain(const void* p) {
  ...
  for (size_t i = 0; i < max_arena; i++) {
    mi_arena_t* arena =
        mi_atomic_load_ptr_acquire(mi_arena_t, &subproc->arenas[i]);
  ...
  for (size_t i = 0; i < arena_count; i++) {
    ...
    if (arena != NULL) {
      ...
      if (mi_memkind_is_os(arena->memid.memkind)) {
        _mi_os_free_ex(mi_arena_start(arena), mi_arena_size(arena), true,
                       arena->memid,
                       subproc);  // pass `subproc` to avoid accessing the theap
                                  // pointer (in `_mi_subproc()`)
      }
    }
  }
...
void _mi_arenas_unsafe_destroy_all(mi_subproc_t* subproc) {
  ...
  // mi_arenas_try_purge(true /* force purge */, true /* visit all*/, subproc, 0
  // /* thread seq */);  // purge non-owned arenas
}
...
static bool mi_arenas_add(mi_subproc_t* subproc, mi_arena_t* arena,
  ...
  for (size_t i = 0; i < count; i++) {
    if (mi_arena_from_index(subproc, i) == NULL) {
      arena->arena_idx = i;
      expected = NULL;
      if (mi_atomic_cas_ptr_strong_release(mi_arena_t, &subproc->arenas[i],
                                           &expected, arena)) {
        // success
        if (arena_id != NULL) {
          *arena_id = arena;
  ...
  while (count < MI_MAX_ARENAS) {
    if (mi_atomic_cas_strong_release(&subproc->arena_count, &count,
                                     count + 1)) {
      arena->arena_idx = count;
      expected = NULL;
      if (mi_atomic_cas_ptr_strong_release(mi_arena_t, &subproc->arenas[count],
                                           &expected, arena)) {
...
static size_t mi_arena_pages_size(size_t slice_count, size_t* bitmap_base) {
  ...
  const size_t base_size =
      _mi_align_up(sizeof(mi_arena_pages_t), MI_BCHUNK_SIZE);
...
static size_t mi_arena_info_slices_needed(size_t slice_count,
                                          size_t* bitmap_base) {
  if (slice_count == 0) slice_count = MI_BCHUNK_BITS;
  mi_assert_internal((slice_count % MI_BCHUNK_BITS) == 0);
  const size_t base_size = _mi_align_up(sizeof(mi_arena_t), MI_BCHUNK_SIZE);
  const size_t bitmaps_count =
      4 + MI_ARENA_BIN_COUNT;  // commit, dirty, purge, pages, and abandoned
  const size_t bitmaps_size =
      bitmaps_count * mi_bitmap_size(slice_count, NULL) +
      mi_bbitmap_size(slice_count, NULL);  // + free
  const size_t size = base_size + bitmaps_size;

  const size_t os_page_size = _mi_os_page_size();
  const size_t info_size =
      _mi_align_up(size, os_page_size) + _mi_os_secure_guard_page_size();
...
static mi_bitmap_t* mi_arena_bitmap_init(size_t slice_count, uint8_t** base) {
  ...
  *base =
      (*base) + mi_bitmap_init(bitmap, slice_count, true /* already zero */);
  return bitmap;
}
...
static mi_bbitmap_t* mi_arena_bbitmap_init(size_t slice_count, uint8_t** base) {
  ...
  *base =
      (*base) + mi_bbitmap_init(bbitmap, slice_count, true /* already zero */);
  return bbitmap;
}
...
static mi_arena_pages_t* mi_arena_pages_alloc(mi_arena_t* arena) {
  ...
  mi_arena_pages_t* arena_pages = (mi_arena_pages_t*)mi_heap_zalloc_aligned(
      mi_heap_main(), size, MI_BCHUNK_SIZE);
...
static bool mi_manage_os_memory_ex2(mi_subproc_t* subproc, void* start,
                                    size_t size, int numa_node, bool exclusive,
                                    mi_memid_t memid,
  ...
  if (!_mi_is_aligned(start, MI_ARENA_SLICE_SIZE)) {
    // we can align the start since the memid tracks the real base of the
    // memory.
  ...
  const size_t slice_count =
      _mi_align_down(size / MI_ARENA_SLICE_SIZE, MI_BCHUNK_BITS);
  if (slice_count > MI_BITMAP_MAX_BIT_COUNT) {  // 16 GiB for now
    // todo: allow larger areas (either by splitting it up in arena's or having
    // larger arena's)
    _mi_warning_message(
        "cannot use OS memory since it is too large (size %zu MiB, maximum is "
        "%zu MiB)",
        size / MI_MiB, mi_size_of_slices(MI_BITMAP_MAX_BIT_COUNT) / MI_MiB);
    return false;
  }
  size_t bitmap_base;
  const size_t info_slices =
      mi_arena_info_slices_needed(slice_count, &bitmap_base);
  if (slice_count < info_slices + 1) {
    _mi_warning_message(
        "cannot use OS memory since it is not large enough (size %zu KiB, "
        "minimum required is %zu KiB)",
        size / MI_KiB, mi_size_of_slices(info_slices + 1) / MI_KiB);
    return false;
  } else if (info_slices >= MI_ARENA_MAX_CHUNK_OBJ_SLICES) {
    _mi_warning_message(
        "cannot use OS memory since it is too large with respect to the "
        "maximum object size (size %zu MiB, meta-info slices %zu, maximum "
        "object slices are %zu)",
        size / MI_MiB, info_slices, MI_ARENA_MAX_CHUNK_OBJ_SLICES);
    return false;
  }
  ...
  if (!memid.initially_committed) {
    ...
    bool ok = false;
    if (commit_fun != NULL) {
      ok = (*commit_fun)(true /* commit */, arena, commit_size, NULL,
                         commit_fun_arg);
    } else {
    ...
    // todo: this does not respect the commit_fun as the memid is of external
    // memory
    _mi_os_secure_guard_page_set_before(
        (uint8_t*)arena + mi_size_of_slices(info_slices), memid);
  }
  if (!memid.initially_zero) {
  ...
  for (size_t i = 0; i < MI_ARENA_BIN_COUNT; i++) {
    arena->pages_main.pages_abandoned[i] =
        mi_arena_bitmap_init(slice_count, &base);
  }
  mi_assert_internal(mi_size_of_slices(info_slices) >=
                     (size_t)(base - mi_arena_start(arena)));

  // reserve our meta info (and reserve slices outside the memory area)
  mi_bbitmap_unsafe_setN(arena->slices_free, info_slices /* start */,
                         arena->slice_count - info_slices);
  if (memid.initially_committed) {
...
bool mi_manage_os_memory_ex(void* start, size_t size, bool is_committed,
                            bool is_pinned, bool is_zero, int numa_node,
                            bool exclusive,
                            mi_arena_id_t* arena_id) mi_attr_noexcept {
  ...
  return mi_manage_os_memory_ex2(_mi_subproc(), start, size, numa_node,
                                 exclusive, memid, NULL, NULL, arena_id);
}

bool mi_manage_memory(void* start, size_t size, bool is_committed, bool is_zero,
                      bool is_pinned, int numa_node, bool exclusive,
                      mi_commit_fun_t* commit_fun, void* commit_fun_arg,
                      mi_arena_id_t* arena_id) mi_attr_noexcept {
  mi_memid_t memid = _mi_memid_create(MI_MEM_EXTERNAL);
  memid.mem.os.base = start;
  memid.mem.os.size = size;
  ...
  memid.is_pinned = is_pinned;
  return mi_manage_os_memory_ex2(_mi_subproc(), start, size, numa_node,
                                 exclusive, memid, commit_fun, commit_fun_arg,
                                 arena_id);
}
...
static int mi_reserve_os_memory_ex2(mi_subproc_t* subproc, size_t size,
  ...
  mi_memid_t memid;
  void* start = _mi_os_alloc_aligned(size, MI_ARENA_SLICE_ALIGN, commit,
                                     allow_large, &memid);
  if (start == NULL) return ENOMEM;
  if (!mi_manage_os_memory_ex2(subproc, start, size, -1 /* numa node */,
                               exclusive, memid, NULL, NULL, arena_id)) {
  ...
  _mi_verbose_message("reserved %zu KiB memory%s\n", _mi_divide_up(size, 1024),
                      memid.is_pinned ? " (in large os pages)" : "");
...
int mi_reserve_os_memory_ex(size_t size, bool commit, bool allow_large,
                            bool exclusive,
                            mi_arena_id_t* arena_id) mi_attr_noexcept {
  return mi_reserve_os_memory_ex2(_mi_subproc(), size, commit, allow_large,
                                  exclusive, arena_id);
}
...
static size_t mi_debug_show_page_bfield(char* buf, size_t* k, mi_arena_t* arena,
                                        size_t slice_index, long* pbit_of_page,
                                        mi_ansi_color_t* pcolor_of_page) {
  size_t bit_set_count = 0;
  ...
  for (int bit = 0; bit < MI_BFIELD_BITS; bit++, bit_of_page--) {
    ...
    if (start == page) {
      ...
      if (bit_of_page > 0) {
        c = '-';
      } else if (_mi_meta_is_meta_page(start)) {
        c = 'm';
        color = MI_GRAY;
      } else if (slice_index + bit < arena->info_slices) {
        c = 'i';
        color = MI_GRAY;
      }
      // else if (mi_bitmap_is_setN(arena->pages_purge, slice_index + bit,
      // NULL)) { c = '*'; }
      else if (mi_bbitmap_is_setN(arena->slices_free, slice_index + bit, 1)) {
        if (mi_bitmap_is_set(arena->slices_purge, slice_index + bit)) {
          c = '~';
          color = MI_ORANGE;
        } else if (mi_bitmap_is_setN(arena->slices_committed, slice_index + bit,
                                     1)) {
          c = '_';
          color = MI_GRAY;
        } else {
      ...
      if (bit == MI_BFIELD_BITS - 1 && bit_of_page > 1) {
        c = '>';
      }
    }
    if (color != prev_color) {
...
static size_t mi_debug_show_chunks(const char* header1, const char* header2,
                                   const char* header3, size_t slice_count,
                                   size_t chunk_count, mi_bchunk_t* chunks,
                                   mi_bchunkmap_t* chunk_bins, bool invert,
                                   mi_arena_t* arena, bool narrow) {
  _mi_raw_message(
      "\x1B[37m%s%s%s (use/commit: \x1B[31m0 - 25%%\x1B[33m - 50%%\x1B[36m - "
      "75%%\x1B[32m - 100%%\x1B[0m)\n",
  ...
  for (size_t i = 0; i < chunk_count && bit_count < slice_count; i++) {
    ...
    for (size_t j = 0; j < MI_BCHUNK_FIELDS; j++) {
      ...
      if (bit_count < slice_count) {
        ...
        size_t xcount =
            (chunks == NULL
                 ? mi_debug_show_page_bfield(buf, &k, arena, bit_count,
                                             &bit_of_page, &color_of_page)
...
// static size_t mi_debug_show_bitmap_binned(const char* header1, const char*
// header2, const char* header3, size_t slice_count,
//                                            mi_bitmap_t* bitmap,
//                                            mi_bchunkmap_t* chunk_bins, bool
//                                            invert, mi_arena_t* arena, bool
//                                            narrow) {
//   return mi_debug_show_chunks(header1, header2, header3, slice_count,
//   mi_bitmap_chunk_count(bitmap), &bitmap->chunks[0], chunk_bins, invert,
//   arena, narrow);
// }

static void mi_debug_show_arenas_ex(mi_heap_t* heap, bool show_pages,
                                    bool narrow) mi_attr_noexcept {
  ...
  for (size_t i = 0; i < max_arenas; i++) {
    ...
    _mi_raw_message("arena %zu at %p: %zu slices (%zu MiB)%s, subproc: %p\n", i,
                    arena, arena->slice_count,
                    (size_t)(mi_size_of_slices(arena->slice_count) / MI_MiB),
                    (arena->memid.is_pinned ? ", pinned" : ""), arena->subproc);
    // if (show_inuse) {
    //   free_total += mi_debug_show_bbitmap("in-use slices",
    //   arena->slice_count, arena->slices_free, true, NULL);
    // }
    // if (show_committed) {
    //   mi_debug_show_bitmap("committed slices", arena->slice_count,
    //   arena->slices_committed, false, NULL);
    // }
    //  todo: abandoned slices
    // if (show_purge) {
    //   purge_total += mi_debug_show_bitmap("purgeable slices",
    //   arena->slice_count, arena->slices_purge, false, NULL);
    // }
    if (show_pages) {
      // mi_arena_pages_t* arena_pages = mi_heap_arena_pages(heap, arena);
      // if (arena_pages != NULL)
      {
        const char* header1 =
            "chunks (p:page, f:full, s:singleton, P,F,S:not abandoned, "
            "i:arena-info, m:meta-data, ~:free-purgable, _:free-committed, "
            ".:free-reserved)";
        const char* header2 = (narrow ? "\n       " : " ");
        const char* header3 =
            "(chunk bin: S:small, M : medium, L : large, X : other)";
        page_total += mi_debug_show_chunks(
            header1, header2, header3, arena->slice_count,
  ...
  // slice_total - free_total); if (show_abandoned) _mi_raw_message("total
  // abandoned slices: %zu\n", abandoned_total);
  if (show_pages) _mi_raw_message("total pages in arenas: %zu\n", page_total);
}

void mi_debug_show_arenas(void) mi_attr_noexcept {
  mi_debug_show_arenas_ex(mi_heap_main(), true /* show pages */,
                          true /* narrow? */);
}
...
int mi_reserve_huge_os_pages_at_ex(size_t pages, int numa_node,
  ...
  if (!mi_manage_os_memory_ex2(_mi_subproc(), p, hsize, numa_node, exclusive,
                               memid, NULL, NULL, arena_id)) {
...
// returns if the memory is no longer committed (versus reset which keeps the
// commit)
static bool mi_arena_purge(mi_arena_t* arena, size_t slice_index,
                           size_t slice_count) {
  ...
  // const bool all_committed = mi_bitmap_is_setN(arena->slices_committed,
  // slice_index, slice_count);
  size_t already_committed;
  mi_bitmap_setN(
      arena->slices_committed, slice_index, slice_count,
      &already_committed);  // pretend all committed.. (as we lack a clearN call
                            // that counts the already set bits..)
  const bool all_committed = (already_committed == slice_count);
  const bool needs_recommit =
      _mi_os_purge_ex(p, size, all_committed /* allow reset? */,
                      mi_size_of_slices(already_committed), arena->commit_fun,
                      arena->commit_fun_arg);

  if (needs_recommit) {
    // no longer committed
    mi_bitmap_clearN(arena->slices_committed, slice_index, slice_count);
    // we just counted in the purge to decommit all, but the some part was not
    // committed so adjust that here mi_subproc_stat_decrease(arena->subproc,
    // committed, mi_size_of_slices(slice_count - already_committed));
  } else if (!all_committed) {
    // we cannot assume any of these are committed any longer (even with reset
    // since we did setN and may have marked uncommitted slices as committed)
    mi_bitmap_clearN(arena->slices_committed, slice_index, slice_count);
...
static void mi_arena_schedule_purge(mi_arena_t* arena, size_t slice_index,
  ...
  if (delay == 0) {
    ...
    if (mi_atomic_casi64_strong_acq_rel(&arena->purge_expire, &expire0,
      ...
      mi_atomic_casi64_strong_acq_rel(&arena->subproc->purge_expire, &expire0,
                                      expire);
    } else {
...
static bool mi_arena_try_purge_range(mi_arena_t* arena, size_t slice_index,
  ...
  if (mi_bbitmap_try_clearNC(arena->slices_free, slice_index, slice_count)) {
    ...
    MI_UNUSED(decommitted);
    mi_assert_internal(
        !decommitted ||
        mi_bitmap_is_clearN(arena->slices_committed, slice_index, slice_count));
...
static bool mi_arena_try_purge_visitor(size_t slice_index, size_t slice_count,
                                       mi_arena_t* arena, void* arg) {
  ...
  // don't clear the purge bits as that is done atomically be the
  // _bitmap_forall_set_ranges mi_bitmap_clearN(arena->slices_purge,
  // slice_index, slice_count);
...
static bool mi_arena_try_purge(mi_arena_t* arena, mi_msecs_t now, bool force) {
  ...
  // this also clears those ranges atomically (so any newly freed blocks will
  // get purged next time around)
  mi_purge_visit_info_t vinfo = {now, mi_arena_purge_delay(), true /*all?*/,
                                 false /*any?*/};

  // we purge by at least `minslices` to not fragment transparent huge pages for
  // example
  const size_t minslices = mi_slice_count_of_size(_mi_os_minimal_purge_size());
  _mi_bitmap_forall_setc_rangesn(arena->slices_purge, minslices,
                                 &mi_arena_try_purge_visitor, arena, &vinfo);
...
static void mi_arenas_try_purge(bool force, bool visit_all,
  ...
  const mi_msecs_t arenas_expire =
      mi_atomic_loadi64_acquire(&subproc->purge_expire);
  ...
  mi_atomic_guard(&purge_guard) {
    ...
    if (arenas_expire > now) {
      mi_atomic_storei64_release(&subproc->purge_expire, now + (delay / 10));
    }
...
static bool mi_heap_visit_page_at(size_t slice_index, size_t slice_count,
                                  mi_arena_t* arena, void* arg) {
  MI_UNUSED(slice_count);
...
bool _mi_heap_visit_blocks(mi_heap_t* heap, bool abandoned_only,
  ...
  // we don't have to claim because we assume we are the only thread running
  // (with this heap). (but we could atomically claim as well by first doing
  // abandoned_reclaim and afterwards reabandoning).
    ...
    if (ok && arena_pages != NULL) {
      if (abandoned_only) {
        for (size_t bin = 0; ok && bin < MI_BIN_COUNT; bin++) {
          // todo: if we had a single abandoned page map as well, this can be
          // faster.
          if (mi_atomic_load_relaxed(&heap->abandoned_count[bin]) > 0) {
            ok = _mi_bitmap_forall_set(arena_pages->pages_abandoned[bin],
                                       &mi_heap_visit_page_at, arena,
                                       &visit_info);
          }
        }
      } else {
        ok = _mi_bitmap_forall_set(arena_pages->pages, &mi_heap_visit_page_at,
                                   arena, &visit_info);
      }
    }
  }
  ...
  // (technically we don't need the initial lock as we assume we are the only
  // thread running in this subproc)
  mi_page_t* page = NULL;
  mi_lock(&heap->os_abandoned_pages_lock) { page = heap->os_abandoned_pages; }
  while (ok && page != NULL) {
    mi_page_t* next =
        page->next;  // read upfront in case the visitor frees the page
...
bool mi_heap_visit_blocks(mi_heap_t* heap, bool visit_blocks,
                          mi_block_visit_fun* visitor, void* arg) {
  return _mi_heap_visit_blocks(heap, false, visit_blocks, visitor, arg);
}

bool mi_heap_visit_abandoned_blocks(mi_heap_t* heap, bool visit_blocks,
                                    mi_block_visit_fun* visitor, void* arg) {
...
static bool mi_heap_delete_page(const mi_heap_t* heap,
  ...
  mi_assert_internal(theap->heap == heap);
  ...
  if (mi_page_is_abandoned(page)) {
    ...
    page->next = page->prev = NULL;  // yikes.. better not to try to access this
                                     // from a thread later on..
  ...
  if (page->used == 0) {
    ...
    // destroy the page
    page->used = 0;  // note: invariant `|local_free| + |free| == reserved -
                     // used`  does not hold in this case
    _mi_arenas_page_free(page, theap);
  } else {
    ...
    mi_arena_pages_t* arena_pages_target =
        mi_heap_ensure_arena_pages(heap_target, arena);
    if mi_unlikely (arena_pages_target == NULL) {
      // if we cannot allocate this, we move it to the main heap instead (which
    ...
    mi_assert_internal(
        mi_bitmap_is_clear(arena_pages_target->pages, slice_index));
...
static void mi_heap_delete_pages(mi_heap_t* heap, mi_heap_t* heap_target) {
  mi_theap_t* const theap_target =
      (heap_target != NULL ? _mi_heap_theap(heap_target) : NULL);
  ...
  for (size_t i = 0; i < MI_ARENA_BIN_COUNT; i++) {
    mi_arena_pages_t* const arena_pages =
        mi_atomic_load_relaxed(&heap->arena_pages[i]);
    if (arena_pages != NULL) {
...
----------------------------------------------------------- */
/*
static bool mi_arena_page_register(size_t slice_index, size_t slice_count,
mi_arena_t* arena, void* arg) { MI_UNUSED(arg); MI_UNUSED(slice_count);
  mi_assert_internal(slice_count == 1);
  mi_page_t* page = (mi_page_t*)mi_arena_slice_start(arena, slice_index);
  mi_assert_internal(mi_bitmap_is_setN(page->memid.mem.arena.arena->pages,
page->memid.mem.arena.slice_index, 1)); if (!_mi_page_map_register(page)) return
false; // break mi_assert_internal(_mi_ptr_page(page)==page); return true;
}

mi_decl_nodiscard static bool mi_arena_pages_reregister(mi_arena_t* arena) {
  return _mi_bitmap_forall_set(arena->pages, &mi_arena_page_register, arena,
NULL);
}

mi_decl_export bool mi_arena_unload(mi_arena_id_t arena_id, void** base, size_t*
accessed_size, size_t* full_size) { mi_arena_t* arena =
_mi_arena_from_id(arena_id); if (arena==NULL) { return false;
  }
  else if (!arena->is_exclusive) {
    _mi_warning_message("cannot unload a non-exclusive arena (id %zu at %p)\n",
arena_id, arena); return false;
  }
  else if (arena->memid.memkind != MI_MEM_EXTERNAL) {
    _mi_warning_message("can only unload managed arena's for external memory (id
%zu at %p)\n", arena_id, arena); return false;
  }
  ...
  for (size_t bin = 0; bin < MI_BIN_COUNT; bin++) {
    ...
    if (count > 0) {
mi_atomic_decrement_acq_rel(&subproc->abandoned_count[bin]); }
  }
...
mi_decl_export bool mi_arena_reload(void* start, size_t size, mi_commit_fun_t*
commit_fun, void* commit_fun_arg, mi_arena_id_t* arena_id) {
...
arena); return false;
  ...
  if (memid.mem.os.base != start) {
    _mi_warning_message("the reloaded arena base address differs from the
external memory (arena: %p, external: %p)\n", arena, start); return false;
  }
  if (memid.mem.os.size != size) {
    _mi_warning_message("the reloaded arena size differs from the external
memory (arena size: %zu, external size: %zu)\n", arena->memid.mem.os.size,
size); return false;
  }
  ...
  for (size_t bin = 0; bin < MI_BIN_COUNT; bin++) {
    const size_t count = mi_bitmap_popcount(arena->pages_abandoned[bin]);
    if (count > 0) {
mi_atomic_decrement_acq_rel(&arena->subproc->abandoned_count[bin]); }
  }
