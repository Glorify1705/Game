...
static mi_os_mem_config_t mi_os_mem_config = {
    ...
    false,  // can we partially free allocated blocks? (on mmap systems we can
            // free anywhere in a mapped range, but on Windows we must free the
            // entire span)
    true,  // has virtual reserve? (if true we can reserve virtual address space
           // without using commit or physical memory)
    false  // has transparent huge pages? (if true we purge in (aligned) large
           // page size chunks only to not fragment such pages)
};

bool _mi_os_has_overcommit(void) { return mi_os_mem_config.has_overcommit; }
...
// OS (small) page size
size_t _mi_os_page_size(void) { return mi_os_mem_config.page_size; }

// if large OS pages are supported (2 or 4MiB), then return the size, otherwise
// return the small page size (4KiB)
size_t _mi_os_large_page_size(void) {
  return (mi_os_mem_config.large_page_size != 0
              ? mi_os_mem_config.large_page_size
              : _mi_os_page_size());
}

// minimal purge size. Can be larger than the page size if transparent huge
// pages are enabled.
...
void _mi_os_init(void) { _mi_prim_mem_init(&mi_os_mem_config); }
...
bool _mi_os_secure_guard_page_set_at(void* addr, mi_memid_t memid) {
  ...
  if (!memid.is_pinned) {
    ...
    if (arena != NULL && arena->commit_fun != NULL) {
      ok = (*(arena->commit_fun))(false /* decommit */, addr,
                                  _mi_os_secure_guard_page_size(), NULL,
  ...
  if (!ok) {
    _mi_error_message(EINVAL,
                      "secure level %d, but failed to commit guard page (at %p "
                      "of size %zu)\n",
...
bool _mi_os_secure_guard_page_set_before(void* addr, mi_memid_t memid) {
  return _mi_os_secure_guard_page_set_at(
      (uint8_t*)addr - _mi_os_secure_guard_page_size(), memid);
}
...
bool _mi_os_secure_guard_page_reset_at(void* addr, mi_memid_t memid) {
  ...
  if (!memid.is_pinned) {
    mi_arena_t* const arena = mi_memid_arena(memid);
    if (arena != NULL && arena->commit_fun != NULL) {
      return (*(arena->commit_fun))(true, addr, _mi_os_secure_guard_page_size(),
                                    NULL, arena->commit_fun_arg);
    } else {
...
bool _mi_os_secure_guard_page_reset_before(void* addr, mi_memid_t memid) {
  return _mi_os_secure_guard_page_reset_at(
      (uint8_t*)addr - _mi_os_secure_guard_page_size(), memid);
}
...
static void mi_os_prim_free(void* addr, size_t size, size_t commit_size,
  ...
  if (subproc == NULL) {
    subproc = _mi_subproc();
  }  // from `mi_arenas_unsafe_destroy` we pass subproc_main explicitly as we
     // can no longer use the theap pointer
  if (commit_size > 0) {
...
void _mi_os_free_ex(void* addr, size_t size, bool still_committed,
                    mi_memid_t memid, mi_subproc_t* subproc /* can be NULL */) {
...
// guaranteed to be aligned. Also `hint_addr` is a hint and may be ignored.
static void* mi_os_prim_alloc_at(void* hint_addr, size_t size,
                                 size_t try_alignment, bool commit,
  ...
  *is_zero = false;
  void* p = NULL;
  int err = _mi_prim_alloc(hint_addr, size, try_alignment, commit, allow_large,
                           is_large, is_zero, &p);
  if (err != 0) {
    _mi_warning_message(
        "unable to allocate OS memory (error: %d (0x%x), addr: %p, size: 0x%zx "
        "bytes, align: 0x%zx, commit: %d, allow large: %d)\n",
...
static void* mi_os_prim_alloc(size_t size, size_t try_alignment, bool commit,
                              bool allow_large, bool* is_large, bool* is_zero) {
  return mi_os_prim_alloc_at(NULL, size, try_alignment, commit, allow_large,
                             is_large, is_zero);
}

// Primitive aligned allocation from the OS.
// This function guarantees the allocated memory is aligned.
static void* mi_os_prim_alloc_aligned(size_t size, size_t alignment,
  ...
  // try a direct allocation if the alignment is below the default, or if larger
  // than 1/8 fraction of the size.
  const bool try_direct_alloc =
      (alignment <= mi_os_mem_config.alloc_granularity || alignment > size / 8);

  void* p = NULL;
  if (try_direct_alloc) {
    p = mi_os_prim_alloc(size, alignment, commit, allow_large, is_large,
                         is_zero);
  }
...
// if not aligned, free it, overallocate, and unmap around it
...
#endif
    if (p != NULL) {
      mi_os_prim_free(p, size, (commit ? size : 0), NULL);
    }
    if (size >= (SIZE_MAX - alignment)) return NULL;  // overflow
    const size_t over_size = size + alignment;

    if (!mi_os_mem_config.has_partial_free) {  // win32 virtualAlloc cannot free
                                               // parts of an allocated block
      // over-allocate uncommitted (virtual) memory
      p = mi_os_prim_alloc(over_size, 1 /*alignment*/, false /* commit? */,
                           false /* allow_large */, is_large, is_zero);
      ...
      *base = p;  // remember the base
      ...
      mi_assert_internal(pre_size < over_size && post_size < over_size &&
                         mid_size >= size);
      if (pre_size > 0) {
        mi_os_prim_free(p, pre_size, (commit ? pre_size : 0), NULL);
      }
      if (post_size > 0) {
        mi_os_prim_free((uint8_t*)aligned_p + mid_size, post_size,
                        (commit ? post_size : 0), NULL);
      }
  ...
  mi_assert_internal(p == NULL || (p != NULL && *base != NULL &&
...
void* _mi_os_alloc_aligned(size_t size, size_t alignment, bool commit,
  ...
  *memid = _mi_memid_none();
  ...
  bool os_is_zero = false;
  void* os_base = NULL;
  void* p = mi_os_prim_alloc_aligned(size, alignment, commit, allow_large,
                                     &os_is_large, &os_is_zero, &os_base);
  if (p == NULL) return NULL;
  ...
  *memid = _mi_memid_create_os(p, size, commit, os_is_zero, os_is_large);
  ...
  memid->mem.os.size +=
      ((uint8_t*)p -
       (uint8_t*)os_base);  // todo: return from prim_alloc_aligned?
...
mi_decl_nodiscard static void* mi_os_ensure_zero(void* p, size_t size,
                                                 mi_memid_t* memid) {
...
/* -----------------------------------------------------------
  ...
  page where the object can be aligned at an offset from the start of the
segment. As we may need to overallocate, we need to free such pointers using
`mi_free_aligned` to use the actual start of the memory region.
----------------------------------------------------------- */
...
void* _mi_os_alloc_aligned_at_offset(size_t size, size_t alignment,
  ...
  *memid = _mi_memid_none();
  if (offset == 0) {
    ...
    void* const start =
        _mi_os_alloc_aligned(oversize, alignment, commit, allow_large, memid);
...
static void* mi_os_page_align_areax(bool conservative, void* addr, size_t size,
  ...
  // page align conservatively within the range, or liberally straddling pages
  // outside the range
...
bool _mi_os_commit_ex(void* addr, size_t size, bool* is_zero,
                      size_t stat_size) {
  if (is_zero != NULL) {
    *is_zero = false;
  }
  ...
  size_t csize;
  void* start =
      mi_os_page_align_areax(false /* conservative? */, addr, size, &csize);
...
// note: the following seems required for asan (otherwise `mimalloc-test-stress`
// fails)
#ifdef MI_TRACK_ASAN
  if (os_is_zero) {
...
#endif
  mi_os_stat_increase(committed,
                      stat_size);  // use size for precise commit vs. decommit
  return true;
}
...
void _mi_os_reuse(void* addr, size_t size) {
  ...
  if (err != 0) {
    _mi_warning_message(
        "cannot reuse OS memory (error: %d (0x%x), address: %p, size: 0x%zx "
        "bytes)\n",
        err, err, start, csize);
  }
}
...
bool _mi_os_purge_ex(void* p, size_t size, bool allow_reset, size_t stat_size,
  ...
  } else if (mi_option_is_enabled(
                 mi_option_purge_decommits) &&  // should decommit?
             !_mi_preloading())  // don't decommit during preloading (unsafe)
  {
    ...
    if (allow_reset) {  // this can sometimes be not allowed if the range is not
                        // fully committed (on Windows, we cannot reset
                        // uncommitted memory)
      _mi_os_reset(p, size);
    }
...
static bool mi_os_protectx(void* addr, size_t size, bool protect) {
  ...
  */
  ...
  if (err != 0) {
    _mi_warning_message(
        "cannot %s OS memory (error: %d (0x%x), address: %p, size: 0x%zx "
        "bytes)\n",
...
void* _mi_os_alloc_huge_os_pages(size_t pages, int numa_node,
  ...
  *memid = _mi_memid_none();
  ...
  while (page < pages) {
    ...
    void* p = NULL;
    int err = _mi_prim_alloc_huge_os_pages(addr, MI_HUGE_OS_PAGE_SIZE,
                                           numa_node, &is_zero, &p);
    if (!is_zero) {
      all_zero = false;
    }
    if (err != 0) {
      _mi_warning_message(
          "unable to allocate huge OS page (error: %d (0x%x), address: %p, "
          "size: %zx bytes)\n",
  ...
  if (page != 0) {
    ...
    *memid = _mi_memid_create_os(start, size, true /* is committed */, all_zero,
                                 true /* is_large */);
