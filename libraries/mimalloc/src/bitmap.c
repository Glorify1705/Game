...
static inline size_t mi_bfield_ctz(mi_bfield_t x) { return mi_ctz(x); }

static inline size_t mi_bfield_clz(mi_bfield_t x) { return mi_clz(x); }
...
static inline mi_bfield_t mi_bfield_zero(void) { return 0; }

static inline mi_bfield_t mi_bfield_one(void) { return 1; }

static inline mi_bfield_t mi_bfield_all_set(void) { return ~((mi_bfield_t)0); }
...
static inline mi_bfield_t mi_bfield_mask(size_t bit_count, size_t shiftl) {
  ...
  const mi_bfield_t mask0 =
      (bit_count < MI_BFIELD_BITS ? (mi_bfield_one() << bit_count) - 1
                                  : mi_bfield_all_set());
  return (mask0 << shiftl);
}

// ------- mi_bfield_atomic_set ---------------------------------------
// the `_set` functions return also the count of bits that were already set (for
// commit statistics) the `_clear` functions return also whether the new bfield
// is all clear or not (for the chunk_map)
...
// Clear a bit but only when/once it is set. This is used by concurrent free's
// while the page is abandoned and mapped. This can incure a busy wait :-( but
// it should happen almost never (and is accounted for in the stats)
static inline void mi_bfield_atomic_clear_once_set(_Atomic(mi_bfield_t)* b,
                                                   size_t idx) {
  mi_assert_internal(idx < MI_BFIELD_BITS);
  const mi_bfield_t mask = mi_bfield_mask(1, idx);
  ;
  ...
  do {
    if mi_unlikely ((old & mask) == 0) {
      ...
      if ((old & mask) == 0) {
        mi_subproc_stat_counter_increase(_mi_subproc(),
                                         pages_unabandon_busy_wait, 1);
      }
  ...
  mi_assert_internal((old & mask) ==
                     mask);  // we should only clear when it was set
}

// Set a mask set of bits atomically, and return true of the mask bits
// transitioned from all 0's to 1's. `already_set` contains the count of bits
// that were already set (used when committing ranges to account statistics
// correctly).
static inline bool mi_bfield_atomic_set_mask(_Atomic(mi_bfield_t)* b,
                                             mi_bfield_t mask,
                                             size_t* already_set) {
  mi_assert_internal(mask != 0);
  mi_bfield_t old = mi_atomic_load_relaxed(b);
  while (!mi_atomic_cas_weak_acq_rel(b, &old, old | mask)) {
  };  // try to atomically set the mask bits until success
  if (already_set != NULL) {
...
// Clear a mask set of bits atomically, and return true of the mask bits
// transitioned from all 1's to 0's `all_clear` is set to `true` if the new
// bfield became zero.
static inline bool mi_bfield_atomic_clear_mask(_Atomic(mi_bfield_t)* b,
                                               mi_bfield_t mask,
                                               bool* all_clear) {
  mi_assert_internal(mask != 0);
  mi_bfield_t old = mi_atomic_load_relaxed(b);
  while (!mi_atomic_cas_weak_acq_rel(b, &old, old & ~mask)) {
  };  // try to atomically clear the mask bits until success
  if (all_clear != NULL) {
    *all_clear = ((old & ~mask) == 0);
  }
  return ((old & mask) == mask);
...
// static inline bool mi_bfield_atomic_clearX(_Atomic(mi_bfield_t)*b, bool*
// all_clear) {
...
// Tries to clear a mask atomically, and returns true if the mask bits
// atomically transitioned from mask to 0 and false otherwise (leaving the bit
// field as is). `all_clear` is set to `true` if the new bfield became zero.
static inline bool mi_bfield_atomic_try_clear_mask_of(_Atomic(mi_bfield_t)* b,
                                                      mi_bfield_t mask,
                                                      mi_bfield_t expect,
                                                      bool* all_clear) {
  mi_assert_internal(mask != 0);
...
static inline bool mi_bfield_atomic_try_clear_mask(_Atomic(mi_bfield_t)* b,
                                                   mi_bfield_t mask,
                                                   bool* all_clear) {
  mi_assert_internal(mask != 0);
...
// Tries to clear a bit atomically. Returns `true` if the bit transitioned from
// 1 to 0 and `false` otherwise leaving the bfield `b` as-is. `all_clear` is set
// to true if the new bfield became zero (and false otherwise)
mi_decl_maybe_unused static inline bool mi_bfield_atomic_try_clear(
    _Atomic(mi_bfield_t)* b, size_t idx, bool* all_clear) {
  mi_assert_internal(idx < MI_BFIELD_BITS);
  const mi_bfield_t mask = mi_bfield_one() << idx;
  return mi_bfield_atomic_try_clear_mask(b, mask, all_clear);
}

// Tries to clear a byte atomically, and returns true if the byte atomically
// transitioned from 0xFF to 0 `all_clear` is set to true if the new bfield
// became zero (and false otherwise)
mi_decl_maybe_unused static inline bool mi_bfield_atomic_try_clear8(
    _Atomic(mi_bfield_t)* b, size_t idx, bool* all_clear) {
  mi_assert_internal(idx < MI_BFIELD_BITS);
...
// Try to clear a full field of bits atomically, and return true all bits
// transitioned from all 1's to 0's. and false otherwise leaving the bit field
// as-is. `all_clear` is set to true if the new bfield became zero (which is
// always the case if successful).
...
static inline bool mi_bfield_atomic_is_set(const _Atomic(mi_bfield_t)* b,
                                           const size_t idx) {
...
static inline bool mi_bfield_atomic_is_clear(const _Atomic(mi_bfield_t)* b,
                                             const size_t idx) {
  const mi_bfield_t x = mi_atomic_load_relaxed(b);
  return ((x & mi_bfield_mask(1, idx)) == 0);
}
...
static inline bool mi_bfield_atomic_is_xset(mi_xset_t set,
                                            const _Atomic(mi_bfield_t)* b,
                                            const size_t idx) {
  if (set)
...
static inline bool mi_bfield_atomic_is_set_mask(const _Atomic(mi_bfield_t)* b,
                                                mi_bfield_t mask) {
  mi_assert_internal(mask != 0);
  const mi_bfield_t x = mi_atomic_load_relaxed(b);
  return ((x & mask) == mask);
...
static inline bool mi_bfield_atomic_is_clear_mask(const _Atomic(mi_bfield_t)* b,
                                                  mi_bfield_t mask) {
  mi_assert_internal(mask != 0);
  const mi_bfield_t x = mi_atomic_load_relaxed(b);
  return ((x & mask) == 0);
...
static inline bool mi_bfield_atomic_is_xset_mask(mi_xset_t set,
                                                 const _Atomic(mi_bfield_t)* b,
                                                 mi_bfield_t mask) {
  mi_assert_internal(mask != 0);
  if (set)
...
static inline size_t mi_bfield_atomic_popcount_mask(_Atomic(mi_bfield_t)* b,
                                                    mi_bfield_t mask) {
  const mi_bfield_t x = mi_atomic_load_relaxed(b);
...
// Set `0 < n <= MI_BFIELD_BITS`, and return true of the mask bits transitioned
// from all 0's to 1's. `already_set` contains the count of bits that were
// already set (used when committing ranges to account statistics correctly).
// Can cross over two bfields.
static inline bool mi_bchunk_setNX(mi_bchunk_t* chunk, size_t cidx, size_t n,
                                   size_t* already_set) {
  mi_assert_internal(cidx < MI_BCHUNK_BITS);
  ...
  if mi_likely (idx + n <= MI_BFIELD_BITS) {
    // within one field
    return mi_bfield_atomic_set_mask(&chunk->bfields[i], mi_bfield_mask(n, idx),
                                     already_set);
  } else {
    // spanning two fields
    ...
    size_t already_set1;
    const bool all_set1 = mi_bfield_atomic_set_mask(
        &chunk->bfields[i], mi_bfield_mask(m, idx), &already_set1);
    mi_assert_internal(n - m > 0);
    mi_assert_internal(n - m < MI_BFIELD_BITS);
    size_t already_set2;
    const bool all_set2 = mi_bfield_atomic_set_mask(
        &chunk->bfields[i + 1], mi_bfield_mask(n - m, 0), &already_set2);
    if (already_set != NULL) {
...
mi_decl_noinline static bool mi_bchunk_xsetNC(mi_xset_t set, mi_bchunk_t* chunk,
                                              size_t cidx, size_t n,
                                              size_t* palready_set,
                                              bool* pmaybe_all_clear) {
  ...
  while (n > 0) {
    ...
    const bool transition =
        (set ? mi_bfield_atomic_set_mask(&chunk->bfields[field], mask,
                                         &already_set)
             : mi_bfield_atomic_clear_mask(&chunk->bfields[field], mask,
                                           &all_clear));
    mi_assert_internal((transition && already_set == 0) ||
                       (!transition && already_set > 0));
...
static inline bool mi_bchunk_setN(mi_bchunk_t* chunk, size_t cidx, size_t n,
                                  size_t* already_set) {
...
static inline bool mi_bchunk_clear(mi_bchunk_t* chunk, size_t cidx,
                                   bool* all_clear) {
  mi_assert_internal(cidx < MI_BCHUNK_BITS);
  const size_t i = cidx / MI_BFIELD_BITS;
  const size_t idx = cidx % MI_BFIELD_BITS;
...
static inline bool mi_bchunk_clearN(mi_bchunk_t* chunk, size_t cidx, size_t n,
  ...
  // if (n==MI_BFIELD_BITS) return mi_bchunk_clearX(chunk, cidx,
  // maybe_all_clear);
...
mi_decl_noinline static size_t mi_bchunk_popcountNC(mi_bchunk_t* chunk,
                                                    size_t field_idx,
                                                    size_t idx, size_t n) {
...
static inline size_t mi_bchunk_popcountN(mi_bchunk_t* chunk, size_t cidx,
                                         size_t n) {
  mi_assert_internal(cidx + n <= MI_BCHUNK_BITS);
  mi_assert_internal(n > 0);
  if (n == 0) return 0;
  ...
  if (idx + n <= MI_BFIELD_BITS) {
    return mi_bfield_atomic_popcount_mask(&chunk->bfields[i],
                                          mi_bfield_mask(n, idx));
  }
...
// Check if a sequence of `n` bits within a chunk are all set/cleared.
// This can cross bfield's
mi_decl_noinline static bool mi_bchunk_is_xsetNC(mi_xset_t set,
                                                 const mi_bchunk_t* chunk,
                                                 size_t field_idx, size_t idx,
                                                 size_t n) {
  mi_assert_internal((field_idx * MI_BFIELD_BITS) + idx + n <= MI_BCHUNK_BITS);
  while (n > 0) {
    size_t m = MI_BFIELD_BITS - idx;  // m is the bits to xset in this field
...
// Check if a sequence of `n` bits within a chunk are all set/cleared.
static inline bool mi_bchunk_is_xsetN(mi_xset_t set, const mi_bchunk_t* chunk,
                                      size_t cidx, size_t n) {
  mi_assert_internal(cidx + n <= MI_BCHUNK_BITS);
  mi_assert_internal(n > 0);
  if (n == 0) return true;
  ...
  if (idx + n <= MI_BFIELD_BITS) {
    return mi_bfield_atomic_is_xset_mask(set, &chunk->bfields[i],
                                         mi_bfield_mask(n, idx));
  }
...
static inline bool mi_bchunk_try_clearNX(mi_bchunk_t* chunk, size_t cidx,
                                         size_t n, bool* pmaybe_all_clear) {
  mi_assert_internal(cidx < MI_BCHUNK_BITS);
  ...
  if mi_likely (idx + n <= MI_BFIELD_BITS) {
    // within one field
    return mi_bfield_atomic_try_clear_mask(
        &chunk->bfields[i], mi_bfield_mask(n, idx), pmaybe_all_clear);
  } else {
    ...
    if (!mi_bfield_atomic_try_clear_mask(
            &chunk->bfields[i], mi_bfield_mask(m, idx), &field1_is_clear))
      return false;
    // try the second field as well
    mi_assert_internal(n - m > 0);
    mi_assert_internal(n - m < MI_BFIELD_BITS);
    bool field2_is_clear;
    if (!mi_bfield_atomic_try_clear_mask(&chunk->bfields[i + 1],
                                         mi_bfield_mask(n - m, 0),
                                         &field2_is_clear)) {
      // we failed to clear the second field, restore the first one
      mi_bfield_atomic_set_mask(&chunk->bfields[i], mi_bfield_mask(m, idx),
                                NULL);
      return false;
    }
    if (pmaybe_all_clear != NULL) {
...
// static inline bool mi_bchunk_try_clearX(mi_bchunk_t* chunk, size_t cidx,
// bool* pmaybe_all_clear) {
//   mi_assert_internal(cidx < MI_BCHUNK_BITS);
...
// Note: this is the complex one as we need to unwind partial atomic operations
// if we fail halfway.. `maybe_all_clear` is set to `true` if all the bfields
// involved become zero.
mi_decl_noinline static bool mi_bchunk_try_clearNC(mi_bchunk_t* chunk,
                                                   size_t cidx, size_t n,
                                                   bool* pmaybe_all_clear) {
  mi_assert_internal(cidx + n <= MI_BCHUNK_BITS);
  mi_assert_internal(n > 0);
  if (pmaybe_all_clear != NULL) {
    *pmaybe_all_clear = true;
  }
  if (n == 0) return true;
  ...
  size_t m =
      MI_BFIELD_BITS - start_idx;  // m are the bits to clear in this field
  if (m > n) {
    m = n;
  }
  ...
  bool maybe_all_clear;
  if (!mi_bfield_atomic_try_clear_mask(&chunk->bfields[field], mask_start,
                                       &maybe_all_clear))
    return false;
  ...
  // continue with mid fields and last field: if these fail we need to recover
  // by unsetting previous fields mid fields?
  ...
  if (n > 0) {
    ...
    bool field_is_clear;
    if (!mi_bfield_atomic_try_clear_mask(&chunk->bfields[field], mask_end,
                                         &field_is_clear))
      goto restore;
    maybe_all_clear = maybe_all_clear && field_is_clear;
  }
...
restore:
  // `field` is the index of the field that failed to set atomically; we need to
  ...
  while (field > start_field) {
    ...
    if (field == start_field) {
      ...
      mi_bfield_atomic_setX(&chunk->bfields[field],
                            NULL);  // mid-field: set all bits again
    }
  }
  return false;
}

static inline bool mi_bchunk_try_clearN(mi_bchunk_t* chunk, size_t cidx,
                                        size_t n, bool* maybe_all_clear) {
  mi_assert_internal(n > 0);
  // if (n==MI_BFIELD_BITS) return mi_bchunk_try_clearX(chunk, cidx,
  // maybe_all_clear);
  if (n <= MI_BFIELD_BITS)
...
static inline bool mi_bchunk_try_find_and_clear_at(mi_bchunk_t* chunk,
                                                   size_t chunk_idx,
                                                   size_t* pidx) {
  mi_assert_internal(chunk_idx < MI_BCHUNK_FIELDS);
  // note: this must be acquire (and not relaxed), or otherwise the AVX code
  // below can loop forever as the compiler won't reload the registers vec1 and
  // vec2 from memory again.
  const mi_bfield_t b = mi_atomic_load_acquire(&chunk->bfields[chunk_idx]);
  size_t idx;
  if (mi_bfield_find_least_bit(b, &idx)) {  // find the least bit
    if mi_likely (mi_bfield_atomic_try_clear_mask_of(
                      &chunk->bfields[chunk_idx], mi_bfield_mask(1, idx), b,
                      NULL)) {  // clear it atomically
...
static inline bool mi_bchunk_try_find_and_clear(mi_bchunk_t* chunk,
                                                size_t* pidx) {
  ...
  for (int tries = 0; tries < 4; tries++) {  // paranoia: at most 4 tries
    ...
    const __m256i vcmp =
        _mm256_cmpeq_epi64(vec, mi_mm256_zero());  // (elem64 == 0 ? 0xFF  : 0)
    const uint32_t mask = ~_mm256_movemask_epi8(
        vcmp);  // mask of most significant bit of each byte (so each 8 bits are
                // all set or clear)
    // mask is inverted, so each 8-bits is 0xFF iff the corresponding elem64 has
    // a bit set (and thus can be cleared)
    if (mask == 0) return false;
    mi_assert_internal((_tzcnt_u32(mask) % 8) ==
                       0);  // tzcnt == 0, 8, 16, or 24
    const size_t chunk_idx = _tzcnt_u32(mask) / 8;
    if (mi_bchunk_try_find_and_clear_at(chunk, chunk_idx, pidx)) return true;
// try again
// note: there must be an atomic release/acquire in between or otherwise the
// registers may not be reloaded we add an explicit memory barrier as older gcc
// compilers do not reload the registers even with an atomic acquire (issue
// #1206)
#if defined(__GNUC__)
  ...
  for (int tries = 0; tries < 4; tries++) {  // paranoia: at most 4 tries
    ...
    const __m256i vec2 =
        _mm256_load_si256(((const __m256i*)chunk->bfields) + 1);
    const __m256i cmpv = mi_mm256_zero();
    const __m256i vcmp1 =
        _mm256_cmpeq_epi64(vec1, cmpv);  // (elem64 == 0 ? 0xFF  : 0)
    const __m256i vcmp2 =
        _mm256_cmpeq_epi64(vec2, cmpv);  // (elem64 == 0 ? 0xFF  : 0)
    const uint32_t mask1 = ~_mm256_movemask_epi8(
        vcmp1);  // mask of most significant bit of each byte (so each 8 bits
                 // are all set or clear)
    const uint32_t mask2 = ~_mm256_movemask_epi8(
        vcmp2);  // mask of most significant bit of each byte (so each 8 bits
                 // are all set or clear)
    const uint64_t mask = ((uint64_t)mask2 << 32) | mask1;
    // mask is inverted, so each 8-bits is 0xFF iff the corresponding elem64 has
    // a bit set (and thus can be cleared)
    if (mask == 0) return false;
    mi_assert_internal((_tzcnt_u64(mask) % 8) ==
                       0);  // tzcnt == 0, 8, 16, 24 , ..
...
// #1206)
  ...
  for (int tries = 0; tries < 4; tries++) {  // paranoia: at most 4 tries
    // a cache line is 64b so we can just as well load all at the same time (?)
    const uint64x2_t vzero1_lo =
        vceqzq_u64(vld1q_u64((uint64_t*)chunk->bfields));  // 2x64 bit is_zero
    const uint64x2_t vzero1_hi = vceqzq_u64(
        vld1q_u64((uint64_t*)chunk->bfields + 2));  // 2x64 bit is_zero
    const uint64x2_t vzero2_lo = vceqzq_u64(
        vld1q_u64((uint64_t*)chunk->bfields + 4));  // 2x64 bit is_zero
    const uint64x2_t vzero2_hi = vceqzq_u64(
        vld1q_u64((uint64_t*)chunk->bfields + 6));  // 2x64 bit is_zero
    const uint32x4_t vzero1 = vuzp1q_u32(
        vreinterpretq_u32_u64(vzero1_lo),
        vreinterpretq_u32_u64(
            vzero1_hi));  // unzip even elements: narrow to 4x32 bit is_zero ()
    const uint32x4_t vzero2 = vuzp1q_u32(
        vreinterpretq_u32_u64(vzero2_lo),
        vreinterpretq_u32_u64(
            vzero2_hi));  // unzip even elements: narrow to 4x32 bit is_zero ()
    const uint32x4_t vzero1x = vreinterpretq_u32_u64(
        vshrq_n_u64(vreinterpretq_u64_u32(vzero1),
                    24));  // shift-right 2x32bit elem by 24: lo 16 bits contain
                           // the 2 lo bytes
    const uint32x4_t vzero2x =
        vreinterpretq_u32_u64(vshrq_n_u64(vreinterpretq_u64_u32(vzero2), 24));
    const uint16x8_t vzero12 = vreinterpretq_u16_u32(vuzp1q_u32(
        vzero1x, vzero2x));  // unzip even 32-bit elements into one vector
    const uint8x8_t vzero = vmovn_u16(vzero12);  // narrow the bottom 16-bits
    const uint64_t mask = ~vget_lane_u64(
        vreinterpret_u64_u8(vzero),
        0);  // 1 byte for each bfield (0xFF => bfield has a bit set)
    if (mask == 0) return false;
...
static inline bool mi_bchunk_try_find_and_clear_1(mi_bchunk_t* chunk, size_t n,
                                                  size_t* pidx) {
...
mi_decl_maybe_unused static inline bool mi_bchunk_try_find_and_clear8_at(
    mi_bchunk_t* chunk, size_t chunk_idx, size_t* pidx) {
  ...
  const mi_bfield_t has_set8 =
      ((~b -
        MI_BFIELD_LO_BIT8) &     // high bit set if byte in x is 0xFF or < 0x7F
  ...
  if (mi_bfield_find_least_bit(has_set8, &idx)) {  // find least 1-bit
    ...
    mi_assert_internal((idx % 8) == 0);
    if mi_likely (mi_bfield_atomic_try_clear_mask_of(
                      &chunk->bfields[chunk_idx], (mi_bfield_t)0xFF << idx, b,
                      NULL)) {  // unset the byte atomically
      *pidx = (chunk_idx * MI_BFIELD_BITS) + idx;
...
// find least aligned byte in a chunk with all bits set, and try unset it
// atomically set `*pidx` to its bit index (0 <= *pidx < MI_BCHUNK_BITS) on
// success. Used to find medium size pages in the free blocks. todo: try neon
// version
static mi_decl_noinline bool mi_bchunk_try_find_and_clear8(mi_bchunk_t* chunk,
                                                           size_t* pidx) {
  ...
  while (true) {
    ...
    const __m256i vcmp1 =
        _mm256_cmpeq_epi8(vec1, cmpv);  // (byte == ~0 ? 0xFF : 0)
    const __m256i vcmp2 =
        _mm256_cmpeq_epi8(vec2, cmpv);  // (byte == ~0 ? 0xFF : 0)
    const uint32_t mask1 = _mm256_movemask_epi8(
        vcmp1);  // mask of most significant bit of each byte
    const uint32_t mask2 = _mm256_movemask_epi8(
        vcmp2);  // mask of most significant bit of each byte
    const uint64_t mask = ((uint64_t)mask2 << 32) | mask1;
    // mask is inverted, so each bit is 0xFF iff the corresponding byte has a
    // bit set (and thus can be cleared)
    if (mask == 0) return false;
    ...
    mi_assert_internal(chunk_idx < MI_BCHUNK_FIELDS);
    if mi_likely (mi_bfield_atomic_try_clear8(&chunk->bfields[chunk_idx], idx,
                                              NULL)) {  // clear it atomically
      *pidx = (chunk_idx * MI_BFIELD_BITS) + idx;
      mi_assert_internal(*pidx + 8 <= MI_BCHUNK_BITS);
      return true;
...
static inline bool mi_bchunk_try_find_and_clear_8(mi_bchunk_t* chunk, size_t n,
                                                  size_t* pidx) {
...
// find a sequence of `n` bits in a chunk with `0 < n <= MI_BFIELD_BITS` with
// all bits set, and try to clear them atomically. set `*pidx` to its bit index
// (0 <= *pidx <= MI_BCHUNK_BITS - n) on success. will cross bfield boundaries.
mi_decl_noinline static bool mi_bchunk_try_find_and_clearNX(mi_bchunk_t* chunk,
                                                            size_t n,
                                                            size_t* pidx) {
  ...
  for (int i = 0; i < MI_BCHUNK_FIELDS; i++) {
    ...
    while (mi_bfield_find_least_bit(b, &idx)) {  // find least 1-bit
      if (idx + n > MI_BFIELD_BITS)
        break;  // too short: maybe cross over, or continue with the next field

      const size_t bmask = mask << idx;
      mi_assert_internal(bmask >> idx == mask);
      if ((b & bmask) ==
          bmask) {  // found a match with all bits set, try clearing atomically
        if mi_likely (mi_bfield_atomic_try_clear_mask_of(&chunk->bfields[i],
                                                         bmask, b0, NULL)) {
          *pidx = (i * MI_BFIELD_BITS) + idx;
          ...
          // if we failed to atomically commit, reload b and try again from the
          // start
          b = b0 = mi_atomic_load_acquire(&chunk->bfields[i]);
        }
      } else {
        // advance by clearing the least run of ones, for example, with n>=4,
        // idx=2: b             = 1111 1101 1010 1100
    ...
    if (b != 0 && i < MI_BCHUNK_FIELDS - 1) {
      ...
      if (post > 0) {
        const size_t pre =
            mi_bfield_ctz(~mi_atomic_load_relaxed(&chunk->bfields[i + 1]));
        if (post + pre >= n) {
...
// find a sequence of `n` bits in a chunk with `n <= MI_BCHUNK_BITS` with all
// bits set, and try to clear them atomically. set `*pidx` to its bit index (0
// <= *pidx <= MI_BCHUNK_BITS - n) on success. This can cross bfield boundaries.
static mi_decl_noinline bool mi_bchunk_try_find_and_clearNC(mi_bchunk_t* chunk,
                                                            size_t n,
                                                            size_t* pidx) {
  if (n == 0 || n > MI_BCHUNK_BITS)
    return false;  // cannot be more than a chunk

  // we first scan ahead to see if there is a range of `n` set bits, and only
  // then try to clear atomically
  mi_assert_internal(n > 0);
  ...
  for (size_t i = 0; i < MI_BCHUNK_FIELDS - skip_count; i++) {
    ...
    if (ones >= m) {
      ...
      while (i + j < MI_BCHUNK_FIELDS) {
        ...
        if (ones >= m) {
          ...
          i = i + j - 1;  // no need to re-scan previous fields, except the last
                          // one (with clz this time)
          mi_assert_internal(m > 0);
          break;
        }
      }
    ...
    if (m == 0) {
      ...
      // note: if we fail for a small `n` on the first field, we don't rescan
      // that field (as `i` is incremented)
    }
...
static bool mi_bitmap_chunkmap_try_clear(mi_bitmap_t* bitmap,
                                         size_t chunk_idx) {
  mi_assert(chunk_idx < mi_bitmap_chunk_count(bitmap));
  ...
  // .. but a concurrent set may have happened in between our all-clear test and
  // the clearing of the bit in the mask. We check again to catch this
  // situation.
  if (!mi_bchunk_all_are_clear_relaxed(&bitmap->chunks[chunk_idx])) {
    mi_bchunk_set(&bitmap->chunkmap, chunk_idx, NULL);
    return false;
...
size_t mi_bitmap_size(size_t bit_count, size_t* pchunk_count) {
  ...
  const size_t size =
      offsetof(mi_bitmap_t, chunks) + (chunk_count * MI_BCHUNK_SIZE);
  mi_assert_internal((size % MI_BCHUNK_SIZE) == 0);
  if (pchunk_count != NULL) {
...
size_t mi_bitmap_init(mi_bitmap_t* bitmap, size_t bit_count,
                      bool already_zero) {
  size_t chunk_count;
  ...
  mi_assert_internal(mi_atomic_load_relaxed(&bitmap->chunk_count) <=
                     MI_BITMAP_MAX_CHUNK_COUNT);
  return size;
}

// Set a sequence of `n` bits in the bitmap (and can cross chunks). Not atomic
// so only use if local to a thread.
static void mi_bchunks_unsafe_setN(mi_bchunk_t* chunks, mi_bchunkmap_t* cmap,
                                   size_t idx, size_t n) {
  mi_assert_internal(n > 0);
...
// Set a sequence of `n` bits in the bitmap; returns `true` if atomically
// transitioned from 0's to 1's (or 1's to 0's).
bool mi_bitmap_setN(mi_bitmap_t* bitmap, size_t idx, size_t n,
                    size_t* palready_set) {
  mi_assert_internal(n > 0);
  ...
  while (n > 0) {
    ...
    were_allclear =
        mi_bchunk_setN(&bitmap->chunks[chunk_idx], cidx, m, &_already_set) &&
        were_allclear;
  ...
  if (palready_set != NULL) {
    *palready_set = already_set;
  }
  return were_allclear;
}

// Clear a sequence of `n` bits in the bitmap; returns `true` if atomically
// transitioned from 1's to 0's.
bool mi_bitmap_clearN(mi_bitmap_t* bitmap, size_t idx, size_t n) {
  ...
  while (n > 0) {
    ...
    were_allset = mi_bchunk_clearN(&bitmap->chunks[chunk_idx], cidx, m,
                                   &maybe_all_clear) &&
                  were_allset;
    if (maybe_all_clear) {
...
// Set/clear a bit in the bitmap; returns `true` if atomically transitioned from
// 0 to 1 (or 1 to 0)
...
bool mi_bitmap_is_xsetN(mi_xset_t set, mi_bitmap_t* bitmap, size_t idx,
                        size_t n) {
  mi_assert_internal(n > 0);
  const size_t maxbits = mi_bitmap_max_bits(bitmap);
  mi_assert_internal(idx + n <= maxbits);
...
bool mi_bitmap_is_all_clear(mi_bitmap_t* bitmap) {
  return mi_bitmap_is_xsetN(MI_BIT_CLEAR, bitmap, 0,
                            mi_bitmap_max_bits(bitmap));
}
...
// - We use `popcount` to improve branch prediction (maybe not needed? can we
// simplify?)
...
#define mi_bfield_iterate(bfield, start, cycle, name_idx, SUF)                 \
  {                                                                            \
    ...
    mi_bfield_t _b##SUF =                                                      \
        bfield & _cycle_mask##SUF; /* process [start, cycle> first*/           \
    while (_bcount##SUF > 0) {                                                 \
      _bcount##SUF--;                                                          \
      if (_b##SUF == 0) {                                                      \
        _b##SUF = bfield & ~_cycle_mask##SUF;                                  \
      } /* process [0,start> + [cycle, MI_BFIELD_BITS> next */                 \
      /* size_t name_idx; */                                                   \
      const bool _found##SUF = mi_bfield_find_least_bit(_b##SUF, &name_idx);   \
      _b##SUF = mi_bfield_clear_least_bit(                                     \
          _b##SUF); /* clear early so `continue` works */                      \
      mi_assert_internal(_found##SUF);                                         \
      MI_UNUSED(_found##SUF);                                                  \
      {
...
#define mi_bfield_cycle_iterate(bfield, tseq, cycle, name_idx, SUF)     \
  {                                                                     \
    const size_t _start##SUF =                                          \
        (uint32_t)(tseq) %                                              \
        (uint32_t)(cycle); /* or: 0 to always search from the start? */ \
...
typedef bool(mi_bitmap_visit_fun_t)(mi_bitmap_t* bitmap, size_t chunk_idx,
                                    size_t n, size_t* idx, void* arg1,
                                    void* arg2);

// Go through the bitmap and for every sequence of `n` set bits, call the
// visitor function. If it returns `true` stop the search.
static inline bool mi_bitmap_find(mi_bitmap_t* bitmap, size_t tseq, size_t n,
                                  size_t* pidx, mi_bitmap_visit_fun_t* on_find,
                                  void* arg1, void* arg2) {
  const size_t chunkmap_max =
      _mi_divide_up(mi_bitmap_chunk_count(bitmap), MI_BFIELD_BITS);
  for (size_t i = 0; i < chunkmap_max; i++) {
    // and for each chunkmap entry we iterate over its bits to find the chunks
    const mi_bfield_t cmap_entry =
        mi_atomic_load_relaxed(&bitmap->chunkmap.bfields[i]);
    size_t hi;
    if (mi_bfield_find_highest_bit(cmap_entry, &hi)) {
      size_t eidx = 0;
      mi_bfield_cycle_iterate(cmap_entry, tseq % 8, hi + 1, eidx,
                              Y)  // reduce the tseq to 8 bins to reduce using
                                  // extra memory (see `mstress`)
      {
...
static bool mi_bitmap_try_find_and_claim_visit(mi_bitmap_t* bitmap,
  ...
  size_t cidx;
  if mi_likely (mi_bchunk_try_find_and_clear(&bitmap->chunks[chunk_idx],
                                             &cidx)) {
    ...
    if ((*claim_fun)(slice_index, claim_data->arena, &keep_set)) {
      ...
      *pidx = slice_index;
      ...
      // failed to claim it, set abandoned mapping again (unless the page was
      // freed)
      if (keep_set) {
        const bool wasclear =
            mi_bchunk_set(&bitmap->chunks[chunk_idx], cidx, NULL);
        mi_assert_internal(wasclear);
        MI_UNUSED(wasclear);
      }
    }
  } else {
    // we may find that all are cleared only on a second iteration but that is
    // ok as the chunkmap is a conservative approximation.
    mi_bitmap_chunkmap_try_clear(bitmap, chunk_idx);
  }
  return false;
}
...
mi_decl_nodiscard bool mi_bitmap_try_find_and_claim(mi_bitmap_t* bitmap,
                                                    size_t tseq, size_t* pidx,
                                                    mi_claim_fun_t* claim,
                                                    mi_arena_t* arena) {
  mi_claim_fun_data_t claim_data = {arena};
  return mi_bitmap_find(bitmap, tseq, 1, pidx,
                        &mi_bitmap_try_find_and_claim_visit, (void*)claim,
                        &claim_data);
}
...
// todo: optimize further? maybe use avx512 to directly get all indices using a
// mask_compressstore?
bool _mi_bitmap_forall_set(mi_bitmap_t* bitmap, mi_forall_set_fun_t* visit,
    ...
    while (mi_bfield_foreach_bit(&cmap_entry, &cmap_idx)) {
      ...
      for (size_t j = 0; j < MI_BCHUNK_FIELDS; j++) {
        const size_t base_idx =
            (chunk_idx * MI_BCHUNK_BITS) + (j * MI_BFIELD_BITS);
...
// Visit all set bits in a bitmap but try to return ranges (within bfields) if
// possible. Also clear those ranges atomically. Used by purging to purge larger
// ranges when possible todo: optimize further? maybe use avx512 to directly get
// all indices using a mask_compressstore?
bool _mi_bitmap_forall_setc_ranges(mi_bitmap_t* bitmap,
                                   mi_forall_set_fun_t* visit,
                                   mi_arena_t* arena, void* arg) {
  // for all chunkmap entries
  const size_t chunkmap_max =
      _mi_divide_up(mi_bitmap_chunk_count(bitmap), MI_BFIELD_BITS);
    ...
    while (mi_bfield_foreach_bit(&cmap_entry, &cmap_idx)) {
      ...
      for (size_t j = 0; j < MI_BCHUNK_FIELDS; j++) {
        ...
        while (mi_bfield_find_least_bit(b, &bidx)) {
          ...
          mi_assert_internal((idx / MI_BCHUNK_BITS) <
                             mi_bitmap_chunk_count(bitmap));
...
// Visit all set bits in a bitmap but try to return ranges (within bfields) if
// possible, but only in chunks of at least `rngslices` slices (that are also
// aligned at `rngslices`) and clear those ranges atomically. However, the
// `rngslices` are capped at `MI_BFIELD_BITS` at most. Used by purging to purge
// larger ranges when possible. With transparent huge pages we only want to
// purge whole huge pages (2 MiB) at a time which is what the `rngslices`
// parameter achieves.
bool _mi_bitmap_forall_setc_rangesn(mi_bitmap_t* bitmap, size_t rngslices,
                                    mi_forall_set_fun_t* visit,
                                    mi_arena_t* arena, void* arg) {
  // use the generic routine for `rngslices<=1` (as that one finds longest
  // ranges at a time)
  if (rngslices <= 1) {
    return _mi_bitmap_forall_setc_ranges(bitmap, visit, arena, arg);
  }
  // mi_assert_internal(rngslices <= MI_BFIELD_BITS);
  if (rngslices > MI_BFIELD_BITS) {
    rngslices = MI_BFIELD_BITS;
  }  // cap at MI_BFIELD_BITS at most
    ...
    while (mi_bfield_foreach_bit(&cmap_entry, &cmap_idx)) {
      ...
      for (size_t j = 0; j < MI_BCHUNK_FIELDS; j++) {
        const size_t base_idx =
            (chunk_idx * MI_BCHUNK_BITS) + (j * MI_BFIELD_BITS);
        mi_bfield_t b =
            mi_atomic_exchange_relaxed(&chunk->bfields[j], 0);  // atomic clear
        mi_bfield_t skipped =
            0;  // but track which bits we skip so we can restore them
        for (size_t shift = 0; rngslices + shift <= MI_BFIELD_BITS;
             shift += rngslices) {  // per `rngslices` to keep alignment
...
size_t mi_bbitmap_size(size_t bit_count, size_t* pchunk_count) {
  ...
  mi_assert_internal(chunk_count >= 1);
  const size_t size =
      offsetof(mi_bbitmap_t, chunks) + (chunk_count * MI_BCHUNK_SIZE);
  mi_assert_internal((size % MI_BCHUNK_SIZE) == 0);
  if (pchunk_count != NULL) {
    *pchunk_count = chunk_count;
  }
...
// initialize a bitmap to all unset; avoid a mem_zero if `already_zero` is true
// returns the size of the bitmap
size_t mi_bbitmap_init(mi_bbitmap_t* bbitmap, size_t bit_count,
                       bool already_zero) {
  size_t chunk_count;
  ...
  mi_assert_internal(mi_atomic_load_relaxed(&bbitmap->chunk_count) <=
                     MI_BITMAP_MAX_CHUNK_COUNT);
  return size;
}
...
static void mi_bbitmap_set_chunk_bin(mi_bbitmap_t* bbitmap, size_t chunk_idx,
                                     mi_chunkbin_t bin) {
  ...
  for (mi_chunkbin_t ibin = MI_CBIN_SMALL; ibin < MI_CBIN_NONE;
    ...
    if (ibin == bin) {
      ...
      if (was_clear) {
        mi_os_stat_increase(chunk_bins[ibin], 1);
      }
    } else {
      const bool was_set =
          mi_bchunk_clear(&bbitmap->chunkmap_bins[ibin], chunk_idx, NULL);
      if (was_set) {
        mi_os_stat_decrease(chunk_bins[ibin], 1);
      }
    }
  }
}

mi_chunkbin_t mi_bbitmap_debug_get_bin(const mi_bchunkmap_t* chunkmap_bins,
                                       size_t chunk_idx) {
  for (mi_chunkbin_t ibin = MI_CBIN_SMALL; ibin < MI_CBIN_NONE;
       ibin = mi_chunkbin_inc(ibin)) {
...
static void mi_bbitmap_chunkmap_set_max(mi_bbitmap_t* bbitmap,
                                        size_t chunk_idx) {
  size_t oldmax = mi_atomic_load_relaxed(&bbitmap->chunk_max_accessed);
  if mi_unlikely (chunk_idx > oldmax) {
    mi_atomic_cas_strong_relaxed(&bbitmap->chunk_max_accessed, &oldmax,
                                 chunk_idx);
  }
}

// Set a bit in the chunkmap
static void mi_bbitmap_chunkmap_set(mi_bbitmap_t* bbitmap, size_t chunk_idx,
                                    bool check_all_set) {
...
static bool mi_bbitmap_chunkmap_try_clear(mi_bbitmap_t* bbitmap,
                                          size_t chunk_idx) {
  mi_assert(chunk_idx < mi_bbitmap_chunk_count(bbitmap));
  // check if the corresponding chunk is all clear
  ...
  // .. but a concurrent set may have happened in between our all-clear test and
  // the clearing of the bit in the mask. We check again to catch this
  // situation. (note: mi_bchunk_clear must be acq-rel)
  if (!mi_bchunk_all_are_clear_relaxed(&bbitmap->chunks[chunk_idx])) {
    mi_bchunk_set(&bbitmap->chunkmap, chunk_idx, NULL);
    return false;
  }
...
bool mi_bbitmap_setN(mi_bbitmap_t* bbitmap, size_t idx, size_t n) {
  ...
  while (n > 0) {
    const size_t m = (cidx + n > MI_BCHUNK_BITS ? MI_BCHUNK_BITS - cidx : n);
    were_allclear =
        mi_bchunk_setN(&bbitmap->chunks[chunk_idx], cidx, m, NULL) &&
        were_allclear;
...
bool mi_bbitmap_try_clearNC(mi_bbitmap_t* bbitmap, size_t idx, size_t n) {
  ...
  const size_t chunk_idx = idx / MI_BCHUNK_BITS;
  const size_t cidx = idx % MI_BCHUNK_BITS;
  mi_assert_internal(cidx + n <=
                     MI_BCHUNK_BITS);  // don't cross chunks (for now)
  mi_assert_internal(chunk_idx < mi_bbitmap_chunk_count(bbitmap));
  if (cidx + n > MI_BCHUNK_BITS) return false;
  bool maybe_all_clear = false;
  const bool cleared = mi_bchunk_try_clearN(&bbitmap->chunks[chunk_idx], cidx,
                                            n, &maybe_all_clear);
  if (cleared && maybe_all_clear) {
    mi_bbitmap_chunkmap_try_clear(bbitmap, chunk_idx);
  }
  // note: we don't set the size class for an explicit try_clearN (only used by
  // purging)
  return cleared;
}
...
// Is a sequence of n bits already all set/cleared?
bool mi_bbitmap_is_xsetN(mi_xset_t set, mi_bbitmap_t* bbitmap, size_t idx,
                         size_t n) {
  mi_assert_internal(n > 0);
  const size_t maxbits = mi_bbitmap_max_bits(bbitmap);
  mi_assert_internal(idx + n <= maxbits);
  ...
  while (n > 0 && xset) {
    const size_t m = (cidx + n > MI_BCHUNK_BITS ? MI_BCHUNK_BITS - cidx : n);
    xset =
        mi_bchunk_is_xsetN(set, &bbitmap->chunks[chunk_idx], cidx, m) && xset;
    mi_assert_internal(m <= n);
    n -= m;
    cidx = 0;
    chunk_idx++;
...
typedef bool(mi_bchunk_try_find_and_clear_fun_t)(mi_bchunk_t* chunk, size_t n,
                                                 size_t* idx);

// Go through the bbitmap and for every sequence of `n` set bits, call the
// visitor function. If it returns `true` stop the search.
//
// This is used for finding free blocks and it is important to be efficient
// (with 2-level bitscan) but also reduce fragmentation (through size bins).
static inline bool mi_bbitmap_try_find_and_clear_generic(
    mi_bbitmap_t* bbitmap, size_t tseq, size_t n, size_t* pidx,
    mi_bchunk_try_find_and_clear_fun_t* on_find) {
  // we space out threads to reduce contention
  const size_t cmap_max_count =
      _mi_divide_up(mi_bbitmap_chunk_count(bbitmap), MI_BFIELD_BITS);
  ...
  mi_bfield_cycle_iterate(cmap_mask, tseq, cmap_cycle, cmap_idx, X) {
    // and for each chunkmap entry we iterate over its bits to find the chunks
    const mi_bfield_t cmap_entry =
        mi_atomic_load_relaxed(&bbitmap->chunkmap.bfields[cmap_idx]);
    const size_t cmap_entry_cycle =
        (cmap_idx != cmap_acc ? MI_BFIELD_BITS : cmap_acc_bits);
    if (cmap_entry == 0) {
      continue;
    }
    ...
    for (mi_chunkbin_t ibin = MI_CBIN_SMALL; ibin < MI_CBIN_NONE;
         ibin = mi_chunkbin_inc(ibin)) {
      const mi_bfield_t cmap_bin = mi_atomic_load_relaxed(
          &bbitmap->chunkmap_bins[ibin].bfields[cmap_idx]);
      cmap_bins[ibin] = cmap_bin & cmap_entry;
      cmap_bins[MI_CBIN_NONE] &=
          ~cmap_bin;  // clear bits that are in an assigned size bin
    }

    // consider only chunks for a particular size bin at a time
    // this picks the best bin only within a cmap entry (~ 1GiB address space),
    // but avoids multiple iterations through all entries.
    mi_assert_internal(bbin < MI_CBIN_NONE);
    for (mi_chunkbin_t ibin = MI_CBIN_SMALL; ibin <= MI_CBIN_NONE;
         // skip from bbin to NONE (so, say, a SMALL will never be placed in a
         // OTHER, MEDIUM, or LARGE chunk to reduce fragmentation)
      ...
      mi_bfield_cycle_iterate(cmap_bin, tseq, cmap_entry_cycle, eidx, Y) {
        ...
        if ((*on_find)(chunk, n, &cidx)) {
          if (cidx == 0 && ibin == MI_CBIN_NONE) {  // only the first block
                                                    // determines the size bin
          ...
          *pidx = (chunk_idx * MI_BCHUNK_BITS) + cidx;
          ...
          // todo: should _on_find_ return a boolean if there is a chance all
          // are clear to avoid calling `try_clear?` we may find that all are
          // cleared only on a second iteration but that is ok as the chunkmap
...
bool mi_bbitmap_try_find_and_clear(mi_bbitmap_t* bbitmap, size_t tseq,
                                   size_t* pidx) {
  return mi_bbitmap_try_find_and_clear_generic(bbitmap, tseq, 1, pidx,
                                               &mi_bchunk_try_find_and_clear_1);
}

bool mi_bbitmap_try_find_and_clear8(mi_bbitmap_t* bbitmap, size_t tseq,
                                    size_t* pidx) {
  return mi_bbitmap_try_find_and_clear_generic(bbitmap, tseq, 8, pidx,
                                               &mi_bchunk_try_find_and_clear_8);
}

// bool mi_bbitmap_try_find_and_clearX(mi_bbitmap_t* bbitmap, size_t tseq,
// size_t* pidx) {
//   return mi_bbitmap_try_find_and_clear_generic(bbitmap, tseq, MI_BFIELD_BITS,
//   pidx, &mi_bchunk_try_find_and_clear_X);
// }

bool mi_bbitmap_try_find_and_clearNX(mi_bbitmap_t* bbitmap, size_t tseq,
                                     size_t n, size_t* pidx) {
  mi_assert_internal(n <= MI_BFIELD_BITS);
  return mi_bbitmap_try_find_and_clear_generic(bbitmap, tseq, n, pidx,
                                               &mi_bchunk_try_find_and_clearNX);
}

bool mi_bbitmap_try_find_and_clearNC(mi_bbitmap_t* bbitmap, size_t tseq,
                                     size_t n, size_t* pidx) {
  mi_assert_internal(n <= MI_BCHUNK_BITS);
  return mi_bbitmap_try_find_and_clear_generic(bbitmap, tseq, n, pidx,
                                               &mi_bchunk_try_find_and_clearNC);
}
...
// Try to atomically clear `n` bits starting at `chunk_idx` where `n` can span
// over multiple chunks
static bool mi_bchunk_try_clearN_(mi_bbitmap_t* bbitmap, size_t chunk_idx,
                                  size_t n) {
  mi_assert_internal((chunk_idx * MI_BCHUNK_BITS) + n <=
                     mi_bbitmap_max_bits(bbitmap));

  size_t m = n;      // bits to go
  size_t count = 0;  // chunk count
  while (m > 0) {
    mi_bchunk_t* chunk = &bbitmap->chunks[chunk_idx + count];
    if (!mi_bchunk_try_clearN(
            chunk, 0, (m > MI_BCHUNK_BITS ? MI_BCHUNK_BITS : m), NULL)) {
      goto rollback;
    }
...
// Go through the bbitmap to find a sequence of `n` bits and clear them
// atomically where `n > MI_ARENA_MAX_CHUNK_OBJ_SIZE` Since these are very large
// object allocations we always search from the start and only consider starting
// at the start of a chunk (for fragmentation and efficiency). Todo: for now we
// try to find full empty chunks to cover `n` but we can allow a partial chunk
// at the end Todo: This scans directly through the chunks -- we might want to
// consult the cmap as well?
bool mi_bbitmap_try_find_and_clearN_(mi_bbitmap_t* bbitmap, size_t tseq,
                                     size_t n, size_t* pidx) {
  MI_UNUSED(tseq);
  mi_assert(n > 0);
  if (n == 0) {
  ...
  const size_t chunk_req =
      _mi_divide_up(n, MI_BCHUNK_BITS);  // minimal number of chunks needed
  if (chunk_max < chunk_req) {
    return false;
  }
