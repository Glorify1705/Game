...
size_t _mi_bin(size_t size) { return mi_bin(size); }
...
#if (MI_DEBUG > 1)
static bool mi_theap_contains_queue(const mi_theap_t* theap,
...
bool _mi_page_queue_is_valid(mi_theap_t* theap, const mi_page_queue_t* pq) {
  ...
  size_t count = 0;
  MI_UNUSED_RELEASE(count);
  mi_page_t* prev = NULL;
  MI_UNUSED_RELEASE(prev);
  for (mi_page_t* page = pq->first; page != NULL; page = page->next) {
    mi_assert_internal(page->prev == prev);
    if (mi_page_is_in_full(page)) {
      mi_assert_internal(_mi_wsize_from_size(pq->block_size) ==
                         MI_LARGE_MAX_OBJ_WSIZE + 2);
    } else if (mi_page_is_huge(page)) {
      mi_assert_internal(_mi_wsize_from_size(pq->block_size) ==
                         MI_LARGE_MAX_OBJ_WSIZE + 1);
    } else {
...
static size_t mi_page_bin(const mi_page_t* page) {
  const size_t bin =
      (mi_page_is_in_full(page)
           ? MI_BIN_FULL
           : (mi_page_is_huge(page) ? MI_BIN_HUGE
                                    : mi_bin(mi_page_block_size(page))));
...
size_t _mi_page_stats_bin(const mi_page_t* page) {
  const size_t bin =
      (mi_page_is_huge(page) ? MI_BIN_HUGE : mi_bin(mi_page_block_size(page)));
  mi_assert_internal(bin <= MI_BIN_HUGE);
  return bin;
}

static mi_page_queue_t* mi_theap_page_queue_of(mi_theap_t* theap,
                                               const mi_page_t* page) {
...
static inline void mi_theap_queue_first_update(mi_theap_t* theap,
                                               const mi_page_queue_t* pq) {
...
static void mi_page_queue_push(mi_theap_t* theap, mi_page_queue_t* queue,
                               mi_page_t* page) {
...
static void mi_page_queue_push_at_end(mi_theap_t* theap, mi_page_queue_t* queue,
                                      mi_page_t* page) {
  mi_assert_internal(mi_page_theap(page) == theap);
  mi_assert_internal(!mi_page_queue_contains(queue, page));
...
static void mi_page_queue_move_to_front(mi_theap_t* theap,
...
static void mi_page_queue_enqueue_from_ex(mi_page_queue_t* to,
...
static void mi_page_queue_enqueue_from_full(mi_page_queue_t* to,
                                            mi_page_queue_t* from,
                                            mi_page_t* page) {
  // note: we could insert at the front to increase reuse, but it slows down
  // certain benchmarks (like `alloc-test`)
  mi_page_queue_enqueue_from_ex(
      to, from, true /* enqueue at the end of the `to` queue? */, page);
}
