...
struct mi_init_done_t {
  mi_init_done_t() { _mi_auto_process_init(); }
  ~mi_init_done_t() { _mi_auto_process_done(); }
};
static mi_init_done_t mi_init_done;
#else
#pragma message( \
    "define a way to call _mi_auto_process_init/done on your platform")
#endif
#endif

// Generic allocator init/done callback
#ifndef MI_PRIM_HAS_ALLOCATOR_INIT
bool _mi_is_redirected(void) { return false; }
