...
// We define a minimal MEM_EXTENDED_PARAMETER ourselves in order to be able to
...
typedef PVOID(__stdcall* PVirtualAlloc2)(HANDLE, PVOID, SIZE_T, ULONG, ULONG,
                                         MI_MEM_EXTENDED_PARAMETER*, ULONG);
typedef LONG(__stdcall* PNtAllocateVirtualMemoryEx)(
    HANDLE, PVOID*, SIZE_T*, ULONG, ULONG, MI_MEM_EXTENDED_PARAMETER*,
    ULONG);  // avoid NTSTATUS as it is not defined on xbox (pr #1084)
static PVirtualAlloc2 pVirtualAlloc2 = NULL;
static PNtAllocateVirtualMemoryEx pNtAllocateVirtualMemoryEx = NULL;

// Similarly, GetNumaProcessorNodeEx is only supported since Windows 7  (and
// GetNumaNodeProcessorMask is not supported on xbox)
...
typedef VOID(__stdcall* PGetCurrentProcessorNumberEx)(
    MI_PROCESSOR_NUMBER* ProcNumber);
typedef BOOL(__stdcall* PGetNumaProcessorNodeEx)(MI_PROCESSOR_NUMBER* Processor,
                                                 PUSHORT NodeNumber);
typedef BOOL(__stdcall* PGetNumaNodeProcessorMaskEx)(
    USHORT Node, PGROUP_AFFINITY ProcessorMask);
typedef BOOL(__stdcall* PGetNumaProcessorNode)(UCHAR Processor,
                                               PUCHAR NodeNumber);
typedef BOOL(__stdcall* PGetNumaNodeProcessorMask)(UCHAR Node,
...
typedef BOOL(__stdcall* PGetPhysicallyInstalledSystemMemory)(
...
static bool win_enable_large_os_pages(size_t* large_page_size) {
  ...
  if (pGetLargePageMinimum == NULL)
    return false;  // no large page support (xbox etc.)
...
void _mi_prim_mem_init(mi_os_mem_config_t* config) {
  ...
  if ((uintptr_t)si.lpMaximumApplicationAddress > 0) {
    const size_t vbits =
        MI_SIZE_BITS - mi_clz((uintptr_t)si.lpMaximumApplicationAddress);
  ...
  if (hDll != NULL) {
    pNtAllocateVirtualMemoryEx =
        (PNtAllocateVirtualMemoryEx)(void (*)(void))GetProcAddress(
            hDll, "NtAllocateVirtualMemoryEx");
    FreeLibrary(hDll);
  }
  ...
  if (hDll != NULL) {
    pGetCurrentProcessorNumberEx =
        (PGetCurrentProcessorNumberEx)(void (*)(void))GetProcAddress(
            hDll, "GetCurrentProcessorNumberEx");
    pGetNumaProcessorNodeEx =
        (PGetNumaProcessorNodeEx)(void (*)(void))GetProcAddress(
            hDll, "GetNumaProcessorNodeEx");
    pGetNumaNodeProcessorMaskEx =
        (PGetNumaNodeProcessorMaskEx)(void (*)(void))GetProcAddress(
            hDll, "GetNumaNodeProcessorMaskEx");
    pGetNumaProcessorNode =
        (PGetNumaProcessorNode)(void (*)(void))GetProcAddress(
            hDll, "GetNumaProcessorNode");
    pGetNumaNodeProcessorMask =
        (PGetNumaNodeProcessorMask)(void (*)(void))GetProcAddress(
            hDll, "GetNumaNodeProcessorMask");
    pGetNumaHighestNodeNumber =
        (PGetNumaHighestNodeNumber)(void (*)(void))GetProcAddress(
            hDll, "GetNumaHighestNodeNumber");
    pGetLargePageMinimum = (PGetLargePageMinimum)(void (*)(void))GetProcAddress(
        hDll, "GetLargePageMinimum");
    // Get physical memory (not available on XP, so check dynamically)
    PGetPhysicallyInstalledSystemMemory pGetPhysicallyInstalledSystemMemory =
        (PGetPhysicallyInstalledSystemMemory)(void (*)(void))GetProcAddress(
    ...
    // Get Windows version
    PGetVersionExW pGetVersionExW =
        (PGetVersionExW)(void (*)(void))GetProcAddress(hDll, "GetVersionExW");
    if (pGetVersionExW != NULL) {
      OSVERSIONINFOW version;
      _mi_memzero_var(version);
...
int _mi_prim_free(void* addr, size_t size) {
  ...
  if (errcode == ERROR_INVALID_ADDRESS) {
    ...
    // inside the memory region returned by VirtualAlloc; in that case we need
    // to free using the start of the region.
...
static void* win_virtual_alloc_prim_once(void* addr, size_t size,
  ...
  if (addr == NULL && try_alignment > win_allocation_granularity &&
      (try_alignment % _mi_os_page_size()) == 0 && pVirtualAlloc2 != NULL) {
    MI_MEM_ADDRESS_REQUIREMENTS reqs = {0, 0, 0};
    reqs.Alignment = try_alignment;
    MI_MEM_EXTENDED_PARAMETER param = {{0, 0}, {0}};
    param.Type.Type = MiMemExtendedParameterAddressRequirements;
    param.Arg.Pointer = &reqs;
    void* p = (*pVirtualAlloc2)(GetCurrentProcess(), addr, size, flags,
                                PAGE_READWRITE, &param, 1);
    if (p != NULL) return p;
    _mi_warning_message(
        "unable to allocate aligned OS memory (0x%zx bytes, error code: 0x%x, "
        "address: %p, alignment: 0x%zx, flags: 0x%x)\n",
...
static void* win_virtual_alloc_prim(void* addr, size_t size,
                                    size_t try_alignment, DWORD flags) {
  long max_retry_msecs = mi_option_get_clamp(mi_option_retry_on_oom, 0,
                                             2000);  // at most 2 seconds
  if (max_retry_msecs == 1) {
    max_retry_msecs = 100;
  }  // if one sets the option to "true"
  for (long tries = 1; tries <= 10;
       tries++) {  // try at most 10 times (=2200ms)
    ...
    if (p != NULL) {
      ...
      _mi_warning_message(
          "out-of-memory on OS allocation, try again... (attempt %lu, 0x%zx "
          "bytes, error code: 0x%x, address: %p, alignment: 0x%zx, flags: "
          "0x%x)\n",
          tries, size, GetLastError(), addr, try_alignment, flags);
      long sleep_msecs = tries * 40;  // increasing waits
      if (sleep_msecs > max_retry_msecs) {
        sleep_msecs = max_retry_msecs;
      }
...
static void* win_virtual_alloc(void* addr, size_t size, size_t try_alignment,
                               DWORD flags, bool large_only, bool allow_large,
                               bool* is_large) {
  mi_assert_internal(!(large_only && !allow_large));
  static _Atomic(size_t) large_page_try_ok;  // = 0;
  void* p = NULL;
  // Try to allocate large OS pages (2MiB) if allowed or required.
  if ((large_only || (_mi_os_canuse_large_page(size, try_alignment) &&
    ...
    if (!large_only && try_ok > 0) {
      ...
      *is_large = true;
      p = win_virtual_alloc_prim(addr, size, try_alignment,
      ...
      if (p == NULL) {
        mi_atomic_store_release(
            &large_page_try_ok,
            10UL);  // on error, don't try again for the next N allocations
      }
    }
  }
  ...
  // if (p == NULL) { _mi_warning_message("unable to allocate OS memory (%zu
  // bytes, error code: 0x%x, address: %p, alignment: %zu, flags: 0x%x, large
  // only: %d, allow large: %d)\n", size, GetLastError(), addr, try_alignment,
  // flags, large_only, allow_large); }
  return p;
}
...
int _mi_prim_alloc(void* hint_addr, size_t size, size_t try_alignment,
  ...
  *addr = win_virtual_alloc(hint_addr, size, try_alignment, flags, false,
...
//---------------------------------------------
#ifdef _MSC_VER
#pragma warning(disable : 6250)  // suppress warning calling VirtualFree without
                                 // MEM_RELEASE (for decommit)
#endif
...
int _mi_prim_commit(void* addr, size_t size, bool* is_zero) {
  *is_zero = false;
  ...
  (VirtualQuery(addr, &meminfo, size) > 0) { if ((meminfo.State & MEM_COMMIT) ==
  0) { *is_zero = true;
    }
  }
  */
...
int _mi_prim_decommit(void* addr, size_t size, bool* needs_recommit) {
  ...
  *needs_recommit = true;  // for safety, assume always decommitted even in the
                           // case of an error.
...
int _mi_prim_protect(void* addr, size_t size, bool protect) {
  DWORD oldprotect = 0;
  BOOL ok = VirtualProtect(addr, size, protect ? PAGE_NOACCESS : PAGE_READWRITE,
                           &oldprotect);
  return (ok ? 0 : (int)GetLastError());
}
...
static void* _mi_prim_alloc_huge_os_pagesx(void* hint_addr, size_t size,
                                           int numa_node) {
  const DWORD flags = MEM_LARGE_PAGES | MEM_COMMIT | MEM_RESERVE;

  win_enable_large_os_pages(NULL);

  MI_MEM_EXTENDED_PARAMETER params[3] = {
      {{0, 0}, {0}}, {{0, 0}, {0}}, {{0, 0}, {0}}};
  ...
  if (pNtAllocateVirtualMemoryEx != NULL && mi_huge_pages_available) {
    ...
    if (err == 0 && base != NULL) {
      ...
      _mi_warning_message(
          "unable to allocate using huge (1GiB) pages, trying large (2MiB) "
          "pages instead (status 0x%lx)\n",
          err);
    }
  }
  ...
  if (pVirtualAlloc2 != NULL && numa_node >= 0) {
    ...
    return (*pVirtualAlloc2)(GetCurrentProcess(), hint_addr, size, flags,
...
size_t _mi_prim_numa_node(void) {
  ...
  if (pGetCurrentProcessorNumberEx != NULL && pGetNumaProcessorNodeEx != NULL) {
    ...
    // Vista or earlier, use older API that is limited to 64 processors. Issue
    // #277
  ...
  if (pGetProcessMemoryInfo == NULL) {
    ...
    if (hDll != NULL) {
      pGetProcessMemoryInfo =
          (PGetProcessMemoryInfo)(void (*)(void))GetProcAddress(
              hDll, "GetProcessMemoryInfo");
    }
  }
...
void _mi_prim_out_stderr(const char* msg) {
  ...
  if (!_mi_preloading()) {
    // _cputs(msg);  // _cputs cannot be used as it aborts when failing to lock
    // the console
    ...
    if (hcon == INVALID_HANDLE_VALUE) {
      ...
      hconIsConsole = ((hcon != INVALID_HANDLE_VALUE) &&
...
// We prefer to use BCryptGenRandom instead of (the unofficial) RtlGenRandom but
// when using dynamic overriding, we observed it can raise an exception when
// compiled with C++, and sometimes deadlocks when also running under the VS
// debugger. In contrast, issue #623 implies that on Windows Server 2019 we need
// to use BCryptGenRandom. To be continued..
#pragma comment(lib, "advapi32.lib")
#define RtlGenRandom SystemFunction036
mi_decl_externc BOOLEAN NTAPI RtlGenRandom(PVOID RandomBuffer,
...
bool _mi_prim_random_buf(void* buf, size_t buf_len) {
  if (pBCryptGenRandom == NULL) {
    ...
    if (hDll != NULL) {
      pBCryptGenRandom = (PBCryptGenRandom)(void (*)(void))GetProcAddress(
          hDll, "BCryptGenRandom");
    }
...
bool _mi_prim_thread_is_in_threadpool(void) {
  ...
  if (win_major_version >= 6) {
    ...
    void* const pool_data =
        *((void**)((uint8_t*)teb + (MI_SIZE_BITS == 32 ? 0x0F90 : 0x1778)));
    ...
    if (_mi_win_tls_offset == 0 && reason == DLL_PROCESS_ATTACH) {
      ...
      if (tls_slot == TLS_OUT_OF_INDEXES) {
        _mi_error_message(EFAULT,
                          "unable to allocate the a TLS slot (rebuild without "
...
#endif
#if MI_HAS_TLS_SLOT >= \
    2  // we must initialize the TLS slot before any allocation
...
// nothing to do since `_mi_thread_done` is handled through the
...
static void NTAPI mi_win_main_attach(PVOID module, DWORD reason,
                                     LPVOID reserved) {
  if (reason == DLL_PROCESS_ATTACH || reason == DLL_THREAD_ATTACH) {
    mi_win_main(module, reason, reserved);
  }
}
static void NTAPI mi_win_main_detach(PVOID module, DWORD reason,
                                     LPVOID reserved) {
...
// Set up TLS callbacks in a statically linked library by using special data
// sections. See
// <https://stackoverflow.com/questions/14538159/tls-callback-in-windows> We use
// 2 entries to ensure we call attach events before constructors are called, and
// detach events after destructors are called.
...
#if defined(_MSC_VER)  // on clang/gcc use the constructor attribute (in
                       // `src/prim/prim.c`)
...
static void NTAPI mi_fls_done(PVOID value) {
  ...
  if (theap != NULL) {
    ...
    FlsSetValue(mi_fls_key, NULL);  // prevent recursion as _mi_thread_done may
                                    // set it back to the main theap, issue #672
  }
}
...
bool _mi_is_redirected(void) { return mi_redirected; }
...
void _mi_allocator_done(void) { mi_allocator_done(); }
#endif
