...
static void zone_free(malloc_zone_t* zone, void* p) {
  MI_UNUSED(zone);
  // mi_cfree(p);  // checked free as `zone_free` may be called with invalid
  // pointers
  mi_free(
      p);  // with the page_map and pagemap_commit=1 we can use the regular free
}
...
static inline malloc_zone_t* mi_get_default_zone(void) {
  ...
  if mi_unlikely (!init) {
    ...
    malloc_zone_register(
        &mi_malloc_zone);  // by calling register we avoid a zone error on free
                           // (see
...
static kern_return_t mi_malloc_get_all_zones(task_t task, memory_reader_t mr,
...
static void mi_malloc_printf(const char* fmt, ...) { MI_UNUSED(fmt); }
...
static void zone_print_ptr_info(void* p) { MI_UNUSED(p); }

static void zone_register(malloc_zone_t* zone) { MI_UNUSED(zone); }

static void zone_unregister(malloc_zone_t* zone) { MI_UNUSED(zone); }
...
#endif
__attribute__((used)) static void _mi_macos_override_malloc(void) {
  ...
  if (default_zone != &mi_malloc_zone) {
    ...
    // Reregister the default zone so free and realloc in that zone keep
    // working.
