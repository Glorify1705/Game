...
mi_theap_t* mi_heap_theap(mi_heap_t* heap) { return _mi_heap_theap(heap); }
...
static mi_theap_t* mi_heap_init_theap(const mi_heap_t* const_heap) {
  ...
  if (_mi_is_heap_main(heap)) {
    // this can be called if the (main) thread is not yet initialized (as no
    // allocation happened)
    mi_thread_init();
  ...
  if (heap->theap == 0) {
    ...
    if (heap->theap == 0) {
      _mi_error_message(
          EFAULT, "unable to dynamically create a thread local for a heap\n");
      return NULL;
    }
  } else {
  ...
  if (theap == NULL) {
    ...
    if (theap == NULL) {
      _mi_error_message(EFAULT,
                        "unable to allocate memory for a thread local heap\n");
      return NULL;
    }
    if (!_mi_thread_local_set(heap->theap, theap)) {
      _mi_error_message(
          EFAULT, "unable to allocate memory for a thread local storage\n");
      return NULL;
    }
  }
  return theap;
...
mi_theap_t* _mi_heap_theap_get_peek(const mi_heap_t* heap) {
  if (heap == NULL || _mi_is_heap_main(heap)) {
    return __mi_theap_main;  // don't call _mi_theap_main as it may still be
                             // NULL
  } else {
...
mi_theap_t* _mi_heap_theap_get_or_init(const mi_heap_t* heap) {
  ...
  if mi_unlikely (theap == NULL) {
    ...
    if (theap == NULL) {
      return (mi_theap_t*)&_mi_theap_empty_wrong;
    }  // this will return NULL from page.c:_mi_malloc_generic
  }
...
mi_heap_t* mi_heap_new_in_arena(mi_arena_id_t exclusive_arena_id) {
  ...
  heap->heap_seq =
      mi_atomic_increment_relaxed(&heap_main->subproc->heap_total_count);
...
mi_heap_t* mi_heap_new(void) { return mi_heap_new_in_arena(0); }
  ...
  mi_lock(&heap->arena_pages_lock) {
    for (size_t i = 0; i < MI_MAX_ARENAS; i++) {
      mi_arena_pages_t* arena_pages =
          mi_atomic_load_ptr_relaxed(mi_arena_pages_t, &heap->arena_pages[i]);
      if (arena_pages != NULL) {
        mi_atomic_store_ptr_relaxed(mi_arena_pages_t, &heap->arena_pages[i],
                                    NULL);
        mi_free(arena_pages);
      }
    }
...
bool mi_any_heap_contains(const void* p) { return (mi_heap_of(p) != NULL); }
...
// deprecated
bool mi_check_owned(const void* p) { return mi_any_heap_contains(p); }
