...
#define MI_STAT_FIELDS()                                                      \
  ...
  MI_STAT_COUNTER(                                                            \
      malloc_normal_count) /* number of blocks <= MI_LARGE_OBJ_SIZE_MAX */    \
  MI_STAT_COUNTER(malloc_huge_count) /* number of huge bloks */               \
  MI_STAT_COUNTER(                                                            \
      malloc_guarded_count) /* number of allocations with guard pages */      \
                                                                              \
  /* internal statistics */                                                   \
...
typedef enum mi_chunkbin_e {
  ...
  MI_CBIN_OTHER,   // slice_count: any other from the other bins, and 1 <=
                   // slice_count <= MI_BCHUNK_BITS
  MI_CBIN_MEDIUM,  // slice_count == 8
  MI_CBIN_LARGE,   // slice_count == MI_SIZE_BITS  (only used if
                   // MI_ENABLE_LARGE_PAGES is 1)
...
// stats from a heap
mi_decl_export bool mi_heap_stats_get(mi_heap_t* heap,
                                      mi_stats_t* stats) mi_attr_noexcept;
mi_decl_export char* mi_heap_stats_get_json(mi_heap_t* heap, size_t buf_size,
                                            char* buf)
    mi_attr_noexcept;  // use mi_free to free the result if the input buf ==
                       // NULL
mi_decl_export void mi_heap_stats_print_out(mi_heap_t* heap, mi_output_fun* out,
                                            void* arg) mi_attr_noexcept;

// stats from a subprocess and its heaps aggregated
mi_decl_export bool mi_subproc_stats_get(mi_subproc_id_t subproc_id,
                                         mi_stats_t* stats) mi_attr_noexcept;
mi_decl_export char* mi_subproc_stats_get_json(mi_subproc_id_t subproc_id,
                                               size_t buf_size, char* buf)
    mi_attr_noexcept;  // use mi_free to free the result if the input buf ==
                       // NULL
mi_decl_export void mi_subproc_stats_print_out(mi_subproc_id_t subproc_id,
                                               mi_output_fun* out,
                                               void* arg) mi_attr_noexcept;
// print subprocess and all its heap stats segregated
mi_decl_export void mi_subproc_heap_stats_print_out(mi_subproc_id_t subproc_id,
                                                    mi_output_fun* out,
                                                    void* arg) mi_attr_noexcept;
...
mi_decl_export char* mi_stats_get_json(size_t buf_size, char* buf)
    mi_attr_noexcept;  // use mi_free to free the result if the input buf ==
                       // NULL
...
mi_decl_export bool mi_subproc_stats_get_exclusive(
    mi_subproc_id_t subproc_id, mi_stats_t* stats) mi_attr_noexcept;

mi_decl_export char* mi_stats_as_json(mi_stats_t* stats, size_t buf_size,
                                      char* buf)
    mi_attr_noexcept;  // use mi_free to free the result if the input buf ==
                       // NULL
