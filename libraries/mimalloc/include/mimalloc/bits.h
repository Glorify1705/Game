...
*/
    ...
    defined(_M_ARM64EC)  // consider arm64ec as arm64
...
#endif
#if defined(_MSC_VER) && \
    (MI_ARCH_X64 || MI_ARCH_X86 || MI_ARCH_ARM64 || MI_ARCH_ARM32)
#include <intrin.h>
#endif
...
#define __BMI2__ 1
#endif
#if MI_ARCH_X64 && (defined(__AVX2__) || defined(__BMI2__)) && \
    !defined(__BMI1__)  // msvc
#define __BMI1__ 1
#endif
...
static inline size_t mi_ctz(size_t x) {
    ...
    defined(__BMI1__)  // on x64 tzcnt is defined for 0
  size_t r;
  ...
  return _tzcnt_u64(x);
    ...
    (MI_ARCH_X64 || MI_ARCH_X86 || MI_ARCH_ARM64 || MI_ARCH_ARM32)
  unsigned long idx;
  return (mi_msc_builtinz(_BitScanForward)(&idx, x) ? (size_t)idx
                                                    : MI_SIZE_BITS);
#elif mi_has_builtinz(ctz)
  return (x != 0 ? (size_t)mi_builtinz(ctz)(x) : MI_SIZE_BITS);
#elif defined(__GNUC__) && (MI_ARCH_X64 || MI_ARCH_X86)
  size_t r =
      MI_SIZE_BITS;  // bsf leaves destination unmodified if the argument is 0
                     // (see <https://github.com/llvm/llvm-project/pull/102885>)
...
static inline size_t mi_clz(size_t x) {
#if defined(__GNUC__) && MI_ARCH_X64 && \
    defined(__BMI1__)  // on x64 lzcnt is defined for 0
  size_t r;
  ...
  return _lzcnt_u64(x);
    ...
    (MI_ARCH_X64 || MI_ARCH_X86 || MI_ARCH_ARM64 || MI_ARCH_ARM32)
  unsigned long idx;
  return (mi_msc_builtinz(_BitScanReverse)(&idx, x)
              ? MI_SIZE_BITS - 1 - (size_t)idx
              : MI_SIZE_BITS);
...
// Bit scan forward: find the least significant bit that is set (i.e. count
// trailing zero's) return false if `x==0` (with `*idx` undefined) and true
// otherwise, with the `idx` is set to the bit index (`0 <= *idx <
// MI_BFIELD_BITS`).
static inline bool mi_bsf(size_t x, size_t* idx) {
  ...
  return !is_zero;
    ...
    (MI_ARCH_X64 || MI_ARCH_X86 || MI_ARCH_ARM64 || MI_ARCH_ARM32)
  unsigned long i;
  return (mi_msc_builtinz(_BitScanForward)(&i, x) ? (*idx = (size_t)i, true)
                                                  : false);
#else
...
static inline bool mi_bsr(size_t x, size_t* idx) {
  ...
  return !is_zero;
    ...
    (MI_ARCH_X64 || MI_ARCH_X86 || MI_ARCH_ARM64 || MI_ARCH_ARM32)
  unsigned long i;
  return (mi_msc_builtinz(_BitScanReverse)(&i, x) ? (*idx = (size_t)i, true)
                                                  : false);
#else
