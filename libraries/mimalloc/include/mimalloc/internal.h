...
#pragma warning( \
    disable      \
    : 4127)  // suppress constant conditional warning (due to MI_SECURE paths)
...
mi_theap_t* _mi_theap_default_safe(
    void);  // ensure the returned theap is initialized
...
void _mi_os_free_ex(void* p, size_t size, bool still_committed,
                    mi_memid_t memid, mi_subproc_t* subproc);
...
void _mi_arenas_page_free(mi_page_t* page,
                          mi_theap_t* current_theapx /* can be NULL */);
...
// "page.c"
void* _mi_malloc_generic(mi_theap_t* theap, size_t size,
                         size_t zero_huge_alignment,
                         size_t* usable) mi_attr_noexcept mi_attr_malloc;
...
size_t _mi_page_queue_append(mi_theap_t* theap, mi_page_queue_t* pq,
...
bool _mi_theap_area_visit_blocks(const mi_heap_area_t* area, mi_page_t* page,
...
mi_decl_cold mi_theap_t* _mi_heap_theap_get_or_init(
    const mi_heap_t*
        heap);  // get (and possible create) the theap belonging to a heap
mi_decl_cold mi_theap_t* _mi_heap_theap_get_peek(
    const mi_heap_t* heap);  // get the theap for a heap without initializing
                             // (and return NULL in that case)
void _mi_heap_move_pages(mi_heap_t* heap_from,
                         mi_heap_t* heap_to);       // in "arena.c"
...
// "alloc.c"
void* _mi_page_malloc_zero(mi_theap_t* theap, mi_page_t* page, size_t size,
                           bool zero)
    mi_attr_noexcept;  // called from `_mi_theap_malloc_aligned`
void* _mi_theap_malloc_zero(mi_theap_t* theap, size_t size, bool zero,
                            size_t* usable) mi_attr_noexcept;
void* _mi_theap_malloc_zero_ex(mi_theap_t* theap, size_t size, bool zero,
                               size_t huge_alignment, size_t* usable)
    mi_attr_noexcept;  // called from `_mi_theap_malloc_aligned`
void* _mi_theap_realloc_zero(mi_theap_t* theap, void* p, size_t newsize,
                             bool zero, size_t* usable_pre,
                             size_t* usable_post) mi_attr_noexcept;
...
mi_decl_noreturn mi_decl_cold void _mi_assert_fail(
    const char* assertion, const char* fname, unsigned int line,
...
// adjust stat in special cases to compensate for double counting (and does not
// adjust peak values and can decrease the total)
...
#define mi_heap_stat_counter_increase(heap, stat, amount) \
  __mi_stat_counter_increase_mt(&(heap)->stats.stat, amount)
#define mi_heap_stat_increase(heap, stat, amount) \
  __mi_stat_increase_mt(&(heap)->stats.stat, amount)
#define mi_heap_stat_decrease(heap, stat, amount) \
  __mi_stat_decrease_mt(&(heap)->stats.stat, amount)
#define mi_heap_stat_adjust_increase(heap, stat, amnt) \
  __mi_stat_adjust_increase_mt(&(heap)->stats.stat, amnt)
#define mi_heap_stat_adjust_decrease(heap, stat, amnt) \
  __mi_stat_adjust_decrease_mt(&(heap)->stats.stat, amnt)

#define mi_subproc_stat_counter_increase(subproc, stat, amount) \
  __mi_stat_counter_increase_mt(&(subproc)->stats.stat, amount)
#define mi_subproc_stat_increase(subproc, stat, amount) \
  __mi_stat_increase_mt(&(subproc)->stats.stat, amount)
#define mi_subproc_stat_decrease(subproc, stat, amount) \
  __mi_stat_decrease_mt(&(subproc)->stats.stat, amount)
#define mi_subproc_stat_adjust_increase(subproc, stat, amount) \
  __mi_stat_adjust_increase_mt(&(subproc)->stats.stat, amount)
#define mi_subproc_stat_adjust_decrease(subproc, stat, amount) \
  __mi_stat_adjust_decrease_mt(&(subproc)->stats.stat, amount)

#define mi_os_stat_counter_increase(stat, amount) \
  mi_subproc_stat_counter_increase(_mi_subproc(), stat, amount)
#define mi_os_stat_increase(stat, amount) \
  mi_subproc_stat_increase(_mi_subproc(), stat, amount)
#define mi_os_stat_decrease(stat, amount) \
  mi_subproc_stat_decrease(_mi_subproc(), stat, amount)

#define mi_theap_stat_counter_increase(theap, stat, amount) \
  __mi_stat_counter_increase(&(theap)->stats.stat, amount)
#define mi_theap_stat_increase(theap, stat, amount) \
  __mi_stat_increase(&(theap)->stats.stat, amount)
#define mi_theap_stat_decrease(theap, stat, amount) \
  __mi_stat_decrease(&(theap)->stats.stat, amount)
#define mi_theap_stat_adjust_increase(theap, stat, amnt) \
  __mi_stat_adjust_increase(&(theap)->stats.stat, amnt)
#define mi_theap_stat_adjust_decrease(theap, stat, amnt) \
  __mi_stat_adjust_decrease(&(theap)->stats.stat, amnt)
...
static inline bool mi_mul_overflow(size_t count, size_t size, size_t* total) {
  *total = count * size;
  if mi_likely (((size | count) >> (4 * MI_SIZE_SIZE)) ==
                0) {  // did size and count fit both in the lower half bits of a
                      // size_t?
    return false;
  } else {
...
extern mi_decl_hidden const mi_theap_t
    _mi_theap_empty;  // read-only empty theap, initial value of the thread
                      // local default theap (in the MI_TLS_MODEL_THREAD_LOCAL)
extern mi_decl_hidden const mi_theap_t
    _mi_theap_empty_wrong;  // read-only empty theap used to signal that a theap
                            // for a heap could not be allocated

static inline bool mi_theap_is_initialized(const mi_theap_t* theap) {
  return (theap != NULL && theap->heap != NULL);
}

static inline mi_page_t* _mi_theap_get_free_small_page(mi_theap_t* theap,
                                                       size_t size) {
...
// single indirection and low commit, but large initial virtual reserve (4 GiB
// with 48 bit virtual addresses) used by default on <= 40 bit virtual address
// spaces.
...
static inline mi_page_t* _mi_ptr_page_ex(const void* p, bool* valid) {
  ...
  return (mi_page_t*)((((uintptr_t)p >> MI_ARENA_SLICE_SHIFT) + 1 - ofs)
...
//
// the page-map is usually 4 MiB (for 48 bit virtual addresses) and points to
// sub maps of 64 KiB. the page-map is committed on-demand (in 64 KiB parts)
// (and sub-maps are committed on-demand as well) one sub page-map = 64 KiB =>
// covers 2^(16-3) * 2^16 = 2^29 = 512 MiB address space the page-map needs
// 48-(16+13) = 19 bits => 2^19 sub map pointers = 2^22 bytes = 4 MiB reserved
// size.
#define MI_PAGE_MAP_SUB_SHIFT (13)
#define MI_PAGE_MAP_SUB_COUNT (MI_ZU(1) << MI_PAGE_MAP_SUB_SHIFT)
#define MI_PAGE_MAP_SHIFT \
...
static inline size_t _mi_page_map_index(const void* p, size_t* sub_idx) {
  ...
  if (sub_idx != NULL) {
    *sub_idx = u % MI_PAGE_MAP_SUB_COUNT;
  }
...
static inline bool mi_page_contains_address(const mi_page_t* page,
                                            const void* p) {
  size_t psize;
...
// This gives the offset relative to the start slice of a page. This may change
// if we ever locate page info outside the page-data itself.
static inline size_t mi_page_slice_offset_of(
    const mi_page_t* page, size_t offset_relative_to_page_start) {
  return (page->page_start - mi_page_slice_start(page)) +
         offset_relative_to_page_start;
}

static inline size_t mi_page_committed(const mi_page_t* page) {
  return (page->slice_committed == 0
              ? mi_page_size(page)
              : page->slice_committed -
                    (page->page_start - mi_page_slice_start(page)));
}
  ...
  return (mi_page_is_singleton(page) &&
          (page->block_size > MI_LARGE_MAX_OBJ_SIZE ||
           (mi_memkind_is_os(page->memid.memkind) &&
            page->memid.mem.os.base < (void*)page)));
}

static inline mi_page_queue_t* mi_page_queue(const mi_theap_t* theap,
                                             size_t size) {
  mi_page_queue_t* const pq = &((mi_theap_t*)theap)->pages[_mi_bin(size)];
  if (size <= MI_LARGE_MAX_OBJ_SIZE) {
    mi_assert_internal(pq->block_size <= MI_LARGE_MAX_OBJ_SIZE);
  }
  return pq;
}
...
static inline void mi_page_flags_set(mi_page_t* page, bool set,
                                     mi_page_flags_t newflag) {
  if (set) {
...
static inline void mi_page_set_has_interior_pointers(mi_page_t* page,
                                                     bool has_aligned) {
...
static inline void mi_page_set_theap(mi_page_t* page, mi_theap_t* theap) {
  // mi_assert_internal(!mi_page_is_in_full(page));  // can happen when
  // destroying pages on theap_destroy
  page->theap = theap;
...
static inline bool mi_page_is_abandoned(const mi_page_t* page) {
  // note: the xtheap field of an abandoned theap is set to the subproc (for
  // fast reclaim-on-free)
...
static inline mi_heap_t* mi_page_heap(const mi_page_t* page) {
  ...
  // we use NULL for the main heap to make `_mi_page_get_associated_theap` fast
  // in `free.c:mi_abandoned_page_try_reclaim`.
...
static inline bool mi_page_is_owned(const mi_page_t* page) {
  return mi_tf_is_owned(
      mi_atomic_load_relaxed(&((mi_page_t*)page)->xthread_free));
}
...
mi_decl_restrict void* _mi_theap_malloc_guarded(mi_theap_t* theap, size_t size,
...
static inline mi_memid_t _mi_memid_create_meta(void* mpage, size_t block_idx,
                                               size_t block_count) {
...
// Generally, we check for "fast short rep movsb/stosb" (FSRM/FSRS) or "fast
// enhanced rep movsb" (ERMS) support (AMD Zen3+ (~2020) or Intel Ice Lake+
// (~2017). See also issue #201 and pr #253. Todo: we see improvements on win32
// but less with glibc; we might want to only enable this on windows.
// ---------------------------------------------------------------------------------

#if !MI_TRACK_ENABLED && (MI_ARCH_X64 || MI_ARCH_X86) && \
...
static inline void _mi_memcpy(void* dst, const void* src, size_t n) {
  if mi_likely (n <= _mi_cpu_movsb_max) {  // has fsrm && n <= 127  (todo: and
                                           // maybe has erms?)
...
static inline void _mi_memzero(void* dst, size_t n) { _mi_memset(dst, 0, n); }
