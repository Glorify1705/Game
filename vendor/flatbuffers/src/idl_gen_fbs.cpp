    ...
    const std::vector<voffset_t>& reserved_ids)
{
    ...
    for (const auto* field : fields) {
        ...
        if (id_attribute != nullptr && !id_attribute->constant.empty()) {
            ...
            auto id_it = std::find(std::begin(reserved_ids), std::end(reserved_ids), proto_id);
    ...
    const StructDef& struct_def, IDLOptions::ProtoIdGapAction gap_action)
{
    ...
    for (const auto* field : fields) {
        ...
        if (id_attribute != nullptr) {
            ...
            if (id_attribute->constant.empty() && field->value.type.base_type == BASE_TYPE_UNION) {
...
std::string GenerateFBS(const Parser& parser, const std::string& file_name)
{
    ...
    for (auto it = parser.structs_.vec.begin(); it != parser.structs_.vec.end();
        ...
        const auto proto_fbs_ids = MapProtoIdsToFieldsId(struct_def, parser.opts.proto_id_gap_action);
...
namespace {
    ...
    public:
        ...
        {
            if (!GenerateFBS(parser, path, filename)) {
                return Status::ERROR;
            }
            return Status::OK;
        }
