...
namespace {
        ...
        const std::string& file_name)
    {
        ...
        for (auto it = parser.enums_.vec.begin(); it != parser.enums_.vec.end();
            ...
            if (!make_rule.empty())
                make_rule += " ";
            std::string directory = BaseGenerator::NamespaceDir(parser, path, *enum_def.defined_namespace);
    ...
    const bool dasherize)
{
    ...
    for (auto it = namespaces.begin(); it != namespaces.end(); ++it) {
        namespace_dir += !dasherize ? *it : ConvertCase(*it, Case::kDasher, Case::kUpperCamel);
...
{
    return path + file_name + options.filename_suffix + "." + (options.filename_extension.empty() ? default_extension_ : options.filename_extension);
}
    ...
    const std::string& file_name)
{
    if (parser.opts.use_flexbuffers) {
        ...
        return !parser.flex_builder_.GetSize() || flatbuffers::SaveFile(BinaryFileName(parser, path, file_name).c_str(), data_ptr, parser.flex_builder_.GetSize(), true);
    }
    return !parser.builder_.GetSize() || flatbuffers::SaveFile(BinaryFileName(parser, path, file_name).c_str(), reinterpret_cast<char*>(parser.builder_.GetBufferPointer()), parser.builder_.GetSize(), true);
}
    ...
    const std::string& file_name)
{
    ...
    std::string make_rule = BinaryFileName(parser, path, filebase) + ": " + file_name;
