      ...
      #undef FLATBUFFERS_TD
        ...
        std::string GetterReturnType(const FieldDef& field) const
        {
            ...
            if (field.IsScalarOptional() ||
                ...
                (!field.IsRequired() && (base_type == BASE_TYPE_STRING || base_type == BASE_TYPE_STRUCT || base_type == BASE_TYPE_UNION)) ||
                // vector of anything not scalar
                (base_type == BASE_TYPE_VECTOR && !field.IsRequired() && !IsScalar(field.value.type.VectorType().base_type))) {
                r_type += "?";
            }
            return r_type;
        }
        ...
        void GenEnum(EnumDef& enum_def, CodeWriter& writer) const
        {
            ...
            GenerateCompanionObject(writer, [&]() {
                ...
                bool generate_names = range / static_cast<uint64_t>(enum_def.size()) < kMaxSparseness && parser_.opts.mini_reflect == IDLOptions::kTypesAndNames;
                if (generate_names) {
            ...
            std::string bb_var_name) const
        {
            ...
            default:
                return bb_var_name + "." + namer_.Method("get", GenTypeBasic(type.base_type));
            }
        }
        ...
        std::string GenOffsetGetter(flatbuffers::FieldDef* key_field,
            const char* num = nullptr) const
        {
            std::string key_offset = "__offset(" + NumToString(key_field->value.offset) + ", ";
            if (num) {
                key_offset += num;
                key_offset += ", _bb)";
            } else {
            ...
            const IDLOptions options) const
        {
            ...
            auto method_name = namer_.LegacyJavaMethod2("finishSizePrefixed", struct_def, "Buffer");
            GenerateFunOneLine(
            ...
            const IDLOptions options) const
        {
            auto id = identifier.length() > 0 ? ", \"" + identifier + "\"" : "";
            auto params = "builder: FlatBufferBuilder, offset: Int";
            auto method_name = namer_.LegacyKotlinMethod("finish", struct_def, "Buffer");
            GenerateFunOneLine(
                writer, method_name, params, "",
            ...
            const IDLOptions options) const
        {
            ...
            auto params = "builder: FlatBufferBuilder, data: " + GenTypeBasic(vector_type.base_type) + "Array";
                        ...
                        for (size_t size = largest; size; size /= 2) {
                            for (auto it = fields_vec.rbegin(); it != fields_vec.rend();
                                ...
                                if (!field.deprecated && (!sortbysize || size == base_type_size)) {
                                    ...
                                    writer += namer_.LegacyKotlinMethod("add", field, "") + "(builder, {{field_name}}\\";
                                    if (!IsScalar(field.value.type.base_type)) {
                                        writer += "Offset\\";
                                    }
                                    // we wrap on null check for scalar optionals
        ...
        void GenerateStructGetters(StructDef& struct_def, CodeWriter& writer) const
        {
            ...
            for (auto it = fields_vec.begin(); it != fields_vec.end(); ++it) {
                ...
                } else if (value_base_type == BASE_TYPE_VECTOR && field.value.type.element == BASE_TYPE_STRUCT) {
                ...
                if (IsScalar(value_base_type)) {
                            ...
                            GenerateFun(
                                writer, field_name, "obj: " + field_type, return_type, [&]() {
                                    ...
                                    OffsetWrapper(
                                        ...
                                        [&]() {
                                            if (field.IsRequired()) {
                                                writer += "throw AssertionError(\"No value for "
                    ...
                    case BASE_TYPE_STRING:
                        ...
                        GenerateGetter(writer, field_name, return_type, [&]() {
                            ...
                            writer.IncrementIdentLevel();
                            if (field.IsRequired()) {
                                writer += "throw AssertionError(\"No value for (required) field "
                                          "{{field_name}}\")";
                            } else {
                                writer += "null";
                            }
                    ...
                    case BASE_TYPE_VECTOR: {
                        ...
                        if (vectortype.base_type == BASE_TYPE_STRUCT || vectortype.base_type == BASE_TYPE_UNION) {
                        ...
                        GenerateFun(writer, field_name, params, return_type, [&]() {
                            ...
                            auto not_found = field.IsRequired()
                            ...
                            if (kfield.key) {
                                ...
                                GenerateFun(writer, name, params, rtype, [&]() {
                                    OffsetWrapper(
                                        writer, offset_val,
                                        [&]() {
                                            writer += qualified_name + ".__lookup_by_key(null, __vector(o), key, bb)";
                                        },
                                        [&]() { writer += "null"; });
                                });

                                auto param2 = "obj: " + qualified_name + ", key: " + GenTypeGet(kfield.value.type);
                                GenerateFun(writer, name, param2, rtype, [&]() {
                                    OffsetWrapper(
                                        writer, offset_val,
                                        [&]() {
                                            writer += qualified_name + ".__lookup_by_key(obj, __vector(o), key, bb)";
                                        },
                                        [&]() { writer += "null"; });
                                });
                ...
                if ((value_base_type == BASE_TYPE_VECTOR && IsScalar(field.value.type.VectorType().base_type)) || value_base_type == BASE_TYPE_STRING) {
                ...
                if (field.nested_flatbuffer) {
                    ...
                    auto nested_method_name = field_name + "As" + field.nested_flatbuffer->name;
                ...
                if (parser_.opts.mutable_buffer) {
                    ...
                    if (IsScalar(value_base_type) || (value_base_type == BASE_TYPE_VECTOR && IsScalar(value_type.VectorType().base_type))) {
                        auto statements = [&]() {
            ...
            const CommentConfig* config) const
        {
            ...
            std::string line_prefix = ((config != nullptr && config->content_line_prefix != nullptr)
        ...
        {
            return "val o = __offset(" + offset + "); return if (o != 0) " + found + " else " + not_found;
        }
