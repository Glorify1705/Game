      ...
      #undef FLATBUFFERS_TD
            ...
            std::string* code_ptr)
        {
            ...
            if (IsScalar(field.value.type.base_type)) {
                std::string acc;
                if (struct_def.fixed) {
                    acc = "buf_.read_" + GenTypeName(field.value.type) + "_le(pos_ + " + offsets + ")";

                } else {
                    auto defval = field.IsOptional() ? "0" : field.value.constant;
                    acc = "buf_.flatbuffers_field_" + GenTypeName(field.value.type) + "(pos_, " + offsets + ", " + defval + ")";
            ...
            case BASE_TYPE_VECTOR: {
                ...
                if (vectortype.base_type == BASE_TYPE_STRUCT) {
                    auto start = "buf_.flatbuffers_field_vector(pos_, " + offsets + ") + i * " + NumToString(InlineSize(vectortype));
                    if (!(vectortype.struct_def->fixed)) {
                        start = "buf_.flatbuffers_indirect(" + start + ")";
                    }
                    code += def + "(i:int) -> " + NamespacedName(*field.value.type.struct_def) + ":\n        return ";
                    code += NamespacedName(*field.value.type.struct_def) + " { buf_, " + start + " }\n";
                } else {
                    ...
                    code += "(buf_.flatbuffers_field_vector(pos_, " + offsets + ") + i * " + NumToString(InlineSize(vectortype)) + ")\n";
                }
                break;
            }
        ...
        void GenTableBuilders(const StructDef& struct_def, std::string* code_ptr)
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                code += "    def add_" + NormalizedName(field) + "(" + NormalizedName(field) + ":" + LobsterType(field.value.type) + "):\n        b_.Prepend" + GenMethod(field.value.type) + "Slot(" + NumToString(offset) + ", " + NormalizedName(field);
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (field.deprecated)
                    continue;
                if (IsVector(field.value.type)) {
                    code += "def " + NormalizedName(struct_def) + "Start" + ConvertCase(NormalizedName(field), Case::kUpperCamel) + "Vector(b_:flatbuffers_builder, n_:int):\n    b_.StartVector(";
                    auto vector_type = field.value.type.VectorType();
                    auto alignment = InlineAlignment(vector_type);
                    auto elem_size = InlineSize(vector_type);
                    code += NumToString(elem_size) + ", n_, " + NumToString(alignment) + ")\n";
                    if (vector_type.base_type != BASE_TYPE_STRUCT || !vector_type.struct_def->fixed) {
                        code += "def " + NormalizedName(struct_def) + "Create" + ConvertCase(NormalizedName(field), Case::kUpperCamel) + "Vector(b_:flatbuffers_builder, v_:[" + LobsterType(vector_type) + "]):\n    b_.StartVector(" + NumToString(elem_size) + ", v_.length, " + NumToString(alignment) + ")\n    reverse(v_) e_: b_.Prepend" + GenMethod(vector_type) + "(e_)\n    return b_.EndVector(v_.length)\n";
                    }
                    code += "\n";
                }
            }
        ...
        void GenStruct(const StructDef& struct_def, std::string* code_ptr)
        {
            ...
            if (!struct_def.fixed) {
                ...
                // the root type.
                code += "def GetRootAs" + NormalizedName(struct_def) + "(buf:string): return " + NormalizedName(struct_def) + " { buf, buf.flatbuffers_indirect(0) }\n\n";
            }
            if (struct_def.fixed) {
        ...
        void GenEnum(const EnumDef& enum_def, std::string* code_ptr)
        {
            ...
            for (auto it = enum_def.Vals().begin(); it != enum_def.Vals().end(); ++it) {
                ...
                code += "    " + enum_def.name + "_" + NormalizedName(ev) + " = " + enum_def.ToString(ev) + "\n";
            }
            code += "\n";
        }
            ...
            std::string* code_ptr)
        {
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (IsStruct(field.value.type)) {
                    ...
                    std::string& code = *code_ptr;
                    code += ", " + (nameprefix + NormalizedName(field)) + ":" + LobsterType(field.value.type);
                }
            }
        }
        ...
        {
            std::string& code = *code_ptr;
            code += "    b_.Prep(" + NumToString(struct_def.minalign) + ", " + NumToString(struct_def.bytesize) + ")\n";
            for (auto it = struct_def.fields.vec.rbegin();
                ...
                if (IsStruct(field.value.type)) {
                    ...
                    code += "    b_.Prepend" + GenMethod(field.value.type) + "(" + nameprefix + NormalizedName(field) + ")\n";
                }
            }
        }
        ...
        {
            std::string& code = *code_ptr;
            code += "def Create" + NormalizedName(struct_def) + "(b_:flatbuffers_builder";
