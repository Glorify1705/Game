...
namespace {
        ...
        bool required)
    {
        ...
        return !offset || v.VerifyFieldStruct(reinterpret_cast<const uint8_t*>(&parent_table), offset, obj.bytesize(), obj.minalign());
    }
        ...
        const reflection::Field& vec_field)
    {
        ...
        case reflection::String: {
            auto vec_string = flatbuffers::GetFieldV<flatbuffers::Offset<flatbuffers::String>>(
                table, vec_field);
        ...
        case reflection::Obj: {
            ...
            if (obj->is_struct()) {
                ...
                auto vec = flatbuffers::GetFieldV<flatbuffers::Offset<flatbuffers::Table>>(
                    table, vec_field);
...
double GetAnyValueF(reflection::BaseType type, const uint8_t* data)
{
    ...
    case reflection::String: {
        auto s = reinterpret_cast<const String*>(ReadScalar<uoffset_t>(data) + data);
        if (s) {
            double d;
...
public:
    ...
    {
        auto dag_idx = reinterpret_cast<const uoffset_t*>(offsetloc) - reinterpret_cast<const uoffset_t*>(buf_.data());
    ...
    void ResizeTable(const reflection::Object& objectdef, Table* table)
    {
        ...
        if (startptr_ <= tableloc) {
            ...
            for (auto it = fielddefs->begin(); it != fielddefs->end(); ++it) {
                ...
                // Ignore scalars.
                if (base_type <= reflection::Double)
                    continue;
    ...
    const reflection::Object* root_table)
{
    ...
    auto start = static_cast<uoffset_t>(vec_start) + static_cast<uoffset_t>(sizeof(uoffset_t)) + elem_size * num_elems;
    if (delta_bytes) {
        if (delta_elem < 0) {
    ...
    const uint8_t* newbuf, size_t newlen)
{
    ...
    while ((flatbuf.size() & (sizeof(uoffset_t) - 1)) || !(flatbuf.size() & (sizeof(largest_scalar_t) - 1))) {
        flatbuf.push_back(0);
    }
    ...
    const Table& table, bool use_string_pooling)
{
    ...
    for (auto it = fielddefs->begin(); it != fielddefs->end(); ++it) {
        ...
        case reflection::Vector: {
            auto vec = table.GetPointer<const Vector<Offset<Table>>*>(fielddef.offset());
