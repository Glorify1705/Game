...
#define EXPECT(tok) ECHECK(Expect(tok))
    ...
    {
        return "[" + NumToString((flatbuffers::numeric_limits<T>::lowest)()) + "; " + NumToString((flatbuffers::numeric_limits<T>::max)()) + "]";
    }
    ...
    static CheckedError atot(const char* s, Parser& parser, T* val)
    {
        ...
        if (0 == *val)
            ...
            return parser.Error("invalid number: \"" + std::string(s) + "\"" + ", constant does not fit " + TypeToIntervalString<T>());
    }
    template <>
...
#undef FLATBUFFERS_TD
        ...
        const FieldDef& key)
    {
        ...
        if (a && b) {
            ...
            return *reinterpret_cast<const String*>(a) < *reinterpret_cast<const String*>(b);
        } else {
            return a ? true : false;
        }
    }
    ...
    static void SimpleQsort(T* begin, T* end, size_t width, F comparator,
        S swapper)
    {
        if (end - begin <= static_cast<ptrdiff_t>(width))
            return;
        auto l = begin + width;
        auto r = end;
        while (l < r) {
    ...
    static uint64_t HashFile(const char* source_filename, const char* source)
    {
        ...
        if (source && *source)
            hash ^= HashFnv1a<uint64_t>(source);
    ...
    {
        return a->defined_namespace->GetFullyQualifiedName(a->name) < b->defined_namespace->GetFullyQualifiedName(b->name);
    }
...
{
    return Error("maximum parsing depth " + NumToString(parse_depth_counter_) + " reached");
}
...
public:
    explicit ParseDepthGuard(Parser* parser_not_null)
        : parser_(*parser_not_null)
        , caller_depth_(parser_.parse_depth_counter_)
    {
        FLATBUFFERS_ASSERT(caller_depth_ <= (FLATBUFFERS_MAX_PARSING_DEPTH) && "Check() must be called to prevent stack overflow");
...
CheckedError Parser::ParseHexNum(int nibbles, uint64_t* val)
{
    ...
    for (int i = 0; i < nibbles; i++)
        if (!is_xdigit(cursor_[i]))
            return Error("escape code must be followed by " + NumToString(nibbles) + " hex digits");
...
CheckedError Parser::Next()
{
    ...
    for (;;) {
        ...
        case '\'': {
                    ...
                    case 'u': {
                        ...
                        if (val >= 0xD800 && val <= 0xDBFF) {
                            ...
                            if (unicode_high_surrogate == -1) {
                                ...
                                int code_point = 0x10000 + ((unicode_high_surrogate & 0x03FF) << 10) + (val & 0x03FF);
            ...
            cursor_++;
            if (!attr_is_trivial_ascii_string_ && !opts.allow_non_utf8 && !ValidateUTF8(attribute_)) {
        ...
        default:
            if (IsIdentifierStart(c)) {
                ...
                while (IsIdentifierStart(*cursor_) || is_digit(*cursor_))
                    cursor_++;
            ...
            if (has_sign) {
                ...
                if (strncmp(cursor_, "inf", 3) == 0 && !(IsIdentifierStart(cursor_[3]) || is_digit(cursor_[3]))) {
            ...
            if (is_digit(c) || has_sign || !dot_lvl) {
                ...
                if (use_hex)
                    start_digits = ++cursor_; // '0x' is the prefix, skip it
                // Read an integer number or mantisa of float-point number.
                do {
                    if (use_hex) {
                        while (is_xdigit(*cursor_))
                            cursor_++;
                    } else {
                ...
                if ((dot_lvl >= 0) && (cursor_ > start_digits)) {
                    ...
                    if ((use_hex && is_alpha_char(*cursor_, 'P')) || is_alpha_char(*cursor_, 'E')) {
...
{
    if (t != token_) {
        return Error("expecting: " + TokenToString(t) + " instead got: " + TokenToStringId(token_));
    }
    NEXT();
    return NoError();
}
...
CheckedError Parser::ParseField(StructDef& struct_def)
{
    ...
    if (struct_def.fixed) {
        if (IsIncompleteStruct(type) || (IsArray(type) && IsIncompleteStruct(type.VectorType()))) {
            std::string type_name = IsArray(type) ? type.VectorType().struct_def->name
                                                  : type.struct_def->name;
            return Error(
                std::string("Incomplete type in struct is not allowed, type name: ") + type_name);
        }
    ...
    if (token_ == '=') {
        ...
        if (IsVector(type) && field->value.constant != "0" && field->value.constant != "[]") {
    ...
    if (IsFloat(type.base_type)) {
        ...
        if ((false == IsIdentifierStart(*s)) && (std::string::npos == field->value.constant.find_first_of(".eEpP"))) {
    ...
    if (hash_name) {
        ...
        case BASE_TYPE_USHORT: {
            if (FindHashFunction16(hash_name->constant.c_str()) == nullptr)
                return Error("Unknown hashing algorithm for 16 bit types: " + hash_name->constant);
            break;
        }
        case BASE_TYPE_INT:
        case BASE_TYPE_UINT: {
            if (FindHashFunction32(hash_name->constant.c_str()) == nullptr)
                return Error("Unknown hashing algorithm for 32 bit types: " + hash_name->constant);
            break;
        }
        case BASE_TYPE_LONG:
        case BASE_TYPE_ULONG: {
            if (FindHashFunction64(hash_name->constant.c_str()) == nullptr)
                return Error("Unknown hashing algorithm for 64 bit types: " + hash_name->constant);
            break;
        }
        default:
            return Error(
    ...
    const bool default_str_or_vec = ((IsString(type) || IsVector(type)) && field->value.constant != "0");
    ...
    if (field->key) {
        ...
        if (IsArray(type)) {
            is_valid |= IsScalar(type.VectorType().base_type) || IsStruct(type.VectorType());
        }
        if (!is_valid) {
            return Error(
    ...
    if (type.enum_def) {
        ...
        if (type.base_type == BASE_TYPE_UNION) {
            ...
            if (!field->IsOptional() && type.enum_def->attributes.Lookup("bit_flags") == nullptr) {
    ...
    if (field->native_inline && !IsStruct(field->value.type) && !IsVectorOfStruct(field->value.type) && !IsVectorOfTable(field->value.type))
        return Error(
    ...
    uoffset_t count, bool inside_vector)
{
    ...
    case BASE_TYPE_UNION: {
                ...
                if (inside_vector) {
                    if (IsVector(type) && type.element == BASE_TYPE_UTYPE) {
                        ...
                        vector_of_union_types = reinterpret_cast<Vector<uint8_t>*>(
                            builder_.GetCurrentBufferPointer() + builder_.GetSize() - offset);
                        break;
                    }
                } else {
        ...
        if (vector_of_union_types) {
            if (vector_of_union_types->size() <= count)
                return Error(
                    "union types vector smaller than union values vector for: " + field->name);
        ...
        if (!enum_val)
            return Error("illegal type id for: " + field->name);
    ...
    case BASE_TYPE_LONG:
    case BASE_TYPE_ULONG: {
        if (field && field->attributes.Lookup("hash") && (token_ == kTokenIdentifier || token_ == kTokenStringConstant)) {
    ...
    const StructDef* struct_def, F body)
{
    ...
    for (;;) {
        if ((!opts.strict_json || !fieldn) && Is(terminator))
            break;
        std::string name;
        if (is_nested_vector) {
            ...
            if (!opts.protobuf_ascii_alike || !(Is('{') || Is('[')))
                EXPECT(':');
        }
    ...
    uoffset_t* ovalue)
{
            ...
            if (!field) {
                ...
                if (IsIdent("null") && !IsScalar(field->value.type.base_type)) {
                    ...
                    for (; elem != field_stack_.rbegin() + fieldn; ++elem) {
                        ...
                        if (existing_field->value.offset < field->value.offset)
                            break;
                    }
    ...
    for (size_t i = 0; i < fieldn_outer; i++)
    ...
    size_t min_align, size_t* align)
{
    ...
    uint8_t align_value;
    if (StringToNumber(align_constant.c_str(), &align_value) && VerifyAlignmentRequirements(static_cast<size_t>(align_value), min_align)) {
        *align = align_value;
        return NoError();
    }
    return Error("unexpected force_align value '" + align_constant + "', alignment must be a power of two integer ranging from the "
                                                                     "type\'s natural alignment "
        + NumToString(min_align) + " to " + NumToString(FLATBUFFERS_MAX_ALIGNMENT));
}
    ...
    *ovalue = builder_.EndVector(count);
    ...
    if (type.base_type == BASE_TYPE_STRUCT && type.struct_def->has_key) {
        ...
        if (type.struct_def->fixed) {
            ...
            auto v = reinterpret_cast<VectorOfAny*>(builder_.GetCurrentBufferPointer());
            SimpleQsort<uint8_t>(
...
CheckedError Parser::ParseArray(Value& array)
{
    ...
    ECHECK(err);
    if (length != count)
        return Error("Fixed-length array size is incorrect.");
    ...
    if (Is('(')) {
        ...
        for (;;) {
            ...
            if (false == (Is(kTokenIdentifier) || Is(kTokenStringConstant)))
                return Error("attribute name must be either identifier or string: " + name);
            if (known_attributes_.find(name) == known_attributes_.end())
                return Error("user define attributes must be declared before use: " + name);
            NEXT();
            auto e = new Value();
            if (attributes->Add(name, e))
                Warning("attribute already found: " + name);
            if (Is(':')) {
                NEXT();
...
CheckedError Parser::ParseEnumFromString(const Type& type,
    std::string* result)
{
    const auto base_type = type.enum_def ? type.enum_def->underlying_type.base_type : type.base_type;
    if (!IsInteger(base_type))
        return Error("not a valid value for this field");
    uint64_t u64 = 0;
...
CheckedError Parser::ParseFunction(const std::string* name, Value& e)
{
    ...
    if (!IsFloat(e.type.base_type)) {
        return Error(functionname + ": type of argument mismatch, expecting: " + TypeName(BASE_TYPE_DOUBLE) + ", found: " + TypeName(e.type.base_type) + ", name: " + (name ? *name : "") + ", value: " + e.constant);
    }
    NEXT();
    EXPECT('(');
  ...
  #undef FLATBUFFERS_FN_DOUBLE
    // clang-format on
    if (true != func_match) {
        return Error(std::string("Unknown conversion function: ") + functionname + ", field name: " + (name ? *name : "") + ", value: " + e.constant);
    }
    ...
    *destmatch = true;
    ...
    if (!check) {
        if (e.type.base_type == BASE_TYPE_NONE) {
            ...
            return Error(std::string("type mismatch: expecting: ") + TypeName(e.type.base_type) + ", found: " + TypeName(req) + ", name: " + (name ? *name : "") + ", value: " + e.constant);
        }
    }
    ...
    if ((kTokenFloatConstant != dtoken) && IsFloat(e.type.base_type)) {
        ...
        if ((std::string::npos != k) && (s.length() > (k + 1)) && (s[k] == '0' && is_alpha_char(s[k + 1], 'X')) && (std::string::npos == s.find_first_of("pP", k + 2))) {
            return Error(
    ...
    bool check_now)
{
    if (token_ == '+' || token_ == '-') {
        ...
        NEXT();
        if (token_ != kTokenIdentifier)
            return Error("constant name expected");
  ...
  #define FORCE_ECHECK(dtoken, check, req) IF_ECHECK_(true, dtoken, check, req)
    ...
    if (is_tok_ident || is_tok_string) {
        ...
        if (!match && is_tok_string && IsScalar(in_type) && !attr_is_trivial_ascii_string_) {
            return Error(
                std::string("type mismatch or invalid value, an initializer of "
                            "non-string field must be trivial ASCII string: type: ")
                + TypeName(in_type) + ", name: " + (name ? *name : "") + ", value: " + attribute_);
        }
        ...
        if (!match && IsInteger(in_type) && !IsBool(in_type) && IsIdentifierStart(*attribute_.c_str())) {
...
#undef IF_ECHECK_
    ...
    if (!match) {
        std::string msg;
        msg += "Cannot assign token starting with '" + TokenToStringId(token_) + "' to value of <" + std::string(TypeName(in_type)) + "> type.";
        return Error(msg);
    }
...
struct EnumValBuilder {
    ...
    FLATBUFFERS_CHECKED_ERROR AcceptEnumerator(const std::string& name)
    {
        ...
        FLATBUFFERS_ASSERT((temp->union_type.enum_def == nullptr) || (temp->union_type.enum_def == &enum_def));
        auto not_unique = enum_def.vals.Add(name, temp);
        temp = nullptr;
        if (not_unique)
            return parser.Error("enum value already exists: " + name);
        return NoError();
    }
    ...
    FLATBUFFERS_CHECKED_ERROR AssignEnumeratorValue(const std::string& value)
    {
        ...
        if (!fit)
            return parser.Error("enum value does not fit, \"" + value + "\"");
        return NoError();
    }
    ...
    inline FLATBUFFERS_CHECKED_ERROR ValidateImpl(int64_t* ev, int m)
    {
        ...
        if (v < dn || v > (up - m)) {
            return parser.Error("enum value does not fit, \"" + NumToString(v) + (m ? " + 1\"" : "\"") + " out of " + TypeToIntervalString<CTYPE>());
        }
    ...
    const char* filename)
{
    ...
    if (filename != nullptr && !opts.project_root.empty()) {
        enum_def->declaration_file = &GetPooledString(RelativeToRootPath(opts.project_root, filename));
    }
        ...
        if (opts.proto_mode && attribute_ == "option") {
            ...
            if (is_union) {
                ...
                if (Is(':')) {
                    ...
                    if (ev.union_type.base_type != BASE_TYPE_STRUCT && ev.union_type.base_type != BASE_TYPE_STRING)
    ...
    for (auto it = prev_it + 1; it != enum_def->Vals().end(); ++it) {
        ...
        if (prev_ev->GetAsUInt64() == ev->GetAsUInt64())
            return Error("all enum values must be unique: " + prev_ev->name + " and " + ev->name + " are both " + NumToString(ev->GetAsInt64()));
    }

    if (dest)
        *dest = enum_def;
    const auto qualified_name = current_namespace_->GetFullyQualifiedName(enum_def->name);
...
CheckedError Parser::StartStruct(const std::string& name, StructDef** dest)
{
    ...
    if (!struct_def.predecl)
        return Error("datatype already exists: " + current_namespace_->GetFullyQualifiedName(name));
    ...
    BaseType basetype)
{
    ...
    for (auto it = fields.begin(); it != fields.end(); ++it) {
        auto& fname = (*it)->name;
        if (fname.length() > len && fname.compare(fname.length() - len, len, suffix) == 0 && (*it)->value.type.base_type != BASE_TYPE_UTYPE) {
            auto field = struct_def->fields.Lookup(fname.substr(0, fname.length() - len));
            if (field && field->value.type.base_type == basetype)
                return Error("Field " + fname + " would clash with generated functions for field " + field->name);
        }
    }
    return NoError();
}
...
{
    static FLATBUFFERS_CONSTEXPR unsigned long supported_langs = IDLOptions::kRust | IDLOptions::kSwift | IDLOptions::kLobster | IDLOptions::kKotlin | IDLOptions::kCpp | IDLOptions::kJava | IDLOptions::kCSharp | IDLOptions::kTs | IDLOptions::kBinary | IDLOptions::kGo | IDLOptions::kPython | IDLOptions::kJson | IDLOptions::kNim;
...
{
    static FLATBUFFERS_CONSTEXPR unsigned long supported_langs = IDLOptions::kRust | IDLOptions::kSwift | IDLOptions::kNim;
    return !(opts.lang_to_generate & ~supported_langs);
}

bool Parser::SupportsAdvancedUnionFeatures() const
{
    return (opts.lang_to_generate & ~(IDLOptions::kCpp | IDLOptions::kTs | IDLOptions::kPhp | IDLOptions::kJava | IDLOptions::kCSharp | IDLOptions::kKotlin | IDLOptions::kBinary | IDLOptions::kSwift | IDLOptions::kNim)) == 0;
}

bool Parser::SupportsAdvancedArrayFeatures() const
{
    return (opts.lang_to_generate & ~(IDLOptions::kCpp | IDLOptions::kPython | IDLOptions::kJava | IDLOptions::kCSharp | IDLOptions::kJsonSchema | IDLOptions::kJson | IDLOptions::kBinary | IDLOptions::kRust | IDLOptions::kTs)) == 0;
}
...
CheckedError Parser::ParseDecl(const char* filename)
{
    ...
    if (!fixed && !IsIdent("table"))
        return Error("declaration expected");
    NEXT();
    std::string name = attribute_;
    EXPECT(kTokenIdentifier);
    ...
    if (filename && !opts.project_root.empty()) {
        struct_def->declaration_file = &GetPooledString(RelativeToRootPath(opts.project_root, filename));
    }
    ECHECK(ParseMetaData(&struct_def->attributes));
    struct_def->sortbysize = struct_def->attributes.Lookup("original_order") == nullptr && !fixed;
    EXPECT('{');
    while (token_ != '}')
        ECHECK(ParseField(*struct_def));
    if (fixed) {
        ...
        if (!struct_def->bytesize)
            return Error("size 0 structs not allowed");
    }
    ...
    if (!fixed && fields.size()) {
        ...
        if (num_id_fields || opts.require_explicit_ids) {
            ...
            FLATBUFFERS_ASSERT(fields.size() <= flatbuffers::numeric_limits<voffset_t>::max());
            for (voffset_t i = 0; i < static_cast<voffset_t>(fields.size()); i++) {
                ...
                const auto done = !atot(id_str.c_str(), *this, &id).Check();
                if (!done)
                    return Error("field id\'s must be non-negative number, field: " + field.name + ", id: " + id_str);
                if (i != id)
                    return Error("field id\'s must be consecutive from 0, id " + NumToString(i) + " missing or set twice, field: " + field.name + ", id: " + id_str);
    ...
    EXPECT('}');
    const auto qualified_name = current_namespace_->GetFullyQualifiedName(struct_def->name);
...
CheckedError Parser::ParseService(const char* filename)
{
    ...
    if (filename != nullptr && !opts.project_root.empty()) {
        service_def.declaration_file = &GetPooledString(RelativeToRootPath(opts.project_root, filename));
    }
    ...
    do {
        ...
        if (reqtype.base_type != BASE_TYPE_STRUCT || reqtype.struct_def->fixed || resptype.base_type != BASE_TYPE_STRUCT || resptype.struct_def->fixed)
...
bool Parser::SetRootType(const char* name)
{
    ...
    if (!root_struct_def_)
        root_struct_def_ = LookupStruct(current_namespace_->GetFullyQualifiedName(name));
...
CheckedError Parser::ParseProtoDecl()
{
    ...
    if (IsIdent("package")) {
        ...
        if (isextend) {
            if (Is('.'))
                NEXT(); // qualified names may start with a . ?
            auto id = attribute_;
            EXPECT(kTokenIdentifier);
            ECHECK(ParseNamespacing(&id, nullptr));
        ...
        return Error("don\'t know how to parse .proto declaration starting with " + TokenToStringId(token_));
    }
    return NoError();
}
    ...
    EnumDef** dest)
{
    ...
    enum_def.underlying_type.base_type = is_union ? BASE_TYPE_UTYPE : BASE_TYPE_INT;
    enum_def.underlying_type.enum_def = &enum_def;
    if (dest)
        *dest = &enum_def;
    return NoError();
       ...
       */
            ...
            if (Is('[')) {
                ...
                for (;;) {
                    ...
                    if (key == "default") {
                        ...
                        if (IsFloat(type.base_type) && (val == "inf" || val == "+inf" || val == "-inf")) {
            ...
            if (anonymous_struct) {
                ...
                for (auto field_it = oneof_struct.fields.vec.begin();
                    ...
                    if (oneof_type.base_type != BASE_TYPE_STRUCT || !oneof_type.struct_def || oneof_type.struct_def->fixed)
                        return Error("oneof '" + name + "' cannot be mapped to a union because member '" + oneof_field.name + "' is not a table type.");
...
CheckedError Parser::ParseProtoKey()
{
    if (token_ == '(') {
        ...
        while (token_ == '.' || token_ == kTokenIdentifier)
            NEXT();
        EXPECT(')');
        while (Is('.')) {
            NEXT();
...
CheckedError Parser::SkipAnyJsonValue()
{
    ...
    case kTokenFloatConstant:
        ...
        break;
    default:
        if (IsIdent("true") || IsIdent("false") || IsIdent("null") || IsIdent("inf")) {
            NEXT();
        } else
            return TokenError();
    }
    ...
    flexbuffers::Builder* builder)
{
    ...
    (void)initial_depth;
    auto ok = !StartParseFile(source, source_filename).Check() && !ParseFlexBufferValue(builder).Check();
    if (ok)
        builder->Finish();
...
bool Parser::ParseJson(const char* json, const char* json_filename)
{
    ...
    (void)initial_depth;
    builder_.Clear();
    const auto done = !StartParseFile(json, json_filename).Check() && !DoParseJson().Check();
    FLATBUFFERS_ASSERT(initial_depth == parse_depth_counter_);
    return done;
}
    ...
    for (auto it = structs_.vec.begin(); it != structs_.vec.end();) {
        ...
        if (struct_def.predecl) {
            if (opts.proto_mode) {
                ...
                for (size_t components = struct_def.defined_namespace->components.size() + 1;
                     components && !enum_def; components--) {
                    auto qualified_name = struct_def.defined_namespace->GetFullyQualifiedName(
                ...
                if (enum_def) {
                    ...
                    if (struct_def.refcount)
                        return Error("internal: " + NumToString(struct_def.refcount) + "/" + NumToString(initial_count) + " use(s) of pre-declaration enum not accounted for: " + enum_def->name);
            ...
            auto err = "type referenced but not defined (check namespace): " + struct_def.name;
    ...
    for (auto it = enums_.vec.begin(); it != enums_.vec.end(); ++it) {
        ...
        if (enum_def.is_union) {
            for (auto val_it = enum_def.Vals().begin();
                ...
                if (!(opts.lang_to_generate != 0 && SupportsAdvancedUnionFeatures()) && (IsStruct(val.union_type) || IsString(val.union_type)))

                    return Error(
                        "only tables can be union elements in the generated language: " + val.name);
            }
        }
    }
...
{
    if (!opts.no_leak_private_annotations)
        return NoError();
    ...
    for (auto it = structs_.vec.begin(); it != structs_.vec.end(); it++) {
        ...
        for (auto fld_it = struct_def.fields.vec.begin();
            ...
            if (field.value.type.enum_def) {
                auto err = CheckPrivatelyLeakedFields(struct_def, *field.value.type.enum_def);
                if (err.Check()) {
                    return err;
                }
        ...
        if (enum_def.is_union) {
            for (auto val_it = enum_def.Vals().begin();
                ...
                if (val.union_type.struct_def) {
                    auto err = CheckPrivatelyLeakedFields(enum_def, *val.union_type.struct_def);
                    if (err.Check()) {
                        return err;
                    }
                }
    ...
    const char* include_filename)
{
    ...
    for (;;) {
        ...
        if (opts.proto_mode && (attribute_ == "option" || attribute_ == "syntax" || attribute_ == "package")) {
            ECHECK(ParseProtoDecl());
            ...
            NEXT();
            if (opts.proto_mode && attribute_ == "public")
                NEXT();
            ...
            std::string filepath;
            if (source_filename) {
                auto source_file_directory = flatbuffers::StripFileName(source_filename);
            ...
            if (included_files_.find(HashFile(filepath.c_str(), contents.c_str())) == included_files_.end()) {
    ...
    while (token_ != kTokenEof) {
        if (opts.proto_mode) {
            ...
            if (file_identifier_.length() != flatbuffers::kFileIdentifierLength)
                return Error("file_identifier must be exactly " + NumToString(flatbuffers::kFileIdentifierLength) + " characters");
            EXPECT(';');
...
CheckedError Parser::DoParseJson()
{
    if (token_ != '{') {
        ...
        if (opts.size_prefixed) {
            ...
            builder_.Finish(Offset<Table>(toff), file_identifier_.length() ? file_identifier_.c_str() : nullptr);
        }
    }
    ...
    const std::string& file_name) const
{
    ...
    while (!to_process.empty()) {
        ...
        auto& new_files = (*const_cast<std::map<std::string, std::set<IncludedFile>>*>(
...
bool StructDef::Deserialize(Parser& parser, const reflection::Object* object)
{
    ...
    for (size_t i = 0; i < indexes.size(); i++) {
        ...
        if (!field_def->Deserialize(parser, field) || fields.Add(field_def->name, field_def)) {
            delete field_def;
            return false;
        }
        if (field_def->key) {
        ...
        if (fixed) {
            ...
            auto next_field = i + 1 < indexes.size() ? of.Get(indexes[i + 1]) : nullptr;
            tmp_struct_size += size;
            field_def->padding = next_field ? (next_field->offset() - field_def->value.offset) - size
...
bool FieldDef::Deserialize(Parser& parser, const reflection::Field* field)
{
    ...
    if (attributes.Lookup("flexbuffer")) {
        ...
        if (value.type.base_type != BASE_TYPE_VECTOR || value.type.element != BASE_TYPE_UCHAR)
            return false;
    }
    if (auto nested = attributes.Lookup("nested_flatbuffer")) {
        auto nested_qualified_name = parser.current_namespace_->GetFullyQualifiedName(nested->constant);
    ...
    const reflection::Service* service)
{
    ...
    if (service->calls()) {
        for (uoffset_t i = 0; i < service->calls()->size(); ++i) {
            ...
            if (!call->Deserialize(parser, service->calls()->Get(i)) || calls.Add(call->name, call)) {
                delete call;
                return false;
            }
        }
...
bool EnumDef::Deserialize(Parser& parser, const reflection::Enum* _enum)
{
    ...
    for (uoffset_t i = 0; i < _enum->values()->size(); ++i) {
        ...
        if (!val->Deserialize(parser, _enum->values()->Get(i)) || vals.Add(val->name, val)) {
            delete val;
            return false;
        }
    }
...
bool EnumVal::Deserialize(Parser& parser, const reflection::EnumVal* val)
{
    ...
    if (!union_type.Deserialize(parser, val->union_type()))
        return false;
    if (!DeserializeAttributes(parser, val->attributes()))
        return false;
...
Offset<reflection::Type> Type::Serialize(FlatBufferBuilder* builder) const
{
    ...
    if (base_type == BASE_TYPE_VECTOR && element == BASE_TYPE_STRUCT && struct_def->bytesize != 0) {
...
bool Type::Deserialize(const Parser& parser, const reflection::Type* type)
{
    ...
    if (type->index() >= 0) {
        bool is_series = type->base_type() == reflection::Vector || type->base_type() == reflection::Array;
        if (type->base_type() == reflection::Obj || (is_series && type->element() == reflection::Obj)) {
...
bool Parser::Deserialize(const reflection::Schema* schema)
{
    ...
    for (auto it = schema->objects()->begin(); it != schema->objects()->end();
        ...
        struct_def->defined_namespace = GetNamespace(qualified_name, namespaces_, namespaces_index);
    ...
    for (auto it = schema->enums()->begin(); it != schema->enums()->end(); ++it) {
        ...
        enum_def->defined_namespace = GetNamespace(qualified_name, namespaces_, namespaces_index);
    ...
    if (schema->services()) {
        for (auto it = schema->services()->begin(); it != schema->services()->end();
            ...
            std::string qualified_name = it->name()->str();
            auto service_def = new ServiceDef();
            service_def->defined_namespace = GetNamespace(qualified_name, namespaces_, namespaces_index);
            if (!service_def->Deserialize(*this, *it) || services_.Add(qualified_name, service_def)) {
                delete service_def;
                return false;
            }
        }
...
std::string Parser::ConformTo(const Parser& base)
{
    for (auto sit = structs_.vec.begin(); sit != structs_.vec.end(); ++sit) {
        auto& struct_def = **sit;
        auto qualified_name = struct_def.defined_namespace->GetFullyQualifiedName(struct_def.name);
            ...
            if (field_base) {
                ...
                for (auto fbit = struct_def_base->fields.vec.begin();
                    ...
                    if (field.value.offset == field_base->value.offset) {
                        ...
                        if (!EqualByName(field.value.type, field_base->value.type)) {
                            const auto qualified_field_base = qualified_name + "." + field_base->name;
                            return "field renamed to different type: " + qualified_field_name + " (renamed from " + qualified_field_base + ")";
                        }
                        break;
                    }
                }
    ...
    for (auto eit = enums_.vec.begin(); eit != enums_.vec.end(); ++eit) {
        auto& enum_def = **eit;
        auto qualified_name = enum_def.defined_namespace->GetFullyQualifiedName(enum_def.name);
