      ...
      #undef FLATBUFFERS_TD
            ...
            bool enableLangOverrides) const
        {
            ...
            if (enableLangOverrides) {
                ...
                if (value.type.enum_def != nullptr && value.type.base_type != BASE_TYPE_UNION) {
            ...
            const IDLOptions& opts) const
        {
            ...
            if (opts.cs_gen_json_serializer && opts.generate_object_based_api) {
                code += "[Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters."
                        "StringEnumConverter))]\n";
            }
            ...
            const std::string& offset) const
        {
            ...
            return "new " + Name(struct_def) + "().__assign(" + offset + ", " + data_buffer + ")";
        }
        ...
        std::string GenSetter(const Type& type) const
        {
            if (IsScalar(type.base_type)) {
                ...
                if (GenTypeBasic(type, false) != "byte" && type.base_type != BASE_TYPE_BOOL) {
            ...
            bool in_array = false) const
        {
            ...
            for (auto it = struct_def.fields.vec.rbegin();
                ...
                if (IsStruct(field_type)) {
                    ...
                    const auto& type = IsArray(field_type) ? field_type.VectorType() : field_type;
        ...
        {
            std::string key_offset = "Table.__offset(" + NumToString(key_field->value.offset) + ", ";
            if (num) {
                key_offset += num;
            ...
            flatbuffers::FieldDef* key_field) const
        {
            ...
            return GenGetterForLookupByKey(struct_def, key_field, "builder.DataBuffer",
                ...
                + ".CompareTo(" + GenGetterForLookupByKey(struct_def, key_field, "builder.DataBuffer", "builder.DataBuffer.Length - o2.Value") + ")";
        }
            ...
            const IDLOptions& opts) const
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                std::string optional = "";
                if (!struct_def.fixed && (field.value.type.base_type == BASE_TYPE_STRUCT || field.value.type.base_type == BASE_TYPE_UNION || (IsVector(field.value.type) && (field.value.type.element == BASE_TYPE_STRUCT || field.value.type.element == BASE_TYPE_UNION)))) {
                    optional = "?";
                ...
                std::string method_start = "  public " + type_name_dest + optional + " " + field_name_camel;
                std::string obj = "(new " + type_name + "())";

                // Most field accessors need to retrieve and test the field offset first,
                // this is the prefix code for that:
                auto offset_prefix = IsArray(field.value.type)
                    ? " { return "
                    : (" { int o = __p.__offset(" + NumToString(field.value.offset) + "); return o != 0 ? ");
                // Generate the accessors that don't do object reuse.
                if (field.value.type.base_type == BASE_TYPE_STRUCT) {
                } else if (IsVector(field.value.type) && field.value.type.element == BASE_TYPE_STRUCT) {
                } else if (field.value.type.base_type == BASE_TYPE_UNION || (IsVector(field.value.type) && field.value.type.VectorType().base_type == BASE_TYPE_UNION)) {
                    method_start += "<TTable>";
                ...
                if ((IsScalar(field.value.type.base_type) || (IsVector(field.value.type) && IsScalar(field.value.type.element)))) {
                    ...
                    if ((field.value.type.enum_def == nullptr || IsVector(field.value.type)) || (IsEnum(field.value.type) && field.IsScalarOptional())) {
                ...
                if (IsScalar(field.value.type.base_type)) {
                    ...
                    case BASE_TYPE_VECTOR: {
                        ...
                        if (vectortype.base_type == BASE_TYPE_UNION && HasUnionStringValue(*vectortype.enum_def)) {
                            code += member_suffix;
                            code += "}\n";
                    ...
                    case BASE_TYPE_UNION:
                        ...
                        for (auto uit = field.value.type.enum_def->Vals().begin();
                            ...
                            code += member_suffix + "}\n";
                            if (val->union_type.base_type == BASE_TYPE_STRUCT && val->union_type.struct_def->attributes.Lookup("private")) {
                                code += "  internal ";
                            } else {
                                code += "  public ";
                            }
                ...
                if ((IsVector(field.value.type) && IsScalar(field.value.type.VectorType().base_type)) || IsString(field.value.type)) {
                    code += "#if ENABLE_SPAN_T\n";
                    code += "  public Span<" + GenTypeBasic(field.value.type.VectorType()) + "> Get";
                    code += Name(field);
                    code += "Bytes() { return ";
                    code += "__p.__vector_as_span<" + GenTypeBasic(field.value.type.VectorType()) + ">(";
                    code += NumToString(field.value.offset);
                ...
                if (field.nested_flatbuffer) {
                    ...
                    auto nested_method_name = Name(field) + "As" + field.nested_flatbuffer->name;
                ...
                if (parser_.opts.mutable_buffer) {
                    ...
                    const auto& underlying_type = is_series ? field.value.type.VectorType() : field.value.type;
                    ...
                    // mutate.
                    auto mutator_params = (is_series ? "(int j, " : "(") + GenTypeGet(underlying_type) + " " + EscapeKeyword(field.name) + ") { ";
                    auto setter_index = is_series
                        ? "__p." + (IsArray(field.value.type) ? "bb_pos + " + NumToString(field.value.offset) : "__vector(o)") + +" + j * " + NumToString(InlineSize(underlying_type))
                        : (struct_def.fixed
                ...
                if (parser_.opts.java_primitive_has_method && IsScalar(field.value.type.base_type) && !struct_def.fixed) {
                    auto vt_offset_constant = "  public static final int VT_" + ConvertCase(field.name, Case::kScreamingSnake) + " = " + NumToString(field.value.offset) + ";";
            ...
            if (struct_def.fixed) {
                ...
                if ((has_no_struct_fields || opts.generate_object_based_api) && num_fields && num_fields < 127) {
                    struct_has_create = true;
                    ...
                    for (size_t size = struct_def.sortbysize ? sizeof(largest_scalar_t) : 1;
                        ...
                        for (auto it = struct_def.fields.vec.rbegin();
                            ...
                            if (!field.deprecated && (!struct_def.sortbysize || size == SizeOf(field.value.type.base_type))) {
                                ...
                                if (IsStruct(field.value.type) && opts.generate_object_based_api) {
                                    code += GenTypePointer(field.value.type) + ".Pack(builder, " + EscapeKeyword(field.name) + ")";
                                } else {
                                    code += EscapeKeyword(field.name);
                                    if (!IsScalar(field.value.type.base_type))
                                        code += "Offset";
                ...
                for (auto it = struct_def.fields.vec.begin();
                    ...
                    if (!IsScalar(field.value.type.base_type) && field.value.type.base_type != BASE_TYPE_UNION) {
                        code += ".Value";
                    }
                    if (!field.IsScalarOptional()) {
                    ...
                    if (IsVector(field.value.type)) {
                        ...
                        if (!IsStruct(vector_type)) {
                            ...
                            code += "data[i]";
                            if (vector_type.base_type == BASE_TYPE_STRUCT || IsString(vector_type))
                                code += ".Value";
                            code += "); return ";
            ...
            if (struct_def.has_key && !struct_def.fixed) {
                ...
                code += "      int tableOffset = Table.__indirect(vectorLocation + 4 * "
                        "(start + middle), bb);\n";
            ...
            std::string* code_ptr) const
        {
            ...
            if (!struct_def.fixed) {
                ...
                for (auto kit = fields.begin(); kit != fields.end(); ++kit) {
                    auto& key_field = **kit;
                    if (key_field.key) {
                        auto nullable_annotation = parser_.opts.gen_nullable ? "@Nullable " : "";
            ...
            const IDLOptions& opts) const
        {
            ...
            code += "  }\n\n";
            // As<T>
            code += "  public T As<T>() where T : class { return this." + class_member + " as T; }\n";
            // As, From
            for (auto it = enum_def.Vals().begin(); it != enum_def.Vals().end(); ++it) {
                auto& ev = **it;
                ...
                std::string accessibility = (ev.union_type.base_type == BASE_TYPE_STRUCT && ev.union_type.struct_def->attributes.Lookup("private"))
                    ? "internal"
                    : "public";
                // As
                code += "  " + accessibility + " " + type_name + " As" + ev.name + "() { return this.As<" + type_name + ">(); }\n";
                // From
                auto lower_ev_name = ev.name;
                std::transform(lower_ev_name.begin(), lower_ev_name.end(),
                    lower_ev_name.begin(), CharToLower);
                code += "  " + accessibility + " static " + union_name + " From" + ev.name + "(" + type_name + " _" + lower_ev_name + ") { return new " + union_name + "{ Type = " + Name(enum_def) + "." + Name(ev) + ", " + class_member + " = _" + lower_ev_name + " }; }\n";
            }
            code += "\n";
            // Pack()
            code += "  public static int Pack(Google.FlatBuffers.FlatBufferBuilder "
                    "builder, "
                + union_name + " _o) {\n";
            code += "    switch (_o.Type) {\n";
            for (auto it = enum_def.Vals().begin(); it != enum_def.Vals().end(); ++it) {
                ...
                if (ev.union_type.base_type == BASE_TYPE_NONE) {
                    ...
                    if (IsString(ev.union_type)) {
                        ...
                        code += GenTypeGet(ev.union_type) + ".Pack(builder, _o.As" + ev.name + "()).Value;\n";
                    }
                }
            }
            code += "    }\n";
            ...
            if (opts.cs_gen_json_serializer) {
                ...
                code += "class " + union_name + "_JsonConverter : Newtonsoft.Json.JsonConverter {\n";
                ...
                code += "  }\n";
                code += "  public override void WriteJson(Newtonsoft.Json.JsonWriter writer, "
                        "object value, "
                ...
                code += "    }\n";
                code += "  }\n";
                code += "  public void WriteJson(Newtonsoft.Json.JsonWriter writer, " + union_name + " _o, "
                                                                                                     "Newtonsoft.Json.JsonSerializer serializer) {\n";
                code += "    if (_o == null) return;\n";
                code += "    serializer.Serialize(writer, _o." + class_member + ");\n";
                code += "  }\n";
                code += "  public override object ReadJson(Newtonsoft.Json.JsonReader "
                        "reader, "
                ...
                for (auto it = enum_def.Vals().begin(); it != enum_def.Vals().end();
                    ...
                    if (ev.union_type.base_type == BASE_TYPE_NONE) {
                        ...
                        auto type_name = GenTypeGet_ObjectAPI(ev.union_type, opts);
                        code += "      case " + Name(enum_def) + "." + Name(ev) + ": _o." + class_member + " = serializer.Deserialize<" + type_name + ">(reader); break;\n";
                    }
                }
                code += "    }\n";
                code += "    return _o;\n";
            ...
            for (auto eit = enum_def.Vals().begin(); eit != enum_def.Vals().end();
                ...
                if (ev.union_type.base_type == BASE_TYPE_NONE) {
                    ...
                    if (IsString(ev.union_type)) {
                        ...
                        code += "<" + GenTypeGet(ev.union_type) + ">" + func_suffix + ".Value.UnPack() : null;\n";
                    }
                    code += indent + "    break;\n";
                }
            }
            ...
            const std::set<FieldDef*>& field_has_create) const
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                case BASE_TYPE_STRUCT: {
                    ...
                    if (fixed) {
                        ...
                        code += start + "this." + camel_name + ".HasValue ? this." + camel_name + ".Value.UnPack() : null;\n";
                    }
                    break;
                }
                case BASE_TYPE_ARRAY: {
                    ...
                    auto unpack_method = field.value.type.struct_def == nullptr ? ""
                        ...
                        ? ".UnPack()"
                        : "?.UnPack()";
                    code += start + "new " + type_name.substr(0, type_name.length() - 1) + length_str + "];\n";
                ...
                case BASE_TYPE_VECTOR:
                    if (field.value.type.element == BASE_TYPE_UNION) {
                        code += start + "new " + GenTypeGet_ObjectAPI(field.value.type, opts) + "();\n";
                        ...
                        if (fixed) {
                            ...
                            code += "this." + camel_name + "(_j).HasValue ? this." + camel_name + "(_j).Value.UnPack() : null";
                        }
                        code += ");}\n";
                    }
                    break;
            ...
            code += "  }\n";
            // Pack()
            code += "  public static " + GenOffsetType(struct_def) + " Pack(FlatBufferBuilder builder, " + struct_name + " _o) {\n";
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                case BASE_TYPE_STRUCT: {
                    if (!field.value.type.struct_def->fixed) {
                        code += "    var _" + field.name + " = _o." + camel_name + " == null ? default(" + GenOffsetType(*field.value.type.struct_def) + ") : " + GenTypeGet(field.value.type) + ".Pack(builder, _o." + camel_name + ");\n";
                ...
                case BASE_TYPE_STRING: {
                    std::string create_string = field.shared ? "CreateSharedString" : "CreateString";
                    code += "    var _" + field.name + " = _o." + camel_name + " == null ? default(StringOffset) : "
                                                                               "builder."
                ...
                case BASE_TYPE_VECTOR: {
                    if (field_has_create.find(&field) != field_has_create.end()) {
                        ...
                        case BASE_TYPE_STRING: {
                            ...
                            to_array += "builder." + create_string + "(_o." + property_name + "[_j])";
                            break;
                        }
                        case BASE_TYPE_STRUCT:
                            array_type = "Offset<" + GenTypeGet(field.value.type) + ">";
                            to_array = GenTypeGet(field.value.type) + ".Pack(builder, _o." + property_name + "[_j])";
                            break;
                        case BASE_TYPE_UTYPE:
                        ...
                        case BASE_TYPE_UNION:
                            array_type = "int";
                            to_array = NamespacedName(*field.value.type.enum_def) + "Union.Pack(builder,  _o." + property_name + "[_j])";
                            break;
                        default:
                            gen_for_loop = false;
                            break;
                        ...
                        auto pack_method = field.value.type.struct_def == nullptr
                            ? "builder.Add" + GenMethod(field.value.type.VectorType()) + "(_o." + camel_name + "[_j]);"
                            : GenTypeGet(field.value.type) + ".Pack(builder, _o." + camel_name + "[_j]);";
                        code += "    var _" + field.name + " = default(VectorOffset);\n";
                ...
                case BASE_TYPE_UNION: {
                    code += "    var _" + field.name + "_type = _o." + camel_name + " == null ? " + NamespacedName(*field.value.type.enum_def) + ".NONE : " + "_o." + camel_name + ".Type;\n";
            ...
            if (struct_has_create) {
                        ...
                        if (struct_def.fixed) {
                            ...
                            if (field.value.type.struct_def->fixed) {
                                if (opts.generate_object_based_api)
                                    ...
                                    code += "      " + GenTypeGet(field.value.type) + ".Pack(builder, _o." + camel_name + ")";
                            } else {
                                code += "      _" + field.name;
                            }
                        }
                ...
                for (auto it = struct_def.fields.vec.begin();
                    ...
                    case BASE_TYPE_STRUCT: {
                        if (field.value.type.struct_def->fixed) {
                            code += "    Add" + camel_name + "(builder, " + GenTypeGet(field.value.type) + ".Pack(builder, _o." + camel_name + "));\n";
                        } else {
            ...
            std::vector<FieldArrayLength>& array_lengths) const
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                const auto& field_type = is_array ? field.value.type.VectorType() : field.value.type;
                FieldArrayLength tmp_array_length = {
                    field.name,
                ...
                if (field_type.struct_def != nullptr) {
                    ...
                    if (array_only_lengths.size() > 0) {
                        ...
                        for (size_t i = 0; i < array_only_lengths.size(); ++i) {
                            ...
                            code += "for (var " + idx + " = 0; " + idx + " < " + NumToString(array_only_lengths[i].length) + "; ++" + idx + ") {";
                        }
                        for (size_t i = 0; i < array_only_lengths.size(); ++i) {
                            auto idx = "idx" + NumToString(i);
                            if (i == 0) {
            ...
            const IDLOptions& opts) const
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (opts.cs_gen_json_serializer) {
                    if (IsUnion(field.value.type)) {
                        ...
                        code += "  [Newtonsoft.Json.JsonProperty(\"" + field.name + "_type\")]\n";
                        if (IsVector(field.value.type)) {
                    ...
                    if (IsUnion(field.value.type)) {
                        auto union_name = (IsVector(field.value.type))
                            ? GenTypeGet_ObjectAPI(field.value.type.VectorType(), opts)
                            : type_name;
                        code += "  [Newtonsoft.Json.JsonConverter(typeof(" + union_name + "_JsonConverter))]\n";
                    }
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (IsScalar(field.value.type.base_type)) {
                    ...
                    case BASE_TYPE_ARRAY: {
                        code += "new " + type_name.substr(0, type_name.length() - 1) + NumToString(field.value.type.fixed_length) + "];\n";
                        break;
                    }
                    default: {
                        code += "null;\n";
            ...
            if (opts.cs_gen_json_serializer && parser_.root_struct_def_ == &struct_def) {
                code += "\n";
