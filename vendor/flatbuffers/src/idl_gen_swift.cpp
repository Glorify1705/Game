...
namespace swift {
    ...
    public:
        ...
        {
            const bool is_private_access = parser_.opts.swift_implementation_only || struct_def.attributes.Lookup("private") != nullptr;
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                std::string valueType = IsEnum(field.value.type) ? "{{BASEVALUE}}" : "{{VALUETYPE}}";
                code_ += "private var _{{FIELDVAR}}: " + valueType;
                const auto accessing_value = IsEnum(field.value.type) ? ".value" : "";
                const auto base_value = IsStruct(field.value.type) ? (type + "()")
                                                                   : SwiftConstant(field);
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (!IsEnum(field.value.type)) {
                    ...
                    code_ += GenReaderMainBody() + "{{VALUETYPE}}(rawValue: _{{FIELDVAR}})! }";
                }
            }
            code_ += "";
            code_ += "{{ACCESS_TYPE}} static func verify<T>(_ verifier: inout Verifier, at "
                     "position: "
                     "Int, of type: T.Type) throws where T: Verifiable {";
            Indent();
            code_ += "try verifier.inBuffer(position: position, of: {{STRUCTNAME}}.self)";
            Outdent();
            code_ += "}";
            Outdent();
            code_ += "}\n";
        ...
        void BuildStructConstructor(const StructDef& struct_def)
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (IsScalar(type.base_type)) {
                    ...
                    code_ += "_{{FIELDVAR}} = {{ACCESS}}.readBuffer(of: {{VALUETYPE}}.self, "
                             "at: {{OFFSET}})";
                } else {
        ...
        void GenMutableStructReader(const StructDef& struct_def)
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (IsScalar(field.value.type.base_type) && !IsEnum(field.value.type)) {
                    code_ += GenReaderMainBody() + "return " + GenReader("VALUETYPE") + " }";
                } else if (IsEnum(field.value.type)) {
                    code_.SetValue("BASEVALUE", GenTypeBasic(field.value.type, false));
                    code_ += GenReaderMainBody() + "return " + GenEnumConstructor("{{OFFSET}}") + "?? " + GenEnumDefaultValue(field) + " }";
            ...
            bool is_obj_api = false)
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (IsStruct(field.value.type)) {
                    ...
                    code += nameprefix + field_var + ": " + obj_api_named + object_name + "." + field_field;
                    code += ", ";
                }
            }
        }
        ...
        void GenObjectHeader(const StructDef& struct_def)
        {
            ...
            code_ += "{{ACCESS_TYPE}} struct {{STRUCTNAME}}{{MUTABLE}}: FlatBufferObject\\";
            if (!struct_def.fixed)
                code_ += ", Verifiable\\";
            ...
            Indent();
            code_ += ValidateFunc();
            code_ += "{{ACCESS_TYPE}} var __buffer: ByteBuffer! { return {{ACCESS}}.bb }";
        ...
        void GenTableWriter(const StructDef& struct_def)
        {
            ...
            code_ += "{{ACCESS_TYPE}} static func start{{SHORT_STRUCTNAME}}(_ fbb: inout "
                     "FlatBufferBuilder) -> "
            ...
            if (key_field != nullptr && !struct_def.fixed && struct_def.has_key) {
                ...
                code_ += "{{ACCESS_TYPE}} static func " + namer_.Method("sort_vector_of", struct_def) + "(offsets:[Offset], "
                                                                                                        "_ fbb: inout FlatBufferBuilder) -> Offset {";
                Indent();
                code_ += spacing + "var off = offsets";
                code_ += spacing + "off.sort { Table.compare(Table.offset(Int32($1.o), vOffset: "
            ...
            std::vector<std::string>* create_header)
        {
            ...
            const auto field_var = namer_.Variable(field);
            const auto type = GenType(field.value.type);
            const auto opt_scalar = field.IsOptional() && IsScalar(field.value.type.base_type);
            ...
            std::string check_if_vector = (IsVector(field.value.type) || IsArray(field.value.type)) ? "VectorOf("
                                                                                                    : "(";
            const auto body = "add" + check_if_vector + field_field + ": ";
            code_ += "{{ACCESS_TYPE}} static func " + body + "\\";

            create_func_body.push_back("{{STRUCTNAME}}." + body + field_field + ", &fbb)");

            if (IsScalar(field.value.type.base_type) && !IsBool(field.value.type.base_type)) {
                const std::string is_enum = IsEnum(field.value.type) ? ".rawValue" : "";
                const std::string optional_enum = IsEnum(field.value.type) ? ("?" + is_enum) : "";
                code_ += "{{VALUETYPE}}" + builder_string + "fbb.add(element: {{FIELDVAR}}\\";
                ...
                create_func_header.push_back(
                    "" + field_field + ": " + nullable_type + " = " + (field.IsOptional() ? "nil" : default_value));
                return;
            }
            ...
            if (IsBool(field.value.type.base_type)) {
                ...
                code_ += " at: {{TABLEOFFSET}}.{{OFFSET}}.p) }";
                create_func_header.push_back(
                    field_var + ": " + nullable_type + " = " + (field.IsOptional() ? "nil" : default_value));
                return;
            }

            if (IsStruct(field.value.type)) {
                const auto create_struct = "guard let {{FIELDVAR}} = {{FIELDVAR}} else { return };"
                ...
                create_func_header.push_back(field_var + ": " + type + (field.IsOptional() ? "? = nil" : ""));
                return;
            }

            const auto arg_label = namer_.Variable(field) + (IsVector(field.value.type) || IsArray(field.value.type) ? "VectorOffset" : "Offset");
            create_func_header.push_back(arg_label + " " + field_var + ": " + "Offset" + (field.IsRequired() ? "" : " = Offset()"));
            const auto reader_type = IsStruct(field.value.type) && field.value.type.struct_def->fixed
            ...
            if ((vectortype.base_type == BASE_TYPE_STRUCT && field.value.type.struct_def->fixed) && (IsVector(field.value.type) || IsArray(field.value.type))) {
                const auto field_name = namer_.NamespacedType(*vectortype.struct_def);
                code_ += "{{ACCESS_TYPE}} static func " + namer_.Method("start_vector_of", field_var) + "(_ size: Int, in builder: inout "
                                                                                                        "FlatBufferBuilder) {";
                Indent();
                code_ += "builder.startVector(size * MemoryLayout<" + field_name + ">.size, elementSize: MemoryLayout<" + field_name + ">.alignment)";
                Outdent();
                code_ += "}";
            }
        }
        ...
        void GenTableReaderFields(const FieldDef& field)
        {
            ...
            if (IsScalar(field.value.type.base_type) && !IsEnum(field.value.type) && !IsBool(field.value.type.base_type)) {
                code_ += GenReaderMainBody(optional) + GenOffset() + const_string + GenReader("VALUETYPE", "o") + " }";
            ...
            if (IsEnum(field.value.type)) {
                const auto default_value = field.IsOptional() ? "nil" : GenEnumDefaultValue(field);
                code_.SetValue("BASEVALUE", GenTypeBasic(field.value.type, false));
                code_ += GenReaderMainBody(optional) + "\\";
                code_ += GenOffset() + "return o == 0 ? " + default_value + " : " + GenEnumConstructor("o") + "?? " + default_value + " }";
            ...
            if (IsStruct(field.value.type) && field.value.type.struct_def->fixed) {
                ...
                code_ += GenReaderMainBody(is_required) + GenOffset() + required_reader + "{{ACCESS}}.readBuffer(of: {{VALUETYPE}}.self, at: o) }";
                code_.SetValue("FIELDVAR", namer_.Variable("mutable", field_field));
                code_.SetValue("VALUETYPE", GenType(field.value.type) + Mutable());
                code_.SetValue("CONSTANT", "nil");
                code_ += GenReaderMainBody(is_required) + GenOffset() + required_reader + GenConstructor("o + {{ACCESS}}.postion");
                return;
            }
            ...
            case BASE_TYPE_STRUCT:
                code_.SetValue("VALUETYPE", GenType(field.value.type));
                code_.SetValue("CONSTANT", "nil");
                code_ += GenReaderMainBody(is_required) + GenOffset() + required_reader + GenConstructor(GenIndirect("o + {{ACCESS}}.postion"));
                break;
            ...
            case BASE_TYPE_STRING: {
                ...
                code_ += GenReaderMainBody(is_required) + GenOffset() + required_reader + "{{ACCESS}}.string(at: o) }";
                code_ += "{{ACCESS_TYPE}} var {{FIELDVAR}}SegmentArray: [UInt8]" + is_required + " { return "
            ...
            case BASE_TYPE_UNION:
                ...
                code_ += "{{ACCESS_TYPE}} func {{FIELDVAR}}<T: "
                         ...
                         "T.Type) -> T"
                    + is_required + " { " + GenOffset() + required_reader + "{{ACCESS}}.union(o) }";
                break;
            default:
        ...
        void GenTableReaderVectorFields(const FieldDef& field)
        {
            ...
            code_ += "{{ACCESS_TYPE}} var {{HAS_FIELDVAR}}: Bool { " + GenOffset() + "return o == 0 ? false : true }";
            code_ += "{{ACCESS_TYPE}} var {{FIELDVAR}}Count: Int32 { " + GenOffset() + "return o == 0 ? 0 : {{ACCESS}}.vector(count: o) }";
            code_.SetValue("CONSTANT", IsScalar(vectortype.base_type) ? "0" : "nil");
            const auto nullable = IsScalar(vectortype.base_type) && !IsEnum(vectortype) ? "" : "?";

            if (vectortype.base_type != BASE_TYPE_UNION) {
                code_ += GenArrayMainBody(nullable) + GenOffset() + "\\";
            } else {
                code_ += "{{ACCESS_TYPE}} func {{FIELDVAR}}<T: FlatbuffersInitializable>(at "
                         "index: "
                         "Int32, type: T.Type) -> T? { "
                    + GenOffset() + "\\";
            }
            ...
            if (IsScalar(vectortype.base_type) && !IsEnum(vectortype) && !IsBool(field.value.type.base_type)) {
            ...
            if (vectortype.base_type == BASE_TYPE_STRUCT && field.value.type.struct_def->fixed) {
                ...
                code_.SetValue("VALUETYPE", GenType(field.value.type) + Mutable());
                code_ += GenArrayMainBody(nullable) + GenOffset() + const_string + GenConstructor("{{ACCESS}}.vector(at: o) + index * {{SIZE}}");
            ...
            if (IsEnum(vectortype)) {
                ...
                code_ += "return o == 0 ? {{VALUETYPE}}" + GenEnumDefaultValue(field) + " : {{VALUETYPE}}(rawValue: {{ACCESS}}.directRead(of: "
        ...
        void GenerateEncoderUnionBody(const FieldDef& field)
        {
            ...
            const auto is_vector = field.value.type.base_type == BASE_TYPE_VECTOR || field.value.type.base_type == BASE_TYPE_ARRAY;
            if (field.value.type.base_type == BASE_TYPE_UTYPE || (is_vector && field.value.type.VectorType().base_type == BASE_TYPE_UTYPE))
                return;
            if (is_vector) {
        ...
        void GenerateEncoderBody(const StructDef& struct_def)
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                const auto is_non_union_vector = (field.value.type.base_type == BASE_TYPE_ARRAY || field.value.type.base_type == BASE_TYPE_VECTOR) && field.value.type.VectorType().base_type != BASE_TYPE_UTYPE;
                ...
                } else if (IsScalar(type.base_type) && !IsEnum(type) && !IsBool(type.base_type) && !field.IsOptional()) {
                    code_ += "if {{FIELDVAR}} != {{CONSTANT}} {";
                ...
                } else if (is_non_union_vector && (!IsScalar(type.VectorType().base_type) || IsEnum(type.VectorType()))) {
                    code_ += "var contentEncoder = container.nestedUnkeyedContainer(forKey: "
                             ".{{FIELDVAR}})";
                    code_ += "for index in 0..<{{FIELDVAR}}Count {";
                    Indent();
                ...
                if (is_non_union_vector || (IsScalar(type.base_type) && !field.IsOptional())) {
                    code_ += "}";
                }
            }
        }
        ...
        {
            const auto is_vector = IsVector(field.value.type) || IsArray(field.value.type);
            if (field.value.type.base_type == BASE_TYPE_UTYPE || (is_vector && field.value.type.VectorType().base_type == BASE_TYPE_UTYPE))
                return;
            EnumDef& union_def = *field.value.type.enum_def;
            code_.SetValue("VALUETYPE", namer_.NamespacedType(union_def));
            code_.SetValue("FUNCTION_NAME", is_vector ? "visitUnionVector" : "visit");
            code_ += "try _v.{{FUNCTION_NAME}}(unionKey: {{TABLEOFFSET}}.{{OFFSET}}Type.p, "
            ...
            for (auto it = union_def.Vals().begin(); it != union_def.Vals().end();
                ...
                if (ev.union_type.base_type == BASE_TYPE_NONE) {
                    ...
                    code_.SetValue("MAINTYPE", ev.union_type.struct_def->fixed ? type : "ForwardOffset<" + type + ">");
        ...
        std::string GenerateVerifierType(const FieldDef& field)
        {
            ...
            if (is_vector) {
                ...
                return "ForwardOffset<Vector<" + GenerateNestedVerifierTypes(vector_type) + ", " + GenType(vector_type) + ">>";
            }
        ...
        void GenByKeyFunctions(const FieldDef& key_field)
        {
            ...
            code_ += "{{ACCESS_TYPE}} func {{FIELDVAR}}By(key: {{TYPE}}) -> {{VALUETYPE}}? "
                     "{ \\";
            code_ += GenOffset() + "return o == 0 ? nil : {{VALUETYPE}}.lookupByKey(vector: "
        ...
        void GenEnum(const EnumDef& enum_def)
        {
            if (enum_def.generated)
                return;
            const bool is_private_access = parser_.opts.swift_implementation_only || enum_def.attributes.Lookup("private") != nullptr;
            ...
            code_ += "{{ACCESS_TYPE}} enum {{ENUM_NAME}}: {{BASE_TYPE}}, {{ENUM_TYPE}} {";
            Indent();
            ...
            code_ += "{{ACCESS_TYPE}} static var byteSize: Int { return "
                     ...
                     "}";
            code_ += "{{ACCESS_TYPE}} var value: {{BASE_TYPE}} { return self.rawValue }";
        ...
        void GenerateObjectAPIExtensionHeader(std::string type_name)
        {
            ...
            Outdent();
            code_ += "}";
            code_ += "{{ACCESS_TYPE}} static func pack(_ builder: inout FlatBufferBuilder, "
                     "obj: "
                     "inout "
                + type_name + "?) -> Offset {";
            Indent();
        ...
        {
            code_ += "{{ACCESS_TYPE}} init(_ _t: inout {{STRUCTNAME}}" + Mutable() + ") {";
            Indent();
            for (auto it = struct_def.fields.vec.begin();
                 it != struct_def.fields.vec.end(); ++it) {
                const auto& field = **it;
        ...
        void GenerateObjectAPITableExtension(const StructDef& struct_def)
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                const auto type = GenType(field.value.type);
                std::string check_if_vector = (IsVector(field.value.type) || IsArray(field.value.type))
                    ? "VectorOf("
                    : "(";
                ...
                case BASE_TYPE_VECTOR: {
                    ...
                    unpack_body.push_back("{{STRUCTNAME}}." + body + "__" + field_var + builder);
                    break;
                }
                case BASE_TYPE_UNION: {
                    code_ += "let __" + field_var + " = obj." + field_var + "?.pack(builder: &builder) ?? Offset()";
                    unpack_body.push_back("if let o = obj." + field_var + "?.type {");
                    unpack_body.push_back("  {{STRUCTNAME}}.add(" + field_var + "Type: o" + builder);
                    unpack_body.push_back("  {{STRUCTNAME}}." + body + "__" + field_var + builder);
                ...
                case BASE_TYPE_STRUCT: {
                    if (field.value.type.struct_def && field.value.type.struct_def->fixed) {
                        ...
                        unpack_body.push_back("{{STRUCTNAME}}." + body + "obj." + field_field + builder);
                    } else {
                        code_ += "let __" + field_var + " = " + type + ".pack(&builder, obj: &obj." + field_field + ")";
                        unpack_body.push_back("{{STRUCTNAME}}." + body + "__" + field_var + builder);
                    }
                    break;
                }
                case BASE_TYPE_STRING: {
                    unpack_body.push_back("{{STRUCTNAME}}." + body + "__" + field_var + builder);
                    if (field.IsRequired()) {
                        code_ += "let __" + field_var + " = builder.create(string: obj." + field_field + ")";
                    } else {
        ...
        void GenerateVectorObjectAPITableExtension(const FieldDef& field_def)
        {
            ...
            case BASE_TYPE_UNION: {
                ...
                Outdent();
                code_ += "}";
                code_ += "let __" + var + " = builder.createVector(ofOffsets: __" + var + "__)";
                code_ += "let __" + var + "Type = builder.createVector(obj." + field + ".compactMap { $0?.type })";
                break;
            }
            case BASE_TYPE_UTYPE:
                break;
            case BASE_TYPE_STRUCT: {
                if (field_type.struct_def && !field_type.struct_def->fixed) {
                    ...
                    code_ += "{{STRUCTNAME}}." + namer_.Method("start_vector_of", var) + "(obj." + field + ".count, in: &builder)";
                    std::string code;
                    ...
                    Outdent();
                    code_ += "}";
                    code_ += "let __" + var + " = builder.endVector(len: obj." + field + ".count)";
                }
                break;
            }
            case BASE_TYPE_STRING: {
                code_ += "let __" + var + " = builder.createVector(ofStrings: obj." + var + ".compactMap({ $0 }) )";
                break;
            }
            default: {
            ...
            std::vector<std::string>& base_constructor)
        {
            ...
            case BASE_TYPE_STRUCT: {
                ...
                const auto optional = (field.value.type.struct_def && field.value.type.struct_def->fixed);
                std::string question_mark = (field.IsRequired() || (optional && is_fixed) ? "" : "?");

                code_ += "{{ACCESS_TYPE}} var {{FIELDVAR}}: {{VALUETYPE}}" + question_mark;
                base_constructor.push_back("" + field_var + " = " + objtype + "()");

                if (field.value.type.struct_def->fixed) {
                    buffer_constructor.push_back("" + field_var + " = _t." + field_field);
                } else {
                    buffer_constructor.push_back("var __" + field_var + " = _t." + field_field);
                    buffer_constructor.push_back(
                        "" + field_var + " = __" + field_var + (field.IsRequired() ? "!" : question_mark) + ".unpack()");
                }
                break;
            }
            case BASE_TYPE_ARRAY:
            ...
            case BASE_TYPE_STRING: {
                ...
                if (field.IsRequired()) {
                    std::string default_value = field.IsDefault() ? SwiftConstant(field) : "";
            ...
            default: {
                ...
                if (IsScalar(field.value.type.base_type) && !IsBool(field.value.type.base_type) && !IsEnum(field.value.type)) {
            ...
            const std::string& indentation)
        {
            ...
            if (vectortype.base_type != BASE_TYPE_UTYPE) {
                ...
                buffer_constructor.push_back("for index in 0..<_t." + field_field + "Count {");
            ...
            case BASE_TYPE_STRUCT: {
                ...
                if (!vectortype.struct_def->fixed) {
                    buffer_constructor.push_back(indentation + "var __v_ = _t." + field_field + "(at: index)");
                    buffer_constructor.push_back(indentation + field_var + ".append(__v_?.unpack())");
                } else {
                    buffer_constructor.push_back(indentation + field_var + ".append(_t." + field_var + "(at: index))");
                }
                break;
            }
            case BASE_TYPE_ARRAY:
            ...
            default: {
                ...
                if (IsEnum(vectortype) && vectortype.base_type != BASE_TYPE_UNION) {
                    const auto default_value = IsEnum(field.value.type)
                        ? GenEnumDefaultValue(field)
                        : SwiftConstant(field);
                    buffer_constructor.push_back(indentation + field_var + ".append(_t." + field_field + "(at: index)!)");
                    break;
                }
                buffer_constructor.push_back(indentation + field_var + ".append(_t." + field_field + "(at: index))");
                break;
            ...
            const bool is_vector = false)
        {
            ...
            buffer_constructor.push_back(indentation + "switch _t." + field + "Type" + vector_reader + (is_vector ? ")" : "") + " {");
            ...
            for (auto it = ed.Vals().begin(); it < ed.Vals().end(); ++it) {
                ...
                buffer_constructor.push_back(
                    indentation + "  var _v = _t." + field + (is_vector ? "" : "(") + vector_reader + (is_vector ? ", " : "") + "type: " + type + ".self)");
                const auto constructor = ns_type + "Union(_v?.unpack(), type: ." + variant + ")";
                buffer_constructor.push_back(
                    indentation + "  " + field + (is_vector ? ".append(" + constructor + ")" : " = " + constructor));
            }
        ...
        void AddMinOrMaxEnumValue(const std::string& str, const std::string& type)
        {
            ...
            code_ += "{{ACCESS_TYPE}} static var " + type + ": {{ENUM_NAME}} { return .{{" + type + "}} }";
        }
        ...
        void GenLookup(const FieldDef& key_field, const std::string& struct_type)
        {
            ...
            std::string offset_reader = "Table.offset(Int32(fbb.capacity) - tableOffset, vOffset: {{OFFSET}}, "
                                        "fbb: fbb)";

            code_.SetValue("TYPE", GenType(key_field.value.type));
            code_ += "fileprivate static func lookupByKey(vector: Int32, key: {{TYPE}}, "
                     "fbb: "
            ...
            code_ += "let tableOffset = Table.indirect(vector + 4 * (start + middle), fbb)";
            if (IsString(key_field.value.type)) {
                code_ += "let comp = Table.compare(" + offset_reader + ", key, fbb: fbb)";
            } else {
                code_ += "let comp = fbb.read(def: {{TYPE}}.self, position: Int(" + offset_reader + "))";
            }
            ...
            if (field.padding) {
                for (int i = 0; i < 4; i++) {
                    if (static_cast<int>(field.padding) & (1 << i)) {
                        ...
                        code_ += "private let padding" + NumToString((*id)++) + "__: UInt" + NumToString(bits) + " = 0";
                    }
                }
            ...
            const std::string& get_offset, bool isRaw = false)
        {
            return "@discardableResult {{ACCESS_TYPE}} func mutate({{FIELDVAR}}: "
                   "{{VALUETYPE}}) -> Bool {"
                + get_offset + " return {{ACCESS}}.mutate({{FIELDVAR}}" + (isRaw ? ".rawValue" : "") + ", index: " + offset + ") }";
        }
        ...
        std::string GenMutateArray()
        {
            return "{{ACCESS_TYPE}} func mutate({{FIELDVAR}}: {{VALUETYPE}}, at "
                   "index: Int32) -> Bool { "
                + GenOffset() + "return {{ACCESS}}.directMutate({{FIELDVAR}}, index: "
                                "{{ACCESS}}.vector(at: o) + index * {{SIZE}}) }";
        }
        ...
        {
            const auto default_value = StringIsFlatbufferNan(field.value.constant) ? ".nan" : StringIsFlatbufferPositiveInfinity(field.value.constant) ? ".infinity"
