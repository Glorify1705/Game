    ...
    public:
            ...
            std::set<std::string>& imports)
        {
            for (const auto& item : definitions) {
                if (item->file == included) {
                    ...
                    std::string filebase = flatbuffers::StripPath(flatbuffers::StripExtension(item->file));
                    std::string filename = namer_.File(filebase + (component.empty() ? "" : "_" + component));

                    imports.emplace("import './" + filename + "'" + (component.empty() ? ";\n" : " as " + ImportAliasName(component) + ";\n"));
                }
            }
        }
        ...
        bool generate()
        {
            ...
            for (const auto& included_file : parser_.GetIncludedFiles()) {
                if (included_file.filename == parser_.file_being_parsed_)
                    continue;
            ...
            for (auto kv = namespace_code.begin(); kv != namespace_code.end(); ++kv) {
                ...
                code = code + "// ignore_for_file: unused_import, unused_field, unused_element, "
        ...
        {
            return (path ? path_ : "") + namer_.File(file_name_ + (suffix.empty() ? "" : "_" + suffix));
        }
    ...
    private:
        ...
        void GenEnum(EnumDef& enum_def, namespace_code_map& namespace_code)
        {
            if (enum_def.generated)
                return;
            std::string& code = namespace_code[namer_.Namespace(*enum_def.defined_namespace)];
            GenDocComment(enum_def.doc_comment, "", code);

            const std::string enum_type = namer_.Type(enum_def) + (enum_def.is_union ? "TypeId" : "");
            ...
            // holes.
            if (!is_bit_flags) {
                code += "  static const int minValue = " + enum_def.ToString(*enum_def.MinValue()) + ";\n";
        ...
        void GenEnumReader(EnumDef& enum_def, const std::string& enum_type,
            std::string& code)
        {
            code += "class _" + enum_type + "Reader extends " + _kFb + ".Reader<" + enum_type + "> {\n";
            ...
            bool constConstruct = true)
        {
            ...
            if (type.base_type == BASE_TYPE_BOOL) {
                ...
                return prefix + ".ListReader<" + GenDartTypeName(type.VectorType(), current_namespace, def) + ">(" + GenReaderTypeName(type.VectorType(), current_namespace, def, true, true, false) + (lazy ? ")" : ", lazy: false)");
            ...
            std::string struct_type_suffix = "")
        {
            ...
            case BASE_TYPE_VECTOR:
                return "List<" + GenDartTypeName(type.VectorType(), current_namespace, def, struct_type_suffix) + ">";
            default:
                assert(0);
                return "dynamic";
            }
        ...
        std::string GenDartTypeName(const Type& type, Namespace* current_namespace,
            ...
            std::string struct_type_suffix)
        {
            std::string typeName = GenDartTypeName(type, current_namespace, def, struct_type_suffix);
            ...
            const FieldDef& field) const
        {
            ...
            const std::string field_namespace = field.value.type.struct_def
            ...
            namespace_code_map& namespace_code)
        {
            ...
            std::string& code = namespace_code[namer_.Namespace(*struct_def.defined_namespace)];
            ...
            const std::vector<std::pair<int, FieldDef*>>& non_deprecated_fields)
        {
            ...
            for (auto it = non_deprecated_fields.begin();
                ...
                const std::string type_name = GenDartTypeName(field.value.type, struct_def.defined_namespace, field,
                ...
                if (!struct_def.fixed && !defaultValue.empty()) {
                    if (IsEnum(field.value.type)) {
                        ...
                        if (auto val = enum_def.FindByValue(defaultValue)) {
                            ...
                            constructor_args += " = const " + namer_.Type(enum_def) + "._(" + defaultValue + ")";
                        }
                    } else {
            ...
            const std::vector<std::pair<int, FieldDef*>>& non_deprecated_fields)
        {
            ...
            for (auto it = non_deprecated_fields.begin();
                ...
                if (type.base_type == BASE_TYPE_STRUCT) {
                    constructor_args += field_name + nullableValueAccessOperator + ".unpack()";
                } else if (type.base_type == BASE_TYPE_VECTOR) {
                    if (type.VectorType().base_type == BASE_TYPE_STRUCT) {
                        constructor_args += field_name + nullableValueAccessOperator + ".map((e) => e.unpack()).toList()";
                    } else {
                        constructor_args += GenReaderTypeName(field.value.type, struct_def.defined_namespace,
                            field, false, false);
                        constructor_args += ".vTableGet";
                        std::string offset = NumToString(field.value.offset);
                        constructor_args += isNullable
        ...
        std::string NamespaceAliasFromUnionType(Namespace* root_namespace,
            const Type& type)
        {
            const std::vector<std::string> qualified_name_parts = type.struct_def->defined_namespace->components;
            ...
            for (auto it = non_deprecated_fields.begin();
                ...
                if (field.value.type.base_type == BASE_TYPE_UNION) {
                    ...
                    for (auto en_it = enum_def.Vals().begin() + 1;
                        ...
                        code += "      case " + enum_def.ToString(ev) + ": return " + enum_name + ".reader.vTableGetNullable(_bc, _bcOffset, " + NumToString(field.value.offset) + ");\n";
                    }
                    ...
                    code += " => ";
                    if (field.value.type.enum_def && field.value.type.base_type != BASE_TYPE_VECTOR) {
                    ...
                    code += GenReaderTypeName(field.value.type,
                        struct_def.defined_namespace, field);
                    if (struct_def.fixed) {
                        code += ".read(_bc, _bcOffset + " + NumToString(field.value.offset) + ")";
                    } else {
                        code += ".vTableGet";
                        std::string offset = NumToString(field.value.offset);
                        if (isNullable) {
            ...
            for (auto it = non_deprecated_fields.begin();
                ...
                if (IsScalar(field.value.type.base_type)) {
                    ...
                    code += GenDartTypeName(field.value.type, struct_def.defined_namespace,
                        field);
                    code += "? " + field_var + ") {\n";
                    code += "    fbBuilder.add" + GenType(field.value.type) + "(" + NumToString(offset) + ", ";
                    code += field_var;
                    if (field.value.type.enum_def) {
                        code += "?.value";
                    }
            ...
            const std::string& builder_name, std::string& code)
        {
            ...
            for (auto it = non_deprecated_fields.begin();
                ...
                code += "  final " + GenDartTypeName(field.value.type, struct_def.defined_namespace, field, !struct_def.fixed, "ObjectBuilder") + " _" + namer_.Variable(field) + ";\n";
            }
            code += "\n";
            code += "  " + builder_name + "(";
            ...
            if (non_deprecated_fields.size() != 0) {
                ...
                for (auto it = non_deprecated_fields.begin();
                    ...
                    code += "    ";
                    code += (struct_def.fixed ? "required " : "") + GenDartTypeName(field.value.type, struct_def.defined_namespace, field, !struct_def.fixed, "ObjectBuilder") + " " + namer_.Variable(field) + ",\n";
                }
                code += "  })\n";
                code += "      : ";
                for (auto it = non_deprecated_fields.begin();
            ...
            bool prependUnderscore = true, bool pack = false)
        {
            ...
            for (auto it = non_deprecated_fields.begin();
                ...
                std::string field_name = (prependUnderscore ? "_" : "") + namer_.Variable(field);
                // custom handling for fixed-sized struct in pack()
                if (pack && IsVector(field.value.type) && field.value.type.VectorType().base_type == BASE_TYPE_STRUCT && field.value.type.struct_def->fixed) {
                ...
                if (IsVector(field.value.type)) {
                    ...
                    case BASE_TYPE_STRING:
                        code += "(" + field_name + "!.map(fbBuilder.writeString).toList());\n";
                        break;
                    case BASE_TYPE_STRUCT:
                        if (field.value.type.struct_def->fixed) {
                            code += "OfStructs(" + field_name + "!);\n";
                        } else {
                            code += "(" + field_name + "!.map((b) => b." + (pack ? "pack" : "getOrCreateOffset") + "(fbBuilder)).toList());\n";
                        }
                        break;
                    default:
            ...
            bool prependUnderscore = true, bool pack = false)
        {
            ...
            for (auto it = non_deprecated_fields.begin();
                ...
                std::string field_var = (prependUnderscore ? "_" : "") + namer_.Variable(field);

                if (IsScalar(field.value.type.base_type)) {
                    code += "    fbBuilder.add" + GenType(field.value.type) + "(" + NumToString(offset) + ", " + field_var;
                    if (field.value.type.enum_def) {
...
std::string DartMakeRule(const Parser& parser, const std::string& path,
    const std::string& file_name)
{
    auto filebase = flatbuffers::StripPath(flatbuffers::StripExtension(file_name));
    dart::DartGenerator generator(parser, path, file_name);
