...
namespace go {
    ...
    public:
        ...
        bool generate()
        {
            ...
            if (parser_.opts.one_file) {
                ...
                code += one_file_code;
                const std::string filename = GeneratedFileName(path_, file_name_, parser_.opts);
    ...
    private:
            ...
            const FieldDef& field, std::string* code_ptr)
        {
            ...
            code += "() " + TypeName(field) + " {\n";
            code += "\treturn " + CastToEnum(field.value.type, getter + "(rcv._tab.Pos + flatbuffers.UOffsetT(" + NumToString(field.value.offset) + "))");
            code += "\n}\n";
        }
            ...
            std::string* code_ptr)
        {
            ...
            auto kit = std::find_if(vector_struct_fields.begin(), vector_struct_fields.end(),
            ...
            code += "(obj *" + TypeName(field);
            code += ", key " + NativeType(key_field.value.type) + ") bool" + OffsetPrefix(field);
            code += "\t\tx := rcv._tab.Vector(o)\n";
            code += "\t\treturn ";
            ...
            std::string* code_ptr)
        {
            ...
            code += "\t\treturn " + CastToEnum(field.value.type, GenGetter(field.value.type) + "(a + flatbuffers.UOffsetT(j*" + NumToString(InlineSize(vectortype)) + "))");
            code += "\n\t}\n";
        ...
        {
            std::string& code = *code_ptr;
            std::string setter = "rcv._tab.Mutate" + namer_.Method(GenTypeBasic(field.value.type));
            GenReceiver(struct_def, code_ptr);
        ...
        {
            std::string& code = *code_ptr;
            std::string setter = "rcv._tab.Mutate" + namer_.Method(GenTypeBasic(field.value.type)) + "Slot";
            GenReceiver(struct_def, code_ptr);
            code += " Mutate" + namer_.Function(field);
            code += "(n " + GenTypeGet(field.value.type) + ") bool {\n\treturn ";
            ...
            std::string* code_ptr)
        {
            std::string& code = *code_ptr;
            auto vectortype = field.value.type.VectorType();
            std::string setter = "rcv._tab.Mutate" + namer_.Method(GenTypeBasic(vectortype));
            GenReceiver(struct_def, code_ptr);
            code += " Mutate" + namer_.Function(field);
            ...
            std::string* code_ptr)
        {
            ...
            if (IsString(field.value.type)) {
                ...
                code += "\t\tcomp := bytes.Compare(obj." + namer_.Function(field.name) + "()";
                code += ", bKey)\n";
            } else {
        ...
        void GenNativeStruct(const StructDef& struct_def, std::string* code_ptr)
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (field.deprecated)
                    continue;
                if (IsScalar(field.value.type.base_type) && field.value.type.enum_def != nullptr && field.value.type.enum_def->is_union)
                    continue;
        ...
        void GenNativeUnionUnPack(const EnumDef& enum_def, std::string* code_ptr)
        {
            ...
            code += "func (rcv " + namer_.Type(enum_def) + ") UnPack(table flatbuffers.Table) *" + NativeName(enum_def) + " {\n";
            code += "\tswitch rcv {\n";
            ...
            for (auto it2 = enum_def.Vals().begin(); it2 != enum_def.Vals().end();
                ...
                code += "\t\treturn &" + WrapInNameSpaceAndTrack(&enum_def, NativeName(enum_def)) + "{ Type: " + namer_.EnumVariant(enum_def, ev) + ", Value: x.UnPack() }\n";
            }
            code += "\t}\n";
            code += "\treturn nil\n";
            code += "}\n\n";
        ...
        void GenNativeTablePack(const StructDef& struct_def, std::string* code_ptr)
        {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (IsString(field.value.type)) {
                    ...
                    code += "\t}\n";
                } else if (IsVector(field.value.type) && field.value.type.element == BASE_TYPE_UCHAR && field.value.type.enum_def == nullptr) {
                    code += "\t" + offset + " := flatbuffers.UOffsetT(0)\n";
                    code += "\tif t." + field_field + " != nil {\n";
                    code += "\t\t" + offset + " = builder.CreateByteString(t." + field_field + ")\n";
                    code += "\t}\n";
                } else if (IsVector(field.value.type)) {
                    code += "\t" + offset + " := flatbuffers.UOffsetT(0)\n";
                    code += "\tif t." + field_field + " != nil {\n";
                    ...
                    if (field.value.type.element == BASE_TYPE_STRING) {
                        code += "\t\t" + offsets + " := make([]flatbuffers.UOffsetT, " + length + ")\n";
                        code += "\t\tfor j := 0; j < " + length + "; j++ {\n";
                        code += "\t\t\t" + offsets + "[j] = builder.CreateString(t." + field_field + "[j])\n";
                        code += "\t\t}\n";
                    } else if (field.value.type.element == BASE_TYPE_STRUCT && !field.value.type.struct_def->fixed) {
                        code += "\t\t" + offsets + " := make([]flatbuffers.UOffsetT, " + length + ")\n";
                        code += "\t\tfor j := 0; j < " + length + "; j++ {\n";
                        code += "\t\t\t" + offsets + "[j] = t." + field_field + "[j].Pack(builder)\n";
                        code += "\t\t}\n";
                    }
                    code += "\t\t" + struct_type + "Start" + namer_.Function(field) + "Vector(builder, " + length + ")\n";
                    code += "\t\tfor j := " + length + " - 1; j >= 0; j-- {\n";
                    if (IsScalar(field.value.type.element)) {
                        code += "\t\t\tbuilder.Prepend" + namer_.Method(GenTypeBasic(field.value.type.VectorType())) + "(" + CastToBaseType(field.value.type.VectorType(), "t." + field_field + "[j]") + ")\n";
                    } else if (field.value.type.element == BASE_TYPE_STRUCT && field.value.type.struct_def->fixed) {
            ...
            for (auto it = struct_def.fields.vec.begin();
                ...
                if (IsScalar(field.value.type.base_type)) {
                    ...
                    if (field.value.type.enum_def == nullptr || !field.value.type.enum_def->is_union) {
                    ...
                    if (field.value.type.base_type == BASE_TYPE_STRUCT && field.value.type.struct_def->fixed) {
                        code += "\t" + offset + " := t." + field_field + ".Pack(builder)\n";
                    } else if (field.value.type.enum_def != nullptr && field.value.type.enum_def->is_union) {
                        code += "\tif t." + field_field + " != nil {\n";
            ...
            std::string* code_ptr)
        {
            ...
            code += "func (rcv *" + struct_type + ") UnPackTo(t *" + NativeName(struct_def) + ") {\n";
            for (auto it = struct_def.fields.vec.begin();
                 it != struct_def.fields.vec.end(); ++it) {
                const FieldDef& field = **it;
                if (field.deprecated)
                ...
                if (IsScalar(field.value.type.base_type)) {
                    ...
                    if (field.value.type.element == BASE_TYPE_STRUCT) {
                        code += "\t\tx := " + WrapInNameSpaceAndTrack(field.value.type.struct_def, field.value.type.struct_def->name) + "{}\n";
            ...
            code += "func (rcv *" + struct_type + ") UnPack() *" + NativeName(struct_def) + " {\n";
            ...
            std::string* code_ptr)
        {
            ...
            code += "func (rcv *" + namer_.Type(struct_def) + ") UnPackTo(t *" + NativeName(struct_def) + ") {\n";
            for (auto it = struct_def.fields.vec.begin();
                 it != struct_def.fields.vec.end(); ++it) {
                const FieldDef& field = **it;
                if (field.value.type.base_type == BASE_TYPE_STRUCT) {
            ...
            code += "func (rcv *" + namer_.Type(struct_def) + ") UnPack() *" + NativeName(struct_def) + " {\n";
            code += "\tif rcv == nil { return nil }\n";
            code += "\tt := &" + NativeName(struct_def) + "{}\n";
            code += "\trcv.UnPackTo(t)\n";
            code += "\treturn t\n";
      ...
      #undef FLATBUFFERS_TD
        ...
        std::string GenConstant(const FieldDef& field)
        {
            ...
            case BASE_TYPE_DOUBLE: {
                const std::string float_type = field.value.type.base_type == BASE_TYPE_FLOAT ? "float32"
                                                                                             : "float64";
                ...
                if (tracked_imported_namespaces_.size() > 0) {
                    ...
                    for (auto it = tracked_imported_namespaces_.begin();
                        ...
                        if ((*it)->defined_namespace->components.empty()) {
                            ...
                            code += "\t" + NamespaceImportName((*it)->defined_namespace) + " \"" + NamespaceImportPath((*it)->defined_namespace) + "\"\n";
                        }
                    }
                }
                code += ")\n\n";
        ...
        {
            if (CurrentNameSpace() == def->defined_namespace)
                return name;
