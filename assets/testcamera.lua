...
function g:update(t, dt)
  ...
  -- Ship movement.
  local dx, dy = 0, 0
  if G.input.is_key_down("w") then dy = -1 end
  if G.input.is_key_down("s") then dy = 1 end
  if G.input.is_key_down("a") then dx = -1 end
  ...
  if scroll ~= 0 then
    ...
    z = z + scroll * 0.1
    if z < 0.25 then z = 0.25 end
    if z > 4.0 then z = 4.0 end
    G.camera.set_zoom(z)
  end
