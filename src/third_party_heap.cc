...
thread_local mi_heap_t* g_thread_heap = nullptr;

mi_heap_t* ThreadHeap() {
  DCHECK(g_initialized, "InitThirdPartyHeap was not called");
  if (g_thread_heap == nullptr) {
    g_thread_heap = mi_heap_new_in_arena(g_arena_id);
    CHECK(g_thread_heap != nullptr, "Failed to create third-party heap");
  }
  return g_thread_heap;
}

}  // namespace

void InitThirdPartyHeap(size_t size) {
  CHECK(!g_initialized, "InitThirdPartyHeap called twice");
