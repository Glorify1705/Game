  ...
  if (ImGui::CollapsingHeader("Draw Calls", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::TreeNode("Flush Reasons")) {
      ...
      if (fs.flush_overflow > 0) {
        ImGui::TextColored(ImVec4(1.0f, 0.3f, 0.3f, 1.0f), "Overflow:  %d",
                           fs.flush_overflow);
      }
      ImGui::TreePop();
    }
...
void DebugUI::DrawFrameBreakdown(const FrameContext& ctx) {
  ...
  if (total > 0.0f) {
    ...
    for (size_t i = 0; i < 4; ++i) {
      ...
      ImGui::ColorButton(
          cats[i].name, ImColor(cats[i].color).Value,
          ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoPicker,
          ImVec2(10, 10));
      ImGui::SameLine();
      ImGui::Text("%s %.1f", cats[i].name, static_cast<double>(cats[i].ms));
    }
  }
  ...
  if (count > 0) {
    ...
    dl->AddRectFilled(cursor,
                      ImVec2(cursor.x + chart_width, cursor.y + chart_height),
