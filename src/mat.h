...
struct FMat2x2 {
  using type = float;
  inline static constexpr std::size_t kDimension = 2;
  ...
  explicit FMat2x2(float value) {
    for (std::size_t i = 0; i < kCardinality; ++i) {
      v[i] = value;
    }
  }
  ...
  FMat2x2& operator*=(float val) {
    for (std::size_t i = 0; i < kCardinality; ++i) v[i] *= val;
    return *this;
  }

  FMat2x2 operator*(float val) const {
    auto result = *this;
    for (std::size_t i = 0; i < kCardinality; ++i) result.v[i] *= val;
    return result;
  }
...
struct FMat3x3 {
  ...
  inline static constexpr std::size_t kCardinality = 9;
...
struct FMat4x4 {
  ...
  inline static constexpr std::size_t kCardinality = 16;
...
struct IMat2x2 {
  ...
  int val(std::size_t i, std::size_t j) const { return v[i * kDimension + j]; }

  int& mut(std::size_t i, std::size_t j) { return v[i * kDimension + j]; }
