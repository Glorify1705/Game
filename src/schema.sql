...
--Asset contents live outside the database in a content -
    addressed blob-- store(loose files in dev mode,
                           assets.zip in packaged builds),
    keyed by-- blob_hash.hash is the checksum of the source file,
    used to skip-- unchanged files when repacking.CREATE TABLE IF NOT EXISTS
    asset_metadata(id INTEGER PRIMARY KEY AUTOINCREMENT,
                   name VARCHAR(255) UNIQUE NOT NULL,
                   type VARCHAR(255) NOT NULL, size INTEGER NOT NULL,
                   hash INTEGER NOT NULL, blob_hash INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS sdf_cache(id INTEGER PRIMARY KEY AUTOINCREMENT,
