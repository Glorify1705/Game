...
void DebugUI::DrawDocsPanel() {
  ...
  if (!ImGui::Begin("API Docs", nullptr, ImGuiWindowFlags_NoFocusOnAppearing)) {
    ImGui::End();
    return;
  }

  ImGui::SetNextItemWidth(-1);
  ImGui::InputTextWithHint("##docs_filter", "Search functions...", docs_filter_,
                           sizeof(docs_filter_));
  ImGui::Separator();
  ...
  auto LibraryMatchesFilter = [&](int lib_idx,
    ...
    LuaTableForEach(L, lib_idx, [&](const char*, std::string_view fname) {
      if (ContainsCI(fname, filter)) {
        found = true;
        return true;
      }
      auto ds = LuaGetString(L, -1, "docstring");
      if (!ds.empty() && ContainsCI(ds, filter)) {
        found = true;
        return true;
      }
      return false;
  ...
  LuaTableForEach(
      L, docs_idx, [&](const char* lib_cstr, std::string_view lib_name) {
        ...
        ImGuiTreeNodeFlags lib_flags = has_filter
        ...
        LuaTableForEach(
            L, lib_idx, [&](const char* func_cstr, std::string_view func_name) {
              if (!lua_istable(L, -1)) return false;
              ...
              if (lua_istable(L, -1)) {
                ...
                if (lua_objlen(L, args_idx) > 0) {
                  ...
                  LuaArrayForEach(L, args_idx, [&](int) {
                    ...
                    if (!adoc.empty()) {
                      ImGui::BulletText(
                          "%.*s - %.*s", static_cast<int>(aname.size()),
                          aname.data(), static_cast<int>(adoc.size()),
                          adoc.data());
                    } else {
                      ImGui::BulletText("%.*s", static_cast<int>(aname.size()),
                                        aname.data());
                    }
                  });
                }
